 ...
 */
// X.KF.W10 G-7 (the `v/w9-staging` landing) — these factories produce the
// CONSTRUCTOR's layer argument, whose declared type is
// `Partial<AnimationLayerConfig>` (`group.ts:182` / `AnimationGroupInput.layer`),
// not the fully-populated interface: the engine merges each partial over
// `defaultLayerConfig` (`{ zIndex: 0, weight: 1, op: "replace", enabled: true }`).
// The branch staged them as the full `AnimationLayerConfig` back when `weight`
// and `enabled` were optional; both are required at the landing tree, so the
// staged spelling no longer typechecks. The partial is the honest type of what
// these factories return, and naming it leaves every value below untouched —
// `addPlain` still omits `weight` so the engine's default 1 fills it, which is
// exactly the arm the golden's "`weight` is inert on op:add" row measures.
function symmetricGroup(
    layer: (z: number) => Partial<AnimationLayerConfig>,
): AnimationGroup<any> {
...
const addWeight = (z: number): Partial<AnimationLayerConfig> => ({
    op: "add",
    zIndex: z,
    weight: 0.5,
});
const addPlain = (z: number): Partial<AnimationLayerConfig> => ({
    op: "add",
    zIndex: z,
});
const replaceWeight = (z: number): Partial<AnimationLayerConfig> => ({
    op: "replace",
    zIndex: z,
    weight: 0.5,
});
