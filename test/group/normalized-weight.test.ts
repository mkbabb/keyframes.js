...
const value = (out: Record<string, unknown>, key: string): number => {
    const authored = out[key];
    if (typeof authored !== "number") {
        throw new TypeError(`Expected ${key} to be an authored number.`);
    }
    return authored;
};
...
describe("U.C15 normalized layer weight axis", () => {
    it.each([
        ...
        [-1, 0],
        [Number.NaN, 0],
    ] as const)("normalizes static weight=%s to %s", (weight, expected) => {
        ...
        const group = new AnimationGroup<any>(
            { animation: base },
            { animation: top, layer: { op: "replace", weight } },
        );
        expect(value(compositeFramesAt(group, 0), "opacity")).toBeCloseTo(
            expected,
            8,
        );
    });
    ...
    it("leaves a live spring's overshoot available to the physical path", () => {
        ...
        const group = new AnimationGroup<any>(
            { animation: base },
            { animation: top, layer: { op: "replace", weight: 0.5 } },
        );
        ...
        const layer =
            group.animations[group.getEntries()[1]!.animation.id]?.layer;
