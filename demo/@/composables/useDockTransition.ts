...
export function useDockTransition(options: UseDockTransitionOptions) {
    ...
    watch(expanded, () => {
        ...
        fadeTimer = setTimeout(() => {
            ...
            nextTick(() => {
                ...
                // Set back to old width, restore transitions, animate
                el.style.width = `${from}px`;
    ...
    return {
        /** Which layer to show — use for :class="{ 'layer-active': visualExpanded }" */
        visualExpanded,
        /** Whether a transition is in progress — use for fade class */
        isTransitioning,
        /** Bind to @transitionend on the root element */
        onTransitionEnd,
    };
}
