 ...
 *      test (test/demo/scenes/amiga-sphere-spin.test.ts — real Three.js mesh/camera/canvas,
...
async function browserHalf() {
    const result = await withPage(
        ...
        // The RIGOROUS proof is test/demo/scenes/amiga-sphere-spin.test.ts (run by the npm
        ...
        if (moved == null) {
            ...
            console.log(
                `  ○ amiga engine-drives-mesh (live): only ${moved.changed} changed frame(s) sampled ` +
                    `(headless WebGL readback confound; the deterministic anchor is test/demo/scenes/amiga-sphere-spin.test.ts)`,
            );
        }
