#!/usr/bin/env node
/**
 * live-session — interaction-driven observation over the built demo.
 *
 ...
 *   KF_REQUIRE_BROWSER=1 KF_PLAYWRIGHT_DIR=… node scripts/observe/demo/live-session.mjs
 */
...
console.log(
    "live-session observation — ONE interaction-driven session over the BUILT dist: " +
...
async function runBattery() {
    ...
    const result = await withPage(
        ...
        // ════════════════════════════════════════════════════════════════════
        {
            ...
            // Glass owns the Drawer's reduced-motion spring. This observation
            // keeps the Keyframes control-dots assertion and records the sheet
            // trail as corroboration rather than duplicating Glass internals.
            dom.S2 = {
                prmMatches,
                controlChurn,
                prmChurn,
                sheetTrail,
                sheetLeg: "Glass-owned Drawer spring; trail recorded only",
        ...
        // Optional source-mapped corroboration of the visibility transition.
        if (USE_DEV_SERVER) {
            await runDevServerB2(budget);
        } else {
            note(
                "B2 dev-server leg skipped (KF_DEV_SERVER unset); the built-dist visibility transition passed above.",
            );
        }
        },
    );
...
async function runDevServerB2(budget) {
    ...
    try {
        ...
        if (after === before) {
            ok(`B2 DEV-SERVER (:5174): the synthetic visibilitychange→hidden on the source-mapped playing easing scene raised zero _gen throws.`);
        } else {
...
function reportBattery(budget, dom) {
    ...
    const verdict = (key, label) => {
        const d = dom[key];
        if (!d) {
            note(`${label} — leg did not run (no DOM verdict captured)`);
            return;
        }
        if (d.pass) ok(`${label} — PASS · ${JSON.stringify(d).slice(0, 180)}`);
        else fail(`${label} — FAIL · ${JSON.stringify(d).slice(0, 220)}`);
    };

    verdict("B1", "B1 COLD hero play STARTS the engine (aria flips Play→Pause + slider advances from \"0\"; idle bob excluded)");
    verdict("B2", "B2 synthetic visibilitychange on a playing scene raises NO _gen throw");
    verdict("B4", "B4 switch-into-easing mounts the EasingPicker + a handle-drag mutates");
    verdict("B3", "B3 amiga centre-drag moves the SUBJECT not the room");
    verdict("B6", "B6 /square drag selects NO text + the transform PERSISTS");
    verdict("B7", "B7 the glass ::before carry NO bloom at rest");
    verdict("B9", "B9 every scene glyph PAINTS");
    verdict("font", "font register — the body font IS Plus Jakarta Sans (T.D3/OD-6)");
    verdict("S5", "S5 EVERY routed scene enters clean + plays/interacts + the covering dock-switch walk lands");
    verdict("S2", "S2 the engine's respectReducedMotion path SNAPS live under PRM (dots rest · sheet one-emit)");
    verdict("S4", "S4 keyboard operability — Tab reaches play · focus-visible ring · Enter + global Space actuate");
    ...
    for (const [key, label] of [
        ["B1", "B1 cold-hero-starts-engine"],
        ["B2", "B2 suspend-no-throw"],
        ["B4", "B4 easing-mount+drag"],
        ["B3", "B3 amiga-subject"],
        ["B6", "B6 square-drag"],
        ["B7", "B7 specular-rest"],
        ["B9", "B9 glyph-paint"],
        ["font", "font-reclaim"],
        ["S5", "S5 every-scene-sweep"],
        ["S2", "S2 reduced-motion-snap"],
        ["S4", "S4 keyboard-focus"],
    ]) {
        const d = dom[key];
        if (d && !d.pass && !failures.some((f) => f.includes(label))) {
            failures.push(`${label} DOM leg failed`);
        }
    }
}
...
if (failures.length > 0) {
    console.error(
        `\nlive-session observation — FAIL (${failures.length}): the human battery (PLAY + SWITCH + DRAG) blew the ` +
            `ERROR BUDGET (S2a) or a product-facing DOM assertion failed.`,
    );
    process.exit(1);
}
console.log(
    "\nlive-session observation — PASS: ONE interaction-driven session over the BUILT dist drove the rainbow play + " +
        ...
        "persisting (B6), the glass ::before flat at rest (B7), every glyph painting (B9), the body font is " +
        "Plus Jakarta Sans (font) — PLUS the J.W4 axes legs: every routed scene entered + played + switched through " +
        "(S5), the live reduced-motion snap (S2), and keyboard operability with a visible focus ring (S4). " +
        "These are direct product observations of what a human using the built demo would see.",
);
