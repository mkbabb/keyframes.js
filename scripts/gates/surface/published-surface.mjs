#!/usr/bin/env node
/**
 * Published-surface package-boundary oracle (J.W5 §Hard gate).
 *
 ...
 *     `scripts/gates/surface/readme-runs.mjs`, a sibling invoked by
 *     `proof:publish`; the package boundary is green only when both pass.
 *
 ...
 * CI wiring: the package-boundary command owns this check.
 */
...
// registry-artifact comparator so release rehearsal can reuse this implementation
// without adding a new package command.
if (process.argv.includes("--diff")) {
    const helper = path.join(REPO, "scripts/gates/surface/verify-diff.mjs");
...
function clauseA() {
    ...
    // The historical typing-dots check emitted beside the library dist; gitignored via the
...
function clauseE() {
    ...
    // the historical scene-colocation check). S.D3 (C-4) DELETED demo/playground/ — the standalone
...
// boundary backstop. The boundary check owns the bundle-level value.js-edge
// proof — this clause does not re-implement it; it locks the published-surface
// certification (the half genuinely missing pre-Q.WA2). The drag2D value export is asserted
...
// ═════════════════════════════════════════════════════════════════════════════
// clause (h) — no `_2`-suffixed collision aliases in the d.ts
