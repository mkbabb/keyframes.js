 ...
 * FORWARD (root) vs BACKWARD (`compile/backward/`), the FORWARD re-export ceremony
 * through `frame-compiler` is DEAD, and the backward leg reaches sibling zones
 ...
 *      `compile/backward/`, NEVER in `compile/` root; the FORWARD set
 *      (`frame-compiler`, `parse-flatten`, `easing-registry`, `easing-option`,
 *      `selector`, `numeric-plan`) + `adapter` (C-9) stay in root. A backward
 *      basename re-appearing in root REDs; a backward file missing from
 *      `compile/backward/` REDs. (a18: the real seam is FORWARD vs BACKWARD.)
 *
...
const BACKWARD = path.join(COMPILE, "backward");
...
// The backward-leg basenames (a18 C-2). These live ONLY under compile/backward/.
 ...
 * @param backwardFiles Set<basename> present in compile/backward/.
 * @param contents   Map<relPath, source> for the modules clauses 2/3 read.
 */
function runGate({ rootFiles, backwardFiles, contents }) {
    const failures = [];
        ...
        if (rootFiles.has(base)) {
            failures.push(
                `(1) [compile-root-forward-only] compile/${base} is in the ` +
                    ...
                    `compile/backward/ (a18 FORWARD-vs-BACKWARD seam; C-2).`,
            );
        }
        if (!backwardFiles.has(base)) {
            failures.push(
                `(1) [compile-root-forward-only] compile/backward/${base} is ` +
                    `MISSING — the backward leg (backward/backward-walk/` +
                    `backward-color/format) must live under compile/backward/.`,
            );
        }
    }

    // ── 2. no-re-export-bridge ───────────────────────────────────────────────
    // Every NON-barrel .ts under compile/ (root + backward/) must carry NO
    ...
    if (backwardMain !== undefined) {
        ...
        if (deep) {
            failures.push(
                `(3) [no-cross-zone-deep-import] compile/backward/backward.ts ` +
    ...
    const backwardWalk = contents.get(
        "src/animation/compile/backward/backward-walk.ts",
    );
    if (backwardWalk !== undefined) {
        ...
        if (fromEngine || !fromLeaf) {
            failures.push(
                `(3) [no-cross-zone-deep-import] compile/backward/backward-walk.ts ` +
...
function liveView() {
    ...
    const backwardFiles = new Set(
        fs.existsSync(BACKWARD)
            ? fs
                  .readdirSync(BACKWARD, { withFileTypes: true })
                  .filter((e) => e.isFile())
                  .map((e) => e.name)
            : [],
    );
    // Read every non-barrel .ts under compile/ (root + backward/) for clauses 2/3.
    ...
    collect(COMPILE);
    return { rootFiles, backwardFiles, contents };
}
...
if (PLANT_TEST) {
    ...
    {
        const c = new Map(base.contents);
        const bw = "src/animation/compile/backward/backward.ts";
        c.set(
            bw,
            (c.get(bw) ?? "") +
    ...
    {
        const c = new Map(base.contents);
        const bw = "src/animation/compile/backward/backward-walk.ts";
        c.set(
            bw,
...
if (failures.length > 0) {
    ...
    console.error(
        "\n  The BACKWARD leg lives under compile/backward/; the FORWARD re-export\n" +
            "  ceremony through frame-compiler is dead; the backward leg reaches sibling\n" +
