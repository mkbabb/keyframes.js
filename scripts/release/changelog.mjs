#!/usr/bin/env node
/**
 * release:changelog — the generalized breaking-set migration-doc oracle (S.C1 · C-18).
 *
 * Semver honesty: a BREAKING removal from the published surface MUST be documented
 * for the consumer. The retired 5.0-only predecessor (Q.WE1) hardcoded the
...
console.log(`release:changelog — migration check (current v${CURRENT})\n`);
...
function report() {
    if (failures.length > 0) {
        console.error(
            `\nrelease:changelog — FAIL (${failures.length} finding(s); an undocumented ` +
                `breaking removal):`,
        );
    ...
    console.log(
        `\nrelease:changelog — PASS: every public-surface row removed between ${prevTag} ` +
