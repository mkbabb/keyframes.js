...
import type { PlaybackHost } from "./playback";
...
// The heavy-surface bundling re-exports (AnimationGroup, getTimingFunction,
// resolveKeyframes, the option constants) MOVED to `./index.ts` (the engine
// barrel — the R.W1 bundling seam; gestalt §5), so `loadAnimationEngine()` still
// hands consumers the whole value.js-bearing engine in one `import("./engine")`.
