 ...
 * `./spring` import — never value.js) and a leaf finite difference, so the static
 * boundary `proof:boundary` gates is unbreached. spring.ts re-exports these public
 * symbols (`VelocityProbe` / `probeVelocity` / `reseatToSpring`) so the barrel —
 * and every consumer — resolves them through `./spring` exactly as before; this is
 * a pure colocation, ZERO behaviour change.
 */
