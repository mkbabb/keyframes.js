...
// into the colocated INTERNAL `waapi-densify.ts` as a cohesive gestalt seam
...
// separable from the eligibility/emission/delegation surface that stays here.
// Re-exported so `proof:waapi-adaptive-densify` (densifyInteriorTimes /
// segmentFlatnessError) + `proof:platform-adopt` S3 (WAAPI_MAX_SUBSEGMENT_STOPS
// / densifyInteriorTimes — the bounded interior sampling) find the anchored
// tokens at THIS host module, and so `toWAAPIKeyframes` below + the test/bench
// imports `from "./waapi"` resolve unchanged.
export {
