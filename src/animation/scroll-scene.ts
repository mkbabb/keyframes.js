...
// `./scroll-grammar`, the SO-1 round-trip half this file re-exports).
import type {
    AnimationRangeValue,
...
// ── re-export the SO-1 grammar ROUND-TRIP half (the value.js consume edge) ──
// The barrel + tests + proof:scroll-roundtrip import the full scroll surface
// from `./scroll-scene` exactly as before the split (zero import-set churn).
export {
    parseScrollTimeline,
    parseScrollRange,
    parseScrollCSS,
...
// Re-export the consumed value.js scroll-grammar TYPES (erased) so a consumer
// annotating a parsed scene reaches the typed surface through `./scroll-scene`.
export type {
    AnimationTimelineValue,
    AnimationRangeValue,
    CSSTimelineOptions,
