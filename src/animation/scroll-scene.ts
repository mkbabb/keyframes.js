 ...
 * it PARSES the `animation-timeline` / `animation-range` / `timeline-scope` /
 * `animation-trigger` the CSS WG standardized (consuming value.js 0.13.0's typed
 * `CSSTimelineOptions` extractor — `parseAnimationTimeline` / `extractTimelineOptions`),
 * DRIVES it on the compositor where the platform allows AND on its shipped
 * physics primitives where the platform falls short (Firefox-default, non-DOM
 * targets, scrub-smoothing, physics-snap), and serializes it BACK to valid CSS
 * (`serializeTimelineOptions`) that degrades into the native engine as browsers
 * catch up. The ONLY library that round-trips a scroll-driven stylesheet.
 *
 * ── The static/dynamic boundary (BINDING — proof:boundary) ─────────────────
 * This module is HEAVY: it carries a STATIC `@mkbabb/value.js` edge (the typed
 * scroll-grammar extractor + inverse serializer). It is reached ONLY via
 * `loadAnimationEngine()` — NEVER a LIGHT static barrel export. The value.js-free
 * LIGHT surface (SpringProgress/SmoothProgress/Timeline/…) stays value.js-free.
 *
 ...
 * `ScrollScene` driver resolves the live scroller/offset against the DOM, maps a
 * scroll position to the [0,1] range, smooths the scrub, settles the snap, and
 * fires the enter/leave thresholds. The impl does NOT ask value.js to resolve
 * offsets — that would re-pre-resolve in the wrong repo.
 *
 * ── The dispatch (conservative-correct, with a queryable reason) ───────────
 * The WAAPI-eligibility discipline (`isWAAPIEligible` — delegate where provably
 * correct, fall back to the always-correct JS driver, query the reason)
 * generalized to the scroll clock: a DOM + WAAPI-eligible animation with a native
 * `ScrollTimeline` present and NO scrub/snap/velocity request dispatches NATIVE
 * (`attachNativeScrollTimeline`, zero main-thread); everything else (Firefox-
 * default, non-DOM targets, scrub-smoothing, physics-snap, the WAAPI-ineligible
 * set) dispatches to the JS `ScrollScene`. The chosen backend + the reason are
 * QUERYABLE (`backend` / `ineligibleReason` — the `waapiIneligibleReason` honesty
 * idiom). This ADDS a parse+dispatch tier ABOVE the JS driver; it deletes no
 * driver (the ScrollTimeline-native-REPLACE ARCH kill is RE-AFFIRMED — the JS
 * `ScrollTimeline`/`ScrollScene` is the universal fallback).
 *
 * ── The pin (SO-3 `position:sticky` SYNTHESIS) ─────────────────────────────
 * `pin()` EMITS `position:sticky` + the range CSS (kf authors the platform's
 * pin); the browser does the pinning on the compositor. kf owns no pin MECHANISM.
 *
 * ── SO-4 transform-pinning — KILLED (RECORDED, never implemented) ──────────
 * Transform-tracked pinning (`transform: translateY(scrollY)` to hold an element
 * in place) is the KILLED primitive and is NOT implemented here. Scroll repaints
 * run on a different thread, so transform-tracked pins jitter/desync against the
 * compositor — the SAME structural lesson the glass-ui dock-VT scar records. The
 * only correct pin is `position:sticky` synthesis (SO-3). This module emits
 * `position:sticky`, NEVER a transform-tracking primitive; `pinCSS()` asserts the
 * property is `sticky`. The kill is re-affirmed, not re-litigated.
 */
