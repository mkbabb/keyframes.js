 ...
 *
 * LIGHT (value.js-free): it reads only the leaf `clamp` (from `./internal/leaves`)
 * and the `SpringProgressOptions` type + the default response from `./spring` — no
 * value.js edge, so the static boundary `proof:boundary` gates stays unbreached.
 * spring.ts re-exports the public `SpringDurationOptions` type and imports
 * `durationToSpringOptions` back for `SpringProgress.fromDuration`, so every
 * consumer resolves the surface through `./spring` exactly as before; this is a
 * pure colocation, ZERO behaviour change.
 */
import { clamp } from "./internal/leaves";
import { DEFAULT_SPRING_RESPONSE, type SpringProgressOptions } from "./spring";
