 ...
 * single `(x, y, vx, vy)` surface. Split out of `drag.ts` (the 1-D engine)
 ...
 * `./drag`, so it carries zero static `@mkbabb/value.js` edge.
 *
 * Re-exported THROUGH `drag.ts` so the barrel `export { drag, Draggable,
 * drag2D } from "./drag"` keeps resolving unchanged.
 */
