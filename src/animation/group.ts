        ...
        for (const groupObject of entries) {
            ...
            switch (layer.blendMode) {
                ...
                case "add":
                    ...
                    // a `ValueUnit[]` — scalar = length 1, multi-component = N).
                    // Numeric add is UN-CLAMPED (CSS `animation-composition: add`
                    // clamps at use, not composition) — `0.8 + 0.8 → 1.6`.
                    for (const key in values) {
                        if (whitelist && !whitelist.has(key)) continue;
                        const incoming = values[key];
                        if (incoming === undefined) continue;
                ...
                case "weighted": {
                    ...
                    // `w`, in place. K.W11 PHYS-C — the spring-driven blend
                    // weight: when the layer carries a `weightSpring` (set by
                    // `transitionLayer`/`crossfade`), `w` is the stepper's CURRENT
                    // value, not the constant `layer.weight` — ONE nullish read
                    // hoisted out of the element loop (a per-LAYER scalar), so the
                    // crossfade can overshoot 1.0 and settle. On settle the spring
                    // clears and the read falls back to the constant (byte-
                    // unchanged when no spring is present).
