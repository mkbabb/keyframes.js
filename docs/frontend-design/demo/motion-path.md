...
**On the crayon primaries (the verdict, made local).** `--rainbow-cyan` is a member of the demo's crayon family (`--rainbow-red`…`--rainbow-violet`, `design-idioms.css:78-90`) — and this scene is the *proof of proportion* for that whole family: it takes exactly ONE crayon, the cyan mid-stop, and commits it as a restrained, intentful signature ground. That is the verdict's thesis made tactile — the crayons are KEPT and *placed*, not killed and not sprayed. So the rule here is **single-crayon discipline**: cyan owns this drafting table end-to-end; we do not desaturate it toward a "tasteful" grey-blue, and we do not invite a second crayon onto the field. The page's restraint is *which* crayon and *where* — pure `hsl(180 80% 50%)` ink on a cyan-tinted ground — not the absence of saturation. (The one proportionate exception is the wink egg's crayon-spark, below.)
...
1. **The stage field is a flat tinted square — no draftsman's ground.**
   `MotionPathTarget.vue:291-303` (`.mp-stage`) is a single `color-mix(… 4% …)` cyan wash over `--background` with a card radius. The wash itself is *right* and stays — it deliberately *killed* the global crosshatch (the D17 fix) for a reason, and we keep that base tint as the ground tone. The gap is that it was never *built on*: a vector-authoring surface needs measurement structure (a graph field, a centerline crosshair, corner ticks) layered over the wash, or it reads as a placeholder rather than a drafting table. The demo already owns `.stage-field-x/-y` (`design-idioms.css:579-594`) and `--graph-pitch/--graph-major` (`design-idioms.css:268-272`) — they are **defined and consumed nowhere on this stage.** Layering them *over the existing wash* (not in place of it) retires the single biggest "AI-slop empty-box" tell on the page.
...
- **Blueprint ground.** *Keep* the flat `.mp-stage` wash (`:297-302`) as the base tone, and **layer** a cyan field on top: the `color-mix(… --stage-field-tint 4% …)` base stays exactly, then **add** a two-tier graph using the *existing* `--graph-pitch` (1rem fine) + `--graph-major` (5rem major) tokens, but **tinted from `--ball-tone`** instead of `--border`, at low alpha (~`4%` fine / `8%` major). This is the `.stage-field-*` idiom's intent, recolored to the scene tone — additive over the owned wash, coherent, not a new system.
...
- **Single-crayon discipline (not crayon-removal).** Note `--color-progress` resolves to `--accent-red` globally (`style.css:370`); this scene correctly shadows it with cyan via `--ball-tone`. Preserve that — the blueprint *ground* is cyan, the one crayon this scene owns. This is the verdict's "keep the crayons, proportion them" rule expressed as placement: the red is not *killed*, it is simply *not the ground crayon here* (it remains the demo-wide progress/destructive tone elsewhere). The discipline is "one crayon per drafting table," not "no crayons" — so the field stays cyan, full-saturation, no second primary sprayed across the graph.
- **The one proportionate crayon-spark (an easter egg).** When the wink fires (`mp-wink-spin`, `:414-428` — the lap-completion 😎), allow a *single-frame* warm crayon flash from the rainbow family (e.g. `--rainbow-yellow`/`--rainbow-orange`) in the glow bloom under the traveller — a tiny celebratory spark on the cyan field, gone in ~120ms. This is exactly the verdict's "crayons as a restrained, intentful signature": the family is KEPT and *earns its appearance* at a delight moment, never as theme. PRM-off.
...
- **Handle hover = spring-pop + halo.** *Keep* the existing fill-tint as the resting hover tier (`:343-346`), and **add** a `scale` spring-pop (`cubic-bezier(0.34,1.56,0.64,1)`, ~160ms) plus a soft `--ball-tone` focus halo ring on `:hover`/`:focus-visible` over it. The fill swap was never wrong — it just wasn't *grabbable*; the pop + halo is the additive tier that makes the node feel pickable. (Swap the `r`-transition target to `transform: scale()` so it composites; the fill transition stays.)
...
**Tokens consumed (all existing):** `--rainbow-cyan` / `--ball-tone`, `--graph-pitch` / `--graph-major` / `--stage-field-tint`, `--font-display` / `--font-mono`, `--type-title` / `--type-mono-caption`, `--radius-card`, the wink's `cubic-bezier(0.34,1.56,0.64,1)`, and (for the crayon-spark egg) one warm `--rainbow-*` stop. **New tokens proposed:** none required — optionally `--mp-bank-damp` (the tangent-lerp factor) if the banking is JS-smoothed rather than CSS-transitioned. The library primitives dogfooded in the load reveal (`DrawSVG`, `stagger`) are already shipped exports.

---

## §Design verdict reconciliation

This treatment was already the verdict's *exemplar* page — the proof that the crayon family can be a restrained, placed signature rather than a sprayed theme — so the revision is light-touch. What changed, and why:

- **Crayons KEPT + proportioned (verdict §1).** This scene's whole stance is *single-crayon discipline*: it takes one member of the demo crayon family (`--rainbow-cyan`, `design-idioms.css:90`) and commits it as the blueprint ground. Made that explicit in §Aesthetic direction ("On the crayon primaries") and re-cast the old "**Keep `--accent-red` out**" line in §COLOR — it now reads as *placement* ("one crayon per drafting table"), NOT crayon-removal. The red is not killed; it is simply not *this* table's ground crayon. No HSL/primary was proposed for elimination here, so there was no kill-clause to reverse — but I added one proportionate **crayon-spark easter egg** (a single-frame warm `--rainbow-*` flash in the wink's glow bloom) so the family visibly *earns its keep* at a delight moment.
- **Refine-not-abrogate (verdict §2).** Tempered every clause that *read* as a wholesale replacement to keep-the-element-and-add: (a) §Audit #1 + §COLOR "Blueprint ground" — "Replace the flat `.mp-stage` wash" → **keep the D17 wash as the base tone, layer the graph over it**; (b) §MICRO-INTERACTIONS handle hover — "Replace the `transition: r 120ms` fill fade" → **keep the fill-tint as the resting tier, add the spring-pop + halo on top**. All four pillars are AMPLIFIED, none swapped: GLASS (the `glass-resting` Card + inner vignette refraction), PAPER (the cyan two-tier graph ground over the EditorShell `--graph-*` substrate), TYPOGRAPHY (the Instrument-Serif titleblock cartouche + Fira Code stamped eyebrow, pushed bolder/more mathematical — no new face), MATHEMATICS (the live `tangentDeg` made VISIBLE as the banking glyph + the dogfooded `DrawSVG`/`stagger` self-build).
- **Signatures as proportionate easter eggs (verdict §3).** The page's signature — *author-the-curve, the-creature-obeys* (the traveller banking into the live tangent) — is already framed as a fold-in over the KEPT traveller-on-a-rail, not a replacement: the ball + path + net all stay, the tangent-bank and marching-ants are *added* as the mathematical overlay. The wink (`mp-wink-spin`, `:414-428`) stays as the lap-completion delight, now with its crayon-spark. Each is a delight, not a re-theme.
- **Deftness / proportion (verdict §4).** Every refinement stays scoped to the two scene files + one composable, consuming existing tokens (zero new required), additive over the owned structure. No glass-ui patch, no new face, no layout teardown.
