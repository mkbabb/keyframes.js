...
plus the THREE CORRECTIVE feedback rounds added after the user audited the running
`tranche-h-impl` demo: H.W9 (round-1 F1–F9), H.W10 (round-2 G1–G8), and H.W11 + H.W12 (round-3
I1–I12, split across two waves): THIRTEEN waves (W0–W12).
...
the live crashes poison every other measurement. H.W9 then H.W10 then H.W11 then H.W12 sit AFTER
the visual waves and BEFORE H.W8's golden baseline (H.W9 → H.W10 → H.W11 → H.W12 → H.W8 golden
capture) — the row order in the table below reflects that dependency placement. H.W9 is the FIRST
feedback round (the F1–F9 design-language fold); H.W10 is the SECOND (the G1–G8 scene-normalization
+ expressive-icon + stage-layout-primitive fold, observed live AFTER W5 landed — it COMPOSES with
H.W9); H.W11 + H.W12 are the THIRD round (the I1–I12 refinement layer, observed live on
`tranche-h-impl` while W10 is LANDING CONCURRENTLY — **H.W11's I5 REVERSES W10 G8's full-bleed**
toward a standard glass-card stage, and **H.W11's I2 EXTENDS the W1 FSM with a per-scene
control-VISIBILITY DFA**; H.W12 standardizes/decomposes/de-brittles + greatly refines sequence/path).)
...
| **H.W11 — The stage-card register + the control-surface DFA + the layout refinements** | the FOUR stage scenes converge to ONE standard non-cartoon glass `<Card>` (I5 REVERSES W10 G8 full-bleed) + the per-scene control-VISIBILITY DFA + transition perf budget (I2 EXTENDS the W1 FSM) + the uniform label SUBGRID (I1 refines W9 F1) + the bezier panel de-nest + grow (I6/I7 refine W9/W10) + rounding consumed-not-patched (I4) + the I1/I2/I4/I5/I6/I7 round-3 fold | H.W11 | CORRECTIVE (the THIRD user-feedback round, observed live while W10 LANDS CONCURRENTLY; SUPERSEDES W10 G8's full-bleed SURFACE — the `.stage-cell` PRIMITIVE half SURVIVES; EXTENDS the W1 FSM with a 3rd orthogonal axis; REFINES W9 F1/F2; net-deletion-leaning; AFTER H.W10, BEFORE H.W12 + H.W8's golden baseline) |
| **H.W12 — Component standardization · decomposition · encapsulation · brittleness/styling audit · sequence+path enrichment + easter eggs** | extract `useDragScrub` (I8) + the composable/store encapsulation audit (I9) + the colocation/decomposition verify (I10) + the deep-nesting/brittle-selector audit (I11) + the styling-idiom localization sweep (I12) + the sequence draggable rows + editable motion-path + copy artifact + tangent readout (I3) + one easter egg per scene (I3) | H.W12 | CORRECTIVE (the THIRD round, second of its two waves; RE-OPENS the W5-BOOKed `useDragScrub` + H-MI-4 + the F4 editable-path elevation now over threshold; EXTENDS W10 G3/G6 to sequence/path; net-deletion-leaning on the share/de-brittle side, additive on the enrichment side; AFTER H.W11, BEFORE H.W8's golden baseline) |
| **H.W8 — Gate-regime upgrade** | the appearance axis + interaction axis + manifest + chronic meta-gate | H.W8 | net-new (closes the blind spots; the LAST re-paper; the H.W9 calm register + the H.W10 colorful-icon / normalized-easing state + the H.W11 glass-card-stage / control-DFA + the H.W12 enriched-sequence/path / localized-idioms state are what its golden baseline locks) |
...
| **I1/I2/I4/I5/I6/I7** (the THIRD user-feedback round, live-observed on `tranche-h-impl` while W10 LANDS CONCURRENTLY) · `audit/feedback/i-_PLAN` (BINDING) · `audit/feedback/i-r5-sequence-path` · direct source reads + git history | **H.W11** | the stage-card register + the control-surface DFA + the layout refinements (the consolidated I1/I2/I4/I5/I6/I7 fold) | CORRECTIVE SHIP-in-H — **I5 REVERSES W10 G8's full-bleed SURFACE**: the FOUR stage scenes (easing/spring/sequence/path) converge from THREE states (full-bleed / cartoon-square / target) to ONE standard non-cartoon glass `<Card>` (the `.stage-cell` LAYOUT PRIMITIVE half of G8 SURVIVES; the user resolved W10 FORK A toward the contained card) + **I2 EXTENDS the W1 FSM** with a per-scene control-VISIBILITY DFA (a 3rd orthogonal axis — `controlSurfaces: Record<SceneId, ControlSurface[]>`; easing shows ONLY easing; the reka-fallback hacks die) + a transition perf budget (the NAMED `bench:scene-transition`, MEASURE-FIRST) + I1 the uniform label SUBGRID (refines W9 F1's per-row `auto` — the `w-20` literal dies) + I4 rounding-by-consumption (the `<Card>` rounds sequence/path for free; the primitive default is a glass-ui HANDOFF) + I6 drop the inner bezier card + I7 grow the bezier canvas + drop the "editing:" subtitle (refine W9 F2). SUPERSEDES W10 G8's full-bleed; EXTENDS W1; REFINES W9 F1/F2; 7 NEW gates; the I5-shadow NAMED-delta fork |
| **I8/I9/I10/I11/I12/I3** (the THIRD round, second of its two waves) · `audit/feedback/i-_PLAN` (BINDING) · `audit/feedback/i-r5-sequence-path` (BINDING for I3) · `a-styling-idioms §43-87` · `a-scene-spring-sequence §273-277` · `a-scene-path-discrete §135-137` | **H.W12** | component standardization · decomposition · encapsulation · brittleness/styling audit · sequence+path enrichment + easter eggs (the consolidated I8/I9/I10/I11/I12/I3 fold) | CORRECTIVE SHIP-in-H — the DRY/encapsulation/de-brittle sweep + the affordance enrichment that dogfoods it. I8 extract `useDragScrub` (RE-OPENS the W5 BOOK now at 3–5 consumers) + share the `.btn-playback` skin to sequence (extends W10 G3/G6) + I9 lift the motion-path gesture engine into `useMotionPathDemo` + W1-store pure-getter audit + I10 VERIFY the decomposition + colocate (the engine is FENCED; no demo file >500L) + I11 name `SAMPLE_STEP` + document the square-viewBox invariant + kill class-string DOM walks + I12 localize the 61 no-op `icon-*` classes in `design-idioms.css` + extend the OWNED-IDIOMS contract + I3 GREATLY REFINE sequence/path (draggable rows re-authoring `at:` + editable motion-path control points + copy-`offset-path` artifact + tangent readout + a swept master-playhead) + ONE easter egg per scene. RE-OPENS the W5-BOOKed `useDragScrub`/H-MI-4/F4-elevation; DOGFOODS public engine primitives (inv ζ); 9 NEW gates; the I3-rung FLOOR fork (default FULL) |
...
- **H.W11 (the stage-card register + the control-surface DFA + the layout refinements) — the
  THIRD feedback round, the I5/G8 reversal + the W1 keystone EXTENDED.** While W10 is LANDING
  CONCURRENTLY (the easing/spring/motion-path/sequence stage files `M` in the working tree), the
  user audited the running demo and named twelve refinements (I1–I12, split across H.W11 + H.W12).
  **The headline is the I5 reversal of W10 G8** (the load-bearing supersede): the four stage scenes
  are in THREE different states today — easing/spring FULL-BLEED-no-card (`EasingTarget.vue:2-10`,
  `SpringTarget.vue:2-8` — W10 G8 just applied), sequence/path bare `glass-resting cartoon-surface`
  divs that compute `border-radius: 0` (`SequenceTarget.vue:3`, `MotionPathTarget.vue:3` — W10 G8
  NEVER reached them), the target a standard glass card. **I5 converges all four to ONE standard,
  NON-cartoon glass `<Card>`** (rounded by construction), reversing ONLY G8's full-bleed SURFACE
  consequence — **the `.stage-cell` dock-band-reserve LAYOUT PRIMITIVE half of G8 SURVIVES** (it
  contains any stage subject between the docks; `dock-inset` stays deleted — no legacy beside the
  replacement); the user resolved W10 FORK A toward the contained card. The control PANELS keep
  cartoon+quiet (W2/W9 — orthogonal). Around it: I2 EXTENDS the W1 FSM with a per-scene
  control-VISIBILITY DFA — a `controlSurfaces: Record<SceneId, ControlSurface[]>` table as a THIRD
  orthogonal axis (W1 owns scene+playback; I2 adds control-surface), so the easing scene shows ONLY
  easing (not the hard-coded keyframes/timeline triggers at `AnimationControls.vue:20-22`), the
  navigation matrix is TOTAL (no undefined behavior), state suspends-and-fully-resumes on
  `SCENE_READY`, and a NAMED `bench:scene-transition` budgets the transition cost (MEASURE-FIRST, not
  asserted) — the reka-tab-fallback hacks (`EasingScene.vue:26-32` etc.) DIE; I1 promotes W9 F1's
  per-row `[auto_1fr]` label rules to a parent `subgrid` so the label column is UNIFORM across rows
  by construction (the `SequenceTarget.vue:25-27` `w-20` magic literal dies); I4 closes
  card-rounding by CONSUMPTION (the `<Card>` carries `rounded-card`, so sequence/path round for free
  via I5; the deeper "impossible to leave square" primitive default is a glass-ui HANDOFF); I6 drops
  the inner bezier card (`TimingFunctionPanel.vue:15` nested inside the controls host at
  `AnimationControlsControls.vue:119` — card-in-card); I7 grows the in-panel canvas above W9's 220px
  ceiling (`TimingFunctionPanel.vue:251-252`) + deletes the redundant "editing:" subtitle (`:33`).
  H.W11 is a CORRECTIVE wave that SUPERSEDES W10 G8's full-bleed SURFACE (the §supersede-map), EXTENDS
  the W1 keystone (a third orthogonal axis, NOT re-authored), and REFINES landed W9 F1/F2; it sits
  AFTER H.W10 and BEFORE H.W12 + H.W8's golden baseline (W9 → W10 → H.W11 → H.W12 → H.W8 golden
  capture), carries 7 gates that each BITE born-RED, and the one I5-shadow NAMED-delta fork
  (MEASURE-FIRST). Net-deletion-leaning; pure demo CSS/markup/Vue + ONE orthogonal table on the W1
  machine + ONE named bench — NO engine source (the engine is FENCED, inv ζ), NO glass-ui patch in
  kf (inv-16), NO dist build (a W10 impl workflow concurrently owns dist)
  (`audit/feedback/i-_PLAN` §0/§1/§2/§4/§5/§6/§7/§8, `audit/feedback/i-r5-sequence-path §0`;
  `waves/H.W11.md`).

- **H.W12 (component standardization · decomposition · encapsulation · brittleness/styling audit ·
  sequence+path enrichment + easter eggs) — the THIRD round's DRY/de-brittle sweep + the affordance
  enrichment that dogfoods it.** The second of the round-3 pair fills the structure H.W11 lands. The
  spine is the share-seam: I8 extracts `useDragScrub({ el, onScrub, project })` into
  `demo/@/composables/` (RE-OPENING the W5-BOOKed extraction — `H.W5.md:66` BOOKed at 2 consumers,
  now at 3–5: `SpringTarget.vue:88-93`, the sequence master-scrub, `MotionPathTarget.vue:124-147`) so
  the 3–4 hand-rolled rect→ratio + pointer-capture + window-listener copies COLLAPSE to one, and
  shares the `.btn-playback` skin to sequence's bespoke transport (extending W10 G3/G6 to the last two
  un-normalized scenes WITHOUT forcing them onto `PlaybackRibbon`, which lacks timeScale). Around it:
  I9 lifts the motion-path projection/scrub/`ManualTimeline` gesture engine from the anomalously-thin
  `useMotionPathDemo.ts` (49L) — the logic lives in `MotionPathTarget.vue:79-220` today, unlike
  sequence/spring whose composables own the engine — and audits the W1 stores for the single-writer /
  pure-getter discipline; I10 VERIFIES the D-tranche decomposition holds + colocates the I8/I9
  extractions (NO manufactured splits — the only >500-line files are the FENCED `src/animation/`
  engine; the max demo file is 418L); I11 names `SAMPLE_STEP=5` + documents the implicit-square-viewBox
  invariant (`MotionPathTarget.vue:118-147`) + replaces any class-string DOM walk with owned refs;
  I12 localizes the styling idioms — the 61 silent no-op `icon-(xs|sm|md|lg)` classes
  (`a-styling-idioms.md:43-87`, e.g. `TimingFunctionPanel.vue:30,41`) get ONE idiom area in
  `design-idioms.css` (a NAMED visual delta — the uniform 24px is the accident), the OWNED-IDIOMS
  contract extends to the full referenced set, magic numbers / brittle calc chains die. Then I3
  GREATLY REFINES sequence + path on the shared seam, inside the I5 cards: draggable storyboard rows
  re-authoring each child's `at:` live (RE-OPENING the W5-BOOKed H-MI-4, dogfooding `Sequence.add`/
  re-sort — `useSequenceDemo.ts:117-120`, inv ζ), an editable motion-path via draggable SVG control
  points re-emitting the single-source `PATH_D` (the F4 elevation — `motionPathGeometry.ts:1-11`
  guarantees the guide `<path>` `d` AND the traveller's `offset-path` move in lockstep, no drift), a
  copy-`offset-path` artifact, a tangent-angle readout, a swept master-playhead, a frontend-design
  usability pass over EVERY scene, and ONE easter egg per scene (EE-SEQ-1 "the reel"; EE-MP-2 "the
  emoji winks"; one per other scene). H.W12 is a CORRECTIVE wave that RE-OPENS the W5-BOOKed
  `useDragScrub`/H-MI-4/F4-elevation (now over their MEASURE-FIRST thresholds) + EXTENDS W10 G3/G6; it
  sits AFTER H.W11 and BEFORE H.W8's golden baseline, carries 9 gates that each BITE born-RED (except
  `proof:demo-no-oversize`'s 500L regression-guard clause, born-GREEN), and the one I3-rung FLOOR fork
  (default FULL). Net-deletion-leaning on the share/de-brittle side, additive on the enrichment side;
  pure demo CSS/markup/Vue + ONE shared composable + ONE design-idiom area — DOGFOODS public
  `Sequence.add`/`ManualTimeline`/`setChildTime().render()` (inv ζ, NO hand-rolled rAF), NO engine
  source (FENCED), NO glass-ui patch in kf (inv-16), NO dist build (`audit/feedback/i-_PLAN`
  §1/§3/§4/§5/§6/§7/§8, `audit/feedback/i-r5-sequence-path` (BINDING for I3), `a-styling-idioms`,
  `a-scene-spring-sequence`, `a-scene-path-discrete`; `waves/H.W12.md`).
...
- **§Design decisions RESOLVED:** ONE new corrective wave, COMPOSE with (not merge into) H.W9 (distinct altitudes — H.W9 = the material register, H.W10 = component + stage normalization + the expressive icon reversal); G3+G6+G7+G5 collapse into ONE normalization decision (the spine — REUSE makes the buttons match + equal-size + flatten + lift sizing all by construction); the easing stage = a ball, the editor stays in the sidebar (G4 — de-dup + the inv ζ dogfood); G8 is a LAYOUT PRIMITIVE, not a per-scene patch (the dock-band reserve derives from existing tokens, ZERO magic numbers); G1 reverses the W5 monochrome decision while KEEPING the substrate (W5 ITSELF pre-authorized this). **H.W10 SUPERSEDES, it does not REWRITE** — the supersede-map is the honest ledger; the gate-blindspot lesson in ACTION AGAIN (the original W5 gates checked icon coverage/shape + per-mode interactivity, NEVER that the new scenes REUSE the standard controls component — W10 normalizes them onto it before the H.W8 golden baseline locks). TWO USER-DECISION forks (G8 full-bleed-vs-contained, default BOTH-altitudes; G1 the exact palette, default the `--rainbow-*` token map) — only the G8 fork needs the user's call if they reject full-bleed. **NB — IMMEDIATELY SUPERSEDED BY H.W11 I5:** the user, observing the W10 full-bleed easing stage LIVE, reversed G8's full-bleed SURFACE consequence toward a standard glass `<Card>` (W10 FORK A resolved toward the contained card). H.W11 I5 supersedes ONLY altitude (b) the full-bleed surface; altitude (a) the `.stage-cell` dock-band-reserve LAYOUT PRIMITIVE SURVIVES untouched. Treat W10's full-bleed as the immediately-superseded baseline.

---

## H.W11 — THE STAGE-CARD REGISTER + THE CONTROL-SURFACE DFA + THE LAYOUT REFINEMENTS (the consolidated round-3 user-feedback fold · I1/I2/I4/I5/I6/I7; I5 REVERSES W10 G8 full-bleed; I2 EXTENDS the W1 FSM)

- **Phase:** IMPL — spec authored in DEV, awaits auth · **Class:** CORRECTIVE SHIP-in-H (the THIRD user-feedback refinement round, observed live while W10 LANDS CONCURRENTLY in the working tree — the I-items SUPERSEDE specific W10 decisions whose impl is still settling, most importantly **I5 REVERSES W10 G8 full-bleed**; net-deletion-leaning — drop the inner bezier Card, the "editing:" subtitle, the per-row `auto` label widths + the `w-20` literal, the reka-tab-fallback hacks; pure demo CSS/markup/Vue + ONE orthogonal table on the W1 machine + ONE named transition bench — NO engine source (FENCED, inv ζ), NO glass-ui patch in kf — inv-16, NO dist build) · **Scope:** the FOUR stage SUBJECT roots → ONE standard non-cartoon glass `<Card>` (REVERT the full-bleed on `EasingTarget.vue:2-10`/`SpringTarget.vue:2-8`/`StartingStyleTarget.vue`; SWAP the bare `glass-resting cartoon-surface` divs on `SequenceTarget.vue:3`/`MotionPathTarget.vue:3` → `<Card>`; the card sits in the `[stage]` track, dock-contained by the surviving `.stage-cell` primitive — `AnimationControlsGroup.vue:58,364-366`) + the per-scene control-VISIBILITY DFA (a `controlSurfaces: Record<SceneId, ControlSurface[]>` table on the W1 `useSceneMachine`; `AnimationControls.vue:8-9,20-22` renders the tab triggers FROM the table not the hard-coded `controls|keyframes|timeline` list; the reka-fallback hacks DIE — `EasingScene.vue:26-32`, `SpringScene.vue:62-74`, `MotionPathScene.vue:25`, `CubeScene.vue` matrix-controls) + the uniform label SUBGRID (the per-row `grid-cols-[auto_1fr]` → a parent `subgrid` track — `EasingSidebar.vue:144-150`, the standard `AnimationControlsControls.vue` W9 F1 rows, `LayerConfigPanel.vue`; the `SequenceTarget.vue:25-27` `w-20` magic literal dies) + the bezier panel de-nest + grow (DROP the inner `<Card>` — `TimingFunctionPanel.vue:15` rendered inside the controls host at `AnimationControlsControls.vue:119`; GROW the in-panel canvas ceiling above the W9 220px — `TimingFunctionPanel.vue:251-252`; DELETE the "editing:" subtitle — `:33`) + card-rounding-by-consumption (I4 closes FOR FREE alongside I5 — the `<Card>` carries `rounded-card`; the deeper primitive default is a glass-ui HANDOFF) + the gate set · **DAG-deps:** AFTER **H.W10** (I5 SUPERSEDES G8's full-bleed SURFACE — the `.stage-cell` PRIMITIVE half SURVIVES; I2's DFA enumerates the W10-normalized scenes; I6/I7 compose with W10's bezier panel), **H.W9** (I1 REFINES F1's per-row split → a parent subgrid; I6/I7 REFINE F2's bezier-panel form; the calm cartoon+quiet PANEL register is UNTOUCHED — I5's glass-card is the orthogonal STAGE register), **H.W1** (I2 EXTENDS its `useSceneMachine` with a third orthogonal axis; every browser gate settle-gates on it resting). Sequenced **AFTER H.W10, BEFORE H.W12 + H.W8's GOLDEN `proof:visual-lock` baseline** (W9 → W10 → H.W11 → H.W12 → H.W8 golden capture). W11 lands the STRUCTURE; W12 fills it. The FULL spec is `waves/H.W11.md`.
- **§Provenance (the folded lanes):** `audit/feedback/i-_PLAN.md` (the COMPLETE, authoritative round-3 plan — BINDING; §0 the binding state / the three-way stage split / the W1 keystone / the engine fence, §1 the two-wave decision, §2 the W11 clauses I5/I4/I1/I2/I6/I7, §4 the gate list, §5 the supersede-map + the I5/G8 reversal, §6 the W1/W9/W10 reconciliation, §7 the forks, §8 the convergence), `audit/feedback/i-r5-sequence-path.md §0` (the live verification of the three-way stage split + §3.2 the `useDragScrub` seam), direct source reads + git history. **Honest provenance:** only `i-r5` of the five round-3 research notes had LANDED at plan-author time; `i-r1..i-r4` were MISSING (written concurrently) — this wave grounds I1/I2/I4/I5/I6/I7 on direct source reads + git history + the landed W9/W10 specs + the deep-audit lanes; when `i-r1..i-r4` land the IMPL lead reconciles them.
- **§The state, verified (file:line / live anchors):** THE HEADLINE — the four STAGE scenes are in THREE different states today: (a) easing/spring FULL-BLEED-no-card (`EasingTarget.vue:1-10` the `<!-- G8 — FULL-BLEED stage … card is DROPPED -->` comment + the `easing-target … h-full w-full` div; `SpringTarget.vue:1-8` the same; `StartingStyleTarget.vue` — W10 G8 just applied); (b) sequence/path bare `glass-resting cartoon-surface` divs that compute `border-radius: 0` (`SequenceTarget.vue:3`, `MotionPathTarget.vue:3` — W10 G8 NEVER reached them; the `cartoon-surface` `@utility` carries NO radius — the radius lives on the `<Card>` root's `rounded-card`, `cards.css:33-48`); (c) the target a standard glass `<Card>` (glass, resting, `rounded-card` by construction). The control PANELS keep cartoon+quiet (W2/W9 — orthogonal, UNTOUCHED). `AnimationControls.vue:20-22` hard-codes THREE tab triggers (Controls/Keyframes/Timeline) for EVERY scene + the model `selectedControl` (`:8-9`); each scene poke-sets it ad-hoc with reka-tab-fallback HACKS (`EasingScene.vue:26-32` the "reka falls back to its first built-in tab (`controls`)" comment + a `nextTick` re-set `:38`; `SpringScene.vue:62-74`; `MotionPathScene.vue:25` `isControlsPanelOpen = false`) — the easing scene renders keyframes/timeline tabs that mean nothing. W9 F1's per-row `grid-template-columns: auto 1fr` (`EasingSidebar.vue:144-150`, the standard rows, `LayerConfigPanel.vue`) derives a PER-ROW `auto` label width (not uniform); the sequence labels are uniform-by-MAGIC-LITERAL (`SequenceTarget.vue:25-27` `w-20 text-right`). The bezier detail editor is a `<Card surface="cartoon" tier="quiet">` (`TimingFunctionPanel.vue:15`) INSIDE the controls host (`AnimationControlsControls.vue:119`) — card-in-card; the in-panel canvas is capped at `clamp(160px, 38cqi, 220px); max-block-size: 220px` (`:251-252`) + a redundant "editing: {{ editingCurveName }}" subtitle (`:33`). The engine is FENCED (the only >500-line files are `src/animation/`; inv ζ).
- **§Goal:** land the stage-card register the round-3 feedback asks for, observed live while W10 settles. The headline is the I5 reversal of W10 G8 (the load-bearing supersede): the four stage scenes converge from THREE states (full-bleed / cartoon-square / target) to ONE standard, NON-cartoon glass `<Card>` (rounded by construction); the control PANELS stay cartoon+quiet (W2/W9). Around it: bake card-rounding by CONSUMPTION not patch (I4 — closes for free alongside I5; the primitive default is a glass-ui HANDOFF); promote the per-row `[auto_1fr]` label rules to a parent `subgrid` so the label column is UNIFORM across rows (I1, the `w-20` literal dies); EXTEND the W1 FSM with a per-scene control-VISIBILITY DFA (I2 — the `controlSurfaces` table, the third orthogonal axis; easing shows ONLY easing; the navigation matrix TOTAL; state suspends-and-fully-resumes on `SCENE_READY`; a NAMED `bench:scene-transition` MEASURE-FIRST budget; the reka hacks die); drop the inner bezier Card (I6); GROW the in-panel canvas above the W9 220px ceiling + DELETE the "editing:" subtitle (I7, which composes with I6). Net-deletion-leaning, the four protagonists converge to ONE register, the W1 keystone EXTENDED not re-authored, the engine FENCED — the chronic-closure discipline in ACTION (the user caught the W10 full-bleed before the H.W8 golden baseline locked it).
- **§Scope:** **S1 (I5 — the glass-card register; REVERSES W10 G8 full-bleed)** — the four STAGE scenes get a STANDARD, NON-cartoon glass `<Card>`; REVERT the full-bleed on `EasingTarget.vue:2-10`/`SpringTarget.vue:2-8`/`StartingStyleTarget.vue`; SWAP `glass-resting cartoon-surface` → `<Card>` on `SequenceTarget.vue:3`/`MotionPathTarget.vue:3`; KEEP each scene's inner flex column EXACTLY — only the OUTER container changes; the card sits in the surviving `.stage-cell` primitive; NAMED-delta to consider (FORK I5-shadow, MEASURE-FIRST): `shadow={false}` for the protagonist plate. **S2 (I4 — rounding by consumption + glass-ui HANDOFF)** — the S1 `<Card>` swap rounds sequence/path with ZERO ad-hoc `rounded-*` literal; the deeper "impossible to leave square" primitive default (a `cartoon-surface` default radius) is a glass-ui HANDOFF (inv-16, born-RED-paired, NOT patched in kf). **S3 (I1 — the uniform label SUBGRID; REFINES W9 F1)** — promote the per-row `grid-cols-[auto_1fr]` to a parent `grid` whose rows share ONE label track via CSS `subgrid`; the `w-20` magic literal dies; the durable home is the glass-ui `LabeledField orientation` HANDOFF (demo-side parent-subgrid wrapper born-GREEN today, mirroring `AssetPropertiesPanel.vue:6`). **S4 (I2 — the control-surface DFA; EXTENDS W1)** — a static per-scene `controlSurfaces` table on `useSceneMachine` (easing → `[easing]`; cube → `[controls, keyframes, timeline, matrix-controls]`; sequence/path → `[]`/`[transport-only]`); `AnimationControls.vue` renders the triggers FROM the table; the reka hacks DIE; the transition routes through the W1 `SCENE_READY` suspend/resume contract; MEASURE-FIRST the NAMED `bench:scene-transition` budget (set from the W1 baseline during impl, NOT guessed); do NOT touch the W1 reducer beyond the orthogonal table + the surface-projection. **S5 (I6 — drop the inner bezier card; REFINES W9 F2/W10)** — drop the inner `<Card>` (`TimingFunctionPanel.vue:15`, + the steps variant `:87`); the bezier editor flows into the parent controls Card; no legacy beside the replacement. **S6 (I7 — grow the bezier + drop the subtitle; REFINES W9 F2)** — RAISE the in-panel canvas ceiling (`:251-252`, composes with S5's freed space; the square LAW preserved); DELETE the "editing:" subtitle (`:33`); MEASURE-FIRST: the grown canvas still fits without scroll (compose with W9 `proof:bezier-no-scroll`).
- **§Hard gate — the 7 gates, each BITES born-RED today (W10's full-bleed as the superseded baseline):** **NEW** `proof:stage-glass-card` (each of {easing, spring, sequence, path} resolves a non-cartoon glass `<Card>` — non-zero `border-radius` + `backdrop-filter !== 'none'` + NOT `cartoon-surface` + NOT full-bleed; reds on easing/spring full-bleed AND sequence/path `cartoon-surface` radius 0); **NEW** `proof:card-rounded-primitive` (kf half: ZERO bare-class `cartoon-surface` stage roots + non-zero radius; glass-ui HANDOFF half born-RED: a `cartoon-surface`-only element rounds from the primitive); **NEW** `proof:label-subgrid` (ALL leaf-row label cells share ONE computed width ±1px via `subgrid`; ≥3 differing-text rows non-vacuity; reds on per-row `auto` + the `w-20` literal); **NEW** `proof:scene-control-dfa` (each scene renders EXACTLY its DFA control-surface set — easing → only easing, no keyframes/timeline node; the (scene→scene) navigation matrix is TOTAL; reds on the hard-coded 3-tab list); **NEW** `proof:scene-transition-perf` (the NAMED `bench:scene-transition` budget met; state suspends-and-fully-resumes across a round-trip — EXTENDS the W1 identity-field-set with the control-surface projection); **NEW** `proof:bezier-single-card` (EXACTLY ONE `<Card>` between the bezier canvas and the controls-pane root; reds on the inner `:15` card nested in the host card); **NEW** `proof:bezier-panel-taller` (in-panel canvas `block-size` > the W9 220px ceiling AND no "editing:" text node AND still fits no-scroll; reds on the 220px cap + the `:33` subtitle). Gate-shape authored where H.W8 owns it; H.W11 wires the born-RED clauses into `proof:*`. **SUPERSEDED:** the W10 `proof:scene-card-rounded`'s full-bleed branch (the stage now resolves a card, not a full-bleed — the disjunct's subject no longer exists). **KEPT GREEN:** `proof:single-column-pack` (the one-column invariant holds), `proof:bezier-no-scroll` (the grown canvas must still fit). **BITE:** revert any fix → its gate reds; the full-bleed div BECOMES a `<Card>`, the reka hacks DIE (replaced by the explicit table), the inner card DIES (subtree deleted), the canvas GROWS — none satisfied by `display:none`/`!important`.
- **§Folds (the supersede-map — the honest ledger):** I5 supersedes W10 G8's full-bleed-no-bg SURFACE consequence (the `.stage-cell` LAYOUT PRIMITIVE half SURVIVES — only the surface reverses; `dock-inset` stays deleted); I4 rides I5 (no landed decision — the bare `cartoon-surface` divs miss `rounded-card`); I1 REFINES W9 F1 (per-row `auto` → parent subgrid); I2 EXTENDS W1 (a 3rd orthogonal axis, NOT re-authored) + SUPERSEDES the reka-tab-fallback hacks; I6 REFINES W9 F2/W10 (drop the inner card, re-home the header); I7 REFINES W9 F2 (grow the canvas within the fit invariant; delete the redundant subtitle). Full table + the I5/G8 reversal statement + the W1/W9/W10 reconciliation in `waves/H.W11.md §supersede-map`.
- **§Design decisions RESOLVED:** I5 REVERSES W10 G8's full-bleed SURFACE (the headline; the four scenes converge to ONE standard non-cartoon glass `<Card>`; the `.stage-cell` PRIMITIVE half SURVIVES; W10 FORK A resolved by the user toward the contained card); TWO registers cleanly separated (cartoon+quiet = the control chrome, W2/W9 untouched; glass-resting = the protagonist plate, I5); I2 EXTENDS the W1 keystone, never re-authors (a third orthogonal axis keyed by scene; the W1 `SCENE_READY` suspend/resume contract carries the control-surface state; the W1 plumbing REUSED); the I2 perf budget is MEASURE-FIRST not asserted (the NAMED `bench:scene-transition`); I4 closes for free in the demo + the deeper primitive is a glass-ui HANDOFF; I6+I7 compose (remove the inner card → more room → grow the canvas). **H.W11 SUPERSEDES, it does not REWRITE** — the supersede-map is the honest ledger; the chronic-closure discipline in ACTION (the user caught the W10 full-bleed, the W9 per-row-auto labels, the W9/W10 bezier double-card + tightness, the un-rounded cartoon cards before the H.W8 golden baseline locked them). The ONE I5-shadow NAMED-delta fork (MEASURE-FIRST) is the only fork; FORK W (two waves vs one) is a lead-tuning call (this round adopts TWO). No engine touched (inv ζ / FENCED); all DEMO-side; the glass-ui rounded-primitive + LabeledField-subgrid are inv-16 HANDOFFs.

---

## H.W12 — COMPONENT STANDARDIZATION · DECOMPOSITION · ENCAPSULATION · BRITTLENESS/STYLING AUDIT · SEQUENCE+PATH ENRICHMENT + EASTER EGGS (the consolidated round-3 user-feedback fold · I8/I9/I10/I11/I12/I3)

- **Phase:** IMPL — spec authored in DEV, awaits auth · **Class:** CORRECTIVE SHIP-in-H (the THIRD user-feedback refinement round, second of its two waves; net-deletion-leaning on the share/de-brittle side — collapse 3–4 hand-rolled drag-scrub copies into ONE `useDragScrub`, lift the motion-path gesture engine out of the Target, EXTEND the OWNED-IDIOMS contract membership to the full referenced-idiom set (NB — the four `icon-(xs|sm|md|lg)` are ALREADY owned by W4 `084feb9` in `design-idioms.css:209-232`, gated GREEN by `proof:icon-idiom`; the "61 no-op" finding is W4-CLOSED — see `i-_HARDEN.md` FORK-I12), kill magic numbers / brittle calc chains; additive on the I3 enrichment side — draggable sequence rows + editable motion-path + a copy artifact + a tangent readout + one easter egg per scene; pure demo CSS/markup/Vue + ONE shared composable + ONE design-idiom area — DOGFOODS public `Sequence.add`/`ManualTimeline`/`setChildTime().render()` (inv ζ, NO hand-rolled rAF), NO engine source (FENCED, inv ζ), NO glass-ui patch in kf — inv-16, NO dist build) · **Scope:** `useDragScrub({ el, onScrub, project })` extracted into `demo/@/composables/` (consumed by `SpringTarget.vue:88-93`, the sequence master-scrub `progressFromEvent`, `MotionPathTarget.vue:124-147`) + share the `.btn-playback` SKIN for sequence Play/Reverse (`PlaybackRibbon.vue:26,73-76`) + the motion-path projection/scrub/`ManualTimeline` gesture engine LIFTED from `MotionPathTarget.vue:79-220` into `useMotionPathDemo.ts` (49L) + the W1 store single-writer / pure-getter audit (`scenePlayback.ts`, `controlOptionsStore.ts`, `animationOptionsStore.ts`) + the demo-decomposition VERIFY + colocate (no demo file >500L; the engine FENCED) + the brittleness audit (`MotionPathTarget.vue:118-147` the `SAMPLE_STEP=5` magic + the implicit-square-viewBox scale `:131-134`; class-string DOM walks; brittle `:deep()` chains) + the styling-idiom localization (the `icon-(xs|sm|md|lg)` idiom is ALREADY owned — `design-idioms.css:209-232`, W4 `084feb9`; W12 EXTENDS the OWNED-IDIOMS contract MEMBERSHIP to the full referenced-idiom set + magic-number/brittle-calc cleanup) + SEQUENCE+PATH GREATLY REFINED (draggable storyboard rows re-authoring each child's `at:` — `SequenceTarget.vue:19-38`, `useSequenceDemo.ts:117-120`; editable motion-path control points re-emitting `PATH_D` — `MotionPathTarget.vue:18-24,42`, `motionPathGeometry.ts:1-11`; the copy-`offset-path` artifact; the tangent-angle readout; a swept master-playhead) + ONE easter egg per scene (EE-SEQ-1 "the reel"; EE-MP-2 "the emoji winks"; one per other scene) + the gate set · **DAG-deps:** AFTER **H.W11** (the I5 glass-card is the CONTAINER the W12 enrichments live INSIDE; the I2 DFA enumerates sequence/path as `{stage-only}` BEFORE W12 enriches their transport), **H.W5** (RE-OPENS the W5-BOOKed `useDragScrub` — `H.W5.md:66`, now at 3–5 consumers, over threshold — + the W5-BOOKed H-MI-4 sequence row-drag + the F4 editable-path elevation), **H.W10** (EXTENDS G3/G6 to sequence/path WITHOUT forcing them onto `PlaybackRibbon`), **H.W9/G.W10/D.W2** (the styling-idiom localization extends the OWNED-IDIOMS contract); every browser gate settle-gates on **H.W1**. Sequenced **AFTER H.W11, BEFORE H.W8's GOLDEN `proof:visual-lock` baseline** (W9 → W10 → H.W11 → H.W12 → H.W8 golden capture — the enriched sequence/path + the localized idioms are the FINAL state H.W8 locks). The FULL spec is `waves/H.W12.md`.
- **§Provenance (the folded lanes):** `audit/feedback/i-_PLAN.md` (the COMPLETE, authoritative round-3 plan — BINDING; §1 the W12 gestalt, §3 the W12 clauses I8/I9/I10/I11/I12/I3, §4 the gate list, §5 the supersede-map with the RE-OPENed W5 BOOKs, §6 the W5/W10 reconciliation, §7 the I3-rung fork, §8 the convergence), `audit/feedback/i-r5-sequence-path.md` (BINDING for I3 — §1.3/§2.3 the headline refinements, §3.2/R-SEQ-F the `useDragScrub` extraction, §W-MP-5 the thin-composable I9 lift, §4 the gate hooks + the easter eggs), `a-styling-idioms.md:43-87` (the 61-site `icon-*` no-op — I12), `a-scene-spring-sequence.md:273-277` (the W5-BOOKed H-MI-4 — I3), `a-scene-path-discrete.md:135-137` (the F4 elevation — I3). **Honest provenance:** `i-r5` is folded in full for I3; I8/I9/I10/I11/I12 ground on direct source reads + git history + the deep-audit lanes; when `i-r1..i-r4` land the IMPL lead reconciles them.
- **§The state, verified (file:line / live anchors):** sequence/path/spring each hand-roll the SAME rect→ratio + pointer-capture + window-listener drag-scrub (`SpringTarget.vue:88-93`, the sequence master-scrub, `MotionPathTarget.vue:124-147` — verified live: all three carry `setPointerCapture`/`getBoundingClientRect`); `useMotionPathDemo.ts` is anomalously thin (49L — the gesture engine lives in `MotionPathTarget.vue:79-220`, unlike sequence/spring whose composables own it); NO demo file exceeds 500L (max `useSpringDemo.ts` 418, `AnimationControlsGroup.vue` 417 — the only >500L files are the FENCED engine: engine.ts 1375, animations.ts 870, group.ts 772); the motion-path projection uses a bare `SAMPLE_STEP=5` linear search + an implicit-square-viewBox client→user scale (`:118-147`/`:131-134`); the OWNED-IDIOMS contract is under-enumerated — `icon-(xs|sm|md|lg)` are 61 SILENT NO-OP classes (`a-styling-idioms.md:43-87`, e.g. `TimingFunctionPanel.vue:30` `icon-sm`, `:41` `icon-md` — every icon paints Lucide's 24px default); sequence rows are READ-ONLY (`SequenceTarget.vue:19-38`; the `at:` re-sort home `useSequenceDemo.ts:117-120`); the motion-path AUTHOR PATH is FIXED (control points not editable; the single-source `PATH_D` `motionPathGeometry.ts:1-11`); no copy-`offset-path` artifact. The engine is FENCED (inv ζ); the I3 affordances DOGFOOD public `Sequence.add`/`ManualTimeline`/`setChildTime().render()`.
- **§Goal:** land the DRY/encapsulation/de-brittle sweep + the affordance enrichment that dogfoods it, inside the I5 glass cards W11 lands. The spine is the share-seam: extract `useDragScrub` (I8) FIRST so I3's new affordances are BORN on it (no churn-then-delete). Around it: lift the motion-path gesture engine into `useMotionPathDemo` + audit the W1 stores for pure getters (I9); VERIFY the decomposition holds + colocate (I10 — no manufactured splits; the engine is FENCED); de-brittle the motion-path projection + replace class-string DOM walks with owned refs (I11); localize the `icon-*` idiom area + extend the OWNED-IDIOMS contract + kill magic numbers (I12, ISOMORPHIC unless a HIGHLY befitting NAMED delta — the icon-size differentiation IS such a delta); GREATLY REFINE sequence/path on the shared seam (I3) — draggable rows re-authoring `at:` (dogfooding `Sequence.add`/re-sort), an editable motion-path re-emitting the single-source `PATH_D` (no drift), a copy artifact, a tangent readout, a swept master-playhead, a frontend-design pass over EVERY scene, and ONE easter egg per scene. Net-deletion-leaning on the share/de-brittle side, additive on the enrichment side; the engine FENCED + DOGFOODED (inv ζ); the demo verified-decomposed not over-split.
- **§Scope:** **S1 (I8 — extract `useDragScrub` + share the `.btn-playback` skin; extends W10 G3/G6)** — `useDragScrub({ el, onScrub, project })` owns pointer-capture + window listeners + clamp; each scene supplies its `project`; sequence keeps its timeScale/Reset domain verbs (the cube model — do NOT force it onto `PlaybackRibbon`). **S2 (I9 — encapsulation)** — lift the motion-path projection/scrub/`ManualTimeline` from the Target into `useMotionPathDemo`; audit the W1 stores for single-writer / pure getters. **S3 (I10 — colocate-verify)** — VERIFY the D-tranche decomposition holds; colocate the S1/S2 extractions; NO manufactured splits (the engine is FENCED). **S4 (I11 — de-brittle)** — name `SAMPLE_STEP` + document the square-viewBox invariant; replace class-string DOM walks with owned `useTemplateRef`s. **S5 (I12 — style-idioms)** — define ONE `icon-(xs|sm|md|lg)` idiom area in `design-idioms.css` (a NAMED visual delta); extend the OWNED-IDIOMS contract to the full referenced set; kill magic numbers / brittle calc chains. **S6 (I3 — sequence/path enrichment + easter eggs; folds i-r5)** — draggable storyboard rows re-authoring `at:` (FLOOR: a stagger-amount slider) + a swept master-playhead + the `.btn-playback` skin; an editable motion-path via draggable SVG control points re-emitting `PATH_D` (FLOOR: 2–3 preset paths) + a copy-`offset-path` artifact + a tangent readout; a frontend-design usability pass over EVERY scene; ONE easter egg per scene (EE-SEQ-1 "the reel"; EE-MP-2 "the emoji winks"; one per other scene).
- **§Hard gate — the 9 gates, each BITES born-RED today (except `proof:demo-no-oversize`'s 500L regression-guard clause, born-GREEN):** **NEW** `proof:dragscrub-single` (≤1 hand-rolled rect-ratio+pointer-capture+window-listener drag block across spring/sequence/path; reds at 3–4 copies); **NEW** `proof:composable-encapsulation` (`useMotionPathDemo` owns the gesture engine — the Target holds no projection math; pure store getters; reds on the gesture engine in the Target); **NEW** `proof:demo-no-oversize` (every demo file ≤500L + coherent colocation; born-GREEN on 500L today, bites a future over-split); **NEW** `proof:no-brittle-selector` (ZERO class-string DOM walks in scene targets; named motion-path constant + documented viewBox invariant; reds on `SAMPLE_STEP` magic + scale coupling); **NEW** `proof:styling-idioms` (every `icon-(xs|sm|md|lg)` ref resolves to a `design-idioms.css` definition; the OWNED-IDIOMS contract covers the full referenced set; reds on the 61 no-op `icon-*`); **NEW** `proof:sequence-rows-draggable` (drag a row handle → `at:`/`delays[i]` changes + `Sequence` re-sorts; reds on read-only rows); **NEW** `proof:motion-path-editable` (drag a control handle → guide `d` AND traveller `offset-path` both change to the SAME `d`; reds on fixed `PATH_D`); **NEW** `proof:motion-path-copy` (a copy affordance emits `offset-path: path(…)`; reds on none); **NEW** `proof:easter-egg` (EACH scene has its egg — hidden trigger → observable effect; reds on no eggs). Gate-shape authored where H.W8 owns it; H.W12 wires the born-RED clauses into `proof:*`. **KEPT GREEN:** `proof:scene-parity` (the I3 affordances ADD interactivity), `proof:icon-idiom` (W4 — `proof:styling-idioms` EXTENDS it to the full set), `proof:scene-uses-standard-ribbon` (W10 — I8 extends G3/G6 WITHOUT forcing sequence/path onto `PlaybackRibbon`). **BITE:** revert any fix → its gate reds; the drag copies COLLAPSE (one extraction), the gesture engine LIFTS (subtree moved), the icon idiom RESOLVES (defined, not suppressed), the rows DRAG (real affordance) — none satisfied by `display:none`/`!important`.
- **§Folds (the supersede-map — the honest ledger):** I8 EXTENDS W10 G3/G6 + RE-OPENS the W5-BOOKed `useDragScrub` (`H.W5.md:66`, now at 3–5 consumers over the 2-consumer MEASURE-FIRST threshold); I9/I10/I11/I12 are consistency tails on W1/W5/W10 (no landed decision — ISOMORPHIC unless a NAMED delta); I3 RE-OPENS the W5-BOOKed H-MI-4 sequence row-drag (`a-scene-spring-sequence.md:273-277`) + the F4 editable-path elevation (`a-scene-path-discrete.md:135-137`) under the user's "greatly refined" mandate (dogfood the engine + the shared seam — no churn-then-delete). Full table + the RE-OPENed-BOOKs rationale in `waves/H.W12.md §supersede-map`.
- **§Design decisions RESOLVED:** I8's `useDragScrub` is the spine, I3's affordances born on it (no churn-then-delete; now over the W5 BOOK's 2-consumer threshold; share idioms, keep earned differences); I9 lifts the gesture engine BEFORE I3 enriches it; I10 is VERIFY not a split campaign (no demo file >500L; the engine FENCED; the 500L gate is a regression guard); I12's icon-size differentiation IS a NAMED delta (the uniform 24px is the accident); I3 dogfoods the engine + the shared seam (the FLOOR is the MEASURE-FIRST scope-guard, FORK I3-rung default FULL). **H.W12 RE-OPENS deferred BOOKs, it does not REWRITE** — the supersede-map records the W5 BOOKs now over threshold + the W10 G3/G6 extension; the chronic-closure discipline in ACTION (the BOOKs re-open under the user's "greatly refined" / "share more" / "better encapsulation" mandate). The ONE I3-rung FLOOR fork (default FULL) is the only fork; the easter-egg selection is a lead-tuning call. No engine touched (inv ζ / FENCED); all DEMO-side (I10); the only sibling HANDOFF is W11's glass-ui LabeledField-subgrid (W12 emits none of its own).
