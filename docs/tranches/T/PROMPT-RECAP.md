...
| S.Z3 (FINAL + version) | PENDING-IMPL | never ran (lane 27 F1) | **T.Z** (T-equivalent close) — the close template is DRAFTED at ⑪ close-prep: `FINAL.md` (verdict→cure map · OD register · backlog residue · version RECO 5.2.0 MINOR · deploy path); the terminal close actions fire at T.Z under the owner-anchored close |
