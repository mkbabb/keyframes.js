...
|---|---|---|---|---|---|
| **OD-1** | morph + motion-path: **PRUNE** outright vs **FUSE** into one `scenes/svg/` | compose is pruned (independent); the SVG factories are LIBRARY, untouched | which of PRUNE / FUSE | **T.E2** (FUSE) ∥ **T.E3** (PRUNE) — mutually exclusive, both spec'd; the svg-fusion sketch | 🟡 **PROVISIONAL-PRUNE 2026-07-05** ("Prune morph and motion path unless you can convince me otherwise" — verbatim; the one-shot FUSE case was presented same-day; PRUNE (T.E3) executes unless the owner's next word says FUSE) |
| **OD-2** | Cursor light: **REMOVE** outright vs **relocate** to the home hero on glass-ui `Aurora` | the compose-scene `--mouse-x` wash dies EITHER way (rides compose prune); no hand-rolled tracker survives | REMOVE vs Aurora-on-hero | **T.D13** + the Aurora-on-hero prototype | ✅ **RULED 2026-07-05: AURORA-ON-HERO** ("Aurora on hero" — verbatim; the P-HERO `?light=1` fork is the blessed direction; T.D13 oracle unlocked) |
