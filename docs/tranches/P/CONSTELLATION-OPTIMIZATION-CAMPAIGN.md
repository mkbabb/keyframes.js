...
- **value.js P → 1.1.0** (API: VJ-L1 `flatLeaf` provenance + VJ-L3 `parseCSSSubValue`) **then
  1.2.0** (perf: `color2Into` gamut zero-alloc tail, the typed channel view, the codegen-consume,
  the `: any` seam narrowing) — and **P.W0 = commit the uncommitted O docs + reconcile the stale
  "DEVELOPMENT — charter only" PROGRESS to CLOSED-as-built** (record-as-built honesty, the FIRST P action).
...
if value.js P slips — the P-inv-28 chronicity-3 early exit.
...
the `.bbnf` "not yet wired to runtime" limbo is resolved); keyframes inherits faster frame-compilation
for free. This re-wires the DEAD O.W6 SpanParser-jump-table consume edge into a LIVE codegen edge.
...
generated function — gated against the A.W3 falsification record + a parity+throughput bench
(target: close the documented 0.58x BBNF-vs-hand-rolled gap toward ≥0.85x, measured portable).

## 5 — The headline novel ideas, by library (grounded; full set in the digest)

**value.js (1.1.0 API · 1.2.0 perf):**
- `color2Into` out-param — collapse the gamut bisection from ~84 → <12 allocs/call (the deferred
  O.W3/O.W5 tail; the `matrix.transformMat3Into` scratch pattern proven one layer down). [aggressive]
- `flatLeaf`-first value model — make the provenance-carrying leaf the canonical parsed shape so
  `flattenObject` is a projection, not a tree-dissolve that drops `FunctionValue.name`. **Dissolves
  the entire kf S8 sidechannel class intrinsically** (VJ-L1 becomes the model, not an add-on). [radical]
- typed Float64 channel view for the interp/conversion loops (kill the string-keyed megamorphic
  `[key:string]:any` reads without removing the public dynamic shape). [aggressive]
...
- **SoA composite buffer for AnimationGroup** — a single contiguous `Float64Array` accumulator
  ...
  (the multi-animation hot path). [radical]
- the **`Playhead` value-object** — transpose the god-object at the TRUE seam (the driver protocol:
  Sequence/Group/ingest poke 4 raw fields today) so `KeyframesAnimation` becomes a thin compile-facade. [radical]
- the **Typed-OM (Houdini `StylePropertyMap`) write path** — batched DOM style mutation, eliminating
  the per-frame string-serialize + browser CSS re-parse (broadly Baseline). [aggressive]
