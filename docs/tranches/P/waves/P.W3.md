...
- **`StylePropertyMap` path (Typed-OM eligible).** When the renderer detects `target.attributeStyleMap instanceof StylePropertyMap` (the Houdini Typed-OM aggregate API, **LIMITED AVAILABILITY — Chrome/Edge 66+, Safari 16.4+, NO Firefox for the aggregate `attributeStyleMap`/`StylePropertyMap` API** — NOT Baseline; `AUDIT-DIGEST.md` K3 :513), build a typed `CSSStyleValue` (a `CSSTransformValue` for `transform`, a `CSSUnitValue` for numeric dimensions, a `CSSKeywordValue`/parsed `CSSStyleValue.parse(prop, str)` for the residual) and `set()` it — NO string serialize, NO re-parse. This is PROGRESSIVE ENHANCEMENT: the path is reached ONLY when the feature-detect confirms `StylePropertyMap`; the `setProperty` fallback (below) is the bit-for-bit default everywhere else (notably ALL of Firefox).
...
**S1** transposes the egress in `transformTargetsStyle` into the two-phase `StylePropertyMap` write with a feature-detect fallback. **S2** lands the O.W8 S8 out-buffer alloc-cure as the unconditional FLOOR (both paths). **S3** records the Typed-OM ADOPT/KILL verdict in a decision JSON beside `spring-vector-decision.json` (the K1-rec P-inv-28 terminal home). **S4** is the sibling **Playhead value-object** transposition (the K2 driver-protocol formalization) over O.W7's shrunk `engine.ts` — and it is **BC-PRESERVING by construction** (see the BC clause below: the `Playhead` value-object is INTERNAL; every published engine-class member keeps its signature via DELEGATING ACCESSORS, gated by `proof:published-surface`). The keystone is **S5** — the born-RED `proof:typed-om-eligible` gate over the REAL runtime DOM-write-cost ratio.

### S4 BC-preservation clause (the 5.1.x committed path — INTERNAL Playhead, DELEGATING accessors)

The Playhead value-object is the K2 driver-protocol transposition, but the version is **5.1.x (BC-additive MINOR)** — so the transposition is INTERNAL and the published engine surface is UNCHANGED:

- **The `Playhead` value-object is INTERNAL.** It is not a named static export; it is the engine's private clock/t/iteration/flags carrier (the K2 :461-464 transposition). A consumer's `import` graph never sees it.
- **The public seam KEEPS its published signatures via DELEGATING ACCESSORS.** The four external drivers (`sequence.ts:426,497`, `group-layer-springs.ts:164,201`, `ingest.ts:345`) poke `startTime`/`pausedTime` as field writes and call `advanceTo`/`interpFrames`/`seek`/read `effectiveT`. P.W3 S4 keeps EVERY one of those at its published signature: `startTime`/`pausedTime` become **delegating accessors** (get/set forwarding to `_playhead.startTime`/`_playhead.pausedTime` — a field-write seam preserved as a setter), and `advanceTo`/`interpFrames`/`seek`/`effectiveT` keep their exact published shapes, delegating into the Playhead internally. NO public engine-class member is removed or retyped.
- **`proof:published-surface` is wired into the S4 born-RED set.** The S4 transposition is gated so that the rolled-up `dist/keyframes.d.ts` carries the SAME public engine-class members (no removed/retyped `startTime`/`pausedTime`/`advanceTo`/`interpFrames`/`seek`/`effectiveT`). A future FULL field-removal (dropping the delegating accessors) would escalate to 6.0.0 — but the committed P path is 5.1.x via delegating accessors, and `proof:published-surface` is the enforcing oracle.
...
**The BC-preservation arm — `proof:published-surface` (S4 born-RED set).** Because the version is 5.1.x (BC-additive MINOR), the Playhead transposition is gated so the published engine surface is UNCHANGED: `proof:published-surface` asserts the rolled-up `dist/keyframes.d.ts` keeps EVERY public engine-class member at its published signature — `startTime`/`pausedTime` (now delegating accessors over `_playhead`, the field-write seam preserved as a setter) and `advanceTo`/`interpFrames`/`seek`/`effectiveT` (delegating into the Playhead, signatures intact). BITE: a Playhead transposition that REMOVES or RETYPES a public member (e.g. dropping the `startTime` setter, or retyping `advanceTo`) reds `proof:published-surface` — the gate that forbids a silent 6.0.0-shaped break on the 5.1.x path. (A future field-removal that drops the delegating accessors escalates to 6.0.0; the committed P path is 5.1.x via delegating accessors.)

---

## Dependencies

- **Typed-OM (`StylePropertyMap` / `CSSTransformValue` / `CSSUnitValue`) — a BROWSER feature at LIMITED AVAILABILITY, NOT Baseline, NOT a sibling publish.** Chrome/Edge 66+, Safari 16.4+, NO Firefox for the aggregate `attributeStyleMap`/`StylePropertyMap` API (`AUDIT-DIGEST.md` K3 :513). Because it is NOT Baseline, the transposition is strictly PROGRESSIVE ENHANCEMENT — the feature-detect fallback is the WHOLE point: this wave needs NO value.js / parse-that / glass-ui publish — it fires entirely on today's installed tree, with the `setProperty` path preserved bit-for-bit where Typed-OM is absent (ALL of Firefox, old browsers, SSR). **Pure NOW.**
...
This file is the Tranche P DEVELOPMENT spec for P.W3 — **DOCS ONLY**. It writes zero engine/demo/library source (inv-16: kf writes only keyframes.js; no cross-repo edit — the Typed-OM win is a kf-internal egress transposition over a browser feature at LIMITED AVAILABILITY (NOT Baseline) consumed as PROGRESSIVE ENHANCEMENT, and the `unflattenObjectToString` `out` arg is an ALREADY-shipped value.js surface, not a new ask). The IMPLEMENTATION (the `transformTargetsStyle` two-phase egress, the `_styleOut` out-buffer, the `proof:typed-om-eligible` gate, the Playhead value-object, the decision JSON) opens only on the owner's explicit authorization. When it opens it is:
...
- **BC-preserving (5.1.x committed path)** — the S4 Playhead is INTERNAL; the public engine surface keeps every member at its published signature via DELEGATING ACCESSORS (`startTime`/`pausedTime` as get/set over `_playhead`; `advanceTo`/`interpFrames`/`seek`/`effectiveT` delegating in), enforced by `proof:published-surface` in the S4 born-RED set. NO public engine-class member is removed or retyped — a future field-removal escalates to 6.0.0, but P commits the 5.1.x delegating-accessor path.
...
| S4 `proof:published-surface` (BC) | The Playhead transposition silently BREAKS the published engine surface on a 5.1.x MINOR — a removed `startTime` setter or a retyped `advanceTo` ships as a non-breaking bump when it is actually a 6.0.0-shaped break; the delegating-accessor seam is the contract the gate enforces |
