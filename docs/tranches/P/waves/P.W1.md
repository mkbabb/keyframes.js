...
| a bench scenario covering the SoA-compositor frontier + unclassified existing cases | `taxonomy.json` classifies 42 cases but the SoA arms are the J.W6 **interp-buffer** SoA (per-frame numeric leaf), NOT the **compositor** SoA (the AnimationGroup blend fold P.W2 builds); zero `color2Into`-consume arm; `bench/sync-step.bench.ts` (4 run-check cases) exists and runs but is NOT in the taxonomy | K1/K3 |
...
| bench-taxonomy-gate | `package.json:68` (`proof:bench-taxonomy`) | the existing coverage gate P.W1 EXTENDS with the new scenario classes (compositor-SoA, color2Into-consume) and classifies the existing sync-step run-check cases |

---

## Scope

Each S-clause is a concrete, falsifiable deliverable. **S1** authors the eslint-flat + dep-cruiser lint-tier (the O Band A carry) + wires it into `check`/`proof:hygiene` (`eslint.config.mjs`, narrowed to `import/no-cycle` + `no-restricted-imports`). **S2** EXTENDS the bench taxonomy with the optimization-frontier scenario classes (compositor-SoA, color2Into-consume) as `pendingBudgeted` arms + classifies the existing `bench/sync-step.bench.ts` 4 run-check cases — the coverage manifest the Band-B waves graduate. **S3** authors `proof:portable-perf` — the ONE shared `ratioGate` + `absoluteGate` (with mandatory `marginComment`) helper a new perf gate imports, ratifying the same-report ratio discipline. **S4** wires the apparatus into CI with the correct posture. Every move is the floor the aggressive Band-B optimizations stand on — NONE a strategy change, NONE a workaround.
...
**Cure.** Author `eslint.config.mjs` (flat config — the Node ≥22 + ESM idiom; `.mjs` is the consistent filename across all doc references) with the **minimum viable rule-set** (`import/no-cycle` + `no-restricted-imports` — KISS; `tsc --strict` already covers most correctness rules; the dep-cruiser boundary rules below handle architectural-boundary checking without eslint rule duplication) and `.dependency-cruiser.cjs` with the architectural-boundary rules: (a) `internal/` leaves may NOT import `../engine` or `@mkbabb/value.js` (the value.js-free-leaf law); (b) the LIGHT barrel exports may NOT statically reach `./engine` (a STATIC pre-flight of what `proof:boundary` bundle-verifies — the same invariant, caught one tier earlier); (c) no circular dependency in `src/animation/`. Add `"lint": "eslint . && depcruise src"` to `package.json` scripts and into `proof:hygiene` (`package.json:194`).
...
**Gate bite.** `proof:lint-clean` (NEW): `eslint.config.mjs` + `.dependency-cruiser.cjs` exist and `npm run lint` exits 0 on the clean tree. BITE: plant a circular import (`internal/leaves.ts` imports `../engine`) → `depcruise` reds; today the file does not exist so the gate cannot run → RED.

---

### S2 — EXTEND the bench taxonomy to the optimization-frontier scenario classes (the coverage manifest Band B graduates)

**Breach.** `taxonomy.json` is gold-standard but covers the O-era frontier (the J.W6 interp-buffer SoA, the spring-vector sugar, warmEngine). It carries ZERO case for the Band-B optimization frontier: (1) the **compositor-SoA** arm (the AnimationGroup `Float64Array` blend fold — P.W2; the existing SoA arms are interp-buffer NUMERIC-LEAF, NOT the group composite — `taxonomy.json` "K=8 … SoA Float64Array+lerpArray" is `interpFrames`, not `transformFramesGrouped`); (2) the **color2Into-consume** arm (the value.js-P gamut zero-alloc tail measured kf-side — the K1 densify-COMPILE cross-repo frontier); (3) **`bench/sync-step.bench.ts`** — a currently-unclassified REAL gap: 4 cases (`drive(SmoothProgress)`, `drive(SpringProgress)`, `play(Animation K=8 transform)`, `play(AnimationGroup 32 cells K=8)`) that run today (confirmed exit 0 in-session) but are NOT in `taxonomy.json`, so the coverage gate does not track them. An aggressive optimization with no bench scenario has no born-RED gate — the campaign's falsify-first mandate is unmet.

> **NOTE — P.W4 RETIRED:** the **codegen-parse** scenario class is DROPPED from S2. P.W4 is a RETIRED TOMBSTONE (owner directive, 2026-06-22). There is no graduating wave; the codegen-parse scenario class must not track a wave that does not exist.

> **NOTE — O.W8 cross-reference corrected:** `bench/numeric-soa.bench.ts` and `bench/color-interp.bench.ts` are referenced in the O.W8 spec (S1/S2) but **do NOT exist on today's tree** (confirmed `ls` → ENOENT). O.W8's bench-file authorship items are **proposed, not shipped**. P.W1 does NOT defer to O.W8 for these files; P.W1 adds the NEW scenario classes O.W8 did not scope.

**Cure.** EXTEND `bench/taxonomy.json` and `proof:bench-taxonomy` (`scripts/proof-bench-taxonomy.mjs`) with NEW scenario classes and entries, authored as `pendingBudgeted` or classified `run-check` entries as appropriate:

1. **compositor-SoA** (P.W2 graduates): a `bench/group-composite.bench.ts` suite with the baseline (the current `transformFramesGrouped` boxed-leaf blend over `_grouped`) + the SoA twin (the contiguous `Float64Array` fold indexed by `_groupedKeys`). Budgeted `floorFraction` ratio = `1.2` against the boxed baseline (the J.W6 ADOPT threshold) — the SAME `baselineCase`×`floorFraction` device-independent shape. Plus an alloc-count arm (heap-delta) on the group draw path. Born `pendingBudgeted` (the bench suite does not exist; confirmed `ls bench/group-composite.bench.ts` → ENOENT).
2. **color2Into-consume** (K1 densify-COMPILE, cross-repo): extend the `crossRepo[]` array with the value.js-P `color2Into` ask (the gamut bisection `~84 → <12` allocs/call — `AUDIT-DIGEST.md` V1-N1), measured kf-side via a `bench/color-interp.bench.ts` densify-COMPILE arm (proposed, not shipped — the file does not exist today; born `pendingBudgeted` awaiting value.js-P re-pin).
3. **sync-step run-check** (the REAL currently-unclassified gap): classify `bench/sync-step.bench.ts`'s 4 cases (`drive(SmoothProgress)`, `drive(SpringProgress)`, `play(Animation K=8 transform)`, `play(AnimationGroup 32 cells K=8)`) as `run-check` scenario-class entries in `taxonomy.json`. The suite EXISTS and runs today (confirmed exit 0, 4 cases; `drive SmoothProgress` ~17281 hz) but is invisible to the taxonomy coverage gate — a genuine unclassified gap.

**Constraint (MEASURE-FIRST, observable-truth).** Each new `pendingBudgeted` arm is born without a floor (the L.W7 MEASURE-FIRST discipline — no floor until a measurement run records the baseline; `taxonomy.json:$comment` "no unproven code"). The `run-check` entry for sync-step carries no floor (it verifies correctness, not performance). The taxonomy STRUCTURE (every scenario classified, every cross-repo ask present) is device-INDEPENDENT and HARD everywhere; the wall-clock magnitude is observe-only. No arm asserts an absolute hz floor — every floor is a `baselineCase`×`floorFraction` same-report ratio (the K3 portability spine).

**Gate bite.** `proof:bench-taxonomy` coverage clause: the manifest names `bench/group-composite.bench.ts` cases the bench REPORT cannot contain (the suite does not exist) → coverage reds (`proof-bench-taxonomy.mjs` "a manifest case with no report row"); `bench/sync-step.bench.ts`'s 4 cases are classified in the manifest (the coverage gate now tracks them). BITE: drop the compositor-SoA scenario class → the Band-B wave that graduates it has no born-RED gate; unclassify the sync-step cases → the real run-check gap is silently un-tracked; the cross-repo VJ ask silently un-tracked.

---

### S3 — `proof:portable-perf`: the ONE shared ratio-gate helper (the portability spine, crystallized)

**Breach.** The `baselineCase`×`floorFraction` ratio math is RE-IMPLEMENTED per budgeted arm inside `proof-bench-taxonomy.mjs` and `proof-spring-vector.mjs`. A NEW perf gate (P.W2's compositor-SoA, P.W3's Typed-OM) must re-derive: read the vitest `--outputJson`, match the baseline + candidate case by name, compute `candHz / baseHz`, compare to `floorFraction`, route the miss through `declarePosture`. Five steps, copy-pasted — the exact "per-bench process.exit() duplication" the X1 novel idea names (`AUDIT-DIGEST.md` X1 §recs "Eliminates the per-bench process.exit() duplication"). And the warmEngine `floorHz: 1000` absolute floor in `taxonomy.json` has no `marginComment` — the honest reason for an absolute floor is undocumented, and any future gate copying the pattern would silently adopt a device-dependent hard predicate.

**Cure.** Author `scripts/lib/portable-perf.mjs` — one helper exporting:

- **`ratioGate({ report, baselineCase, candidateCase, floorFraction, posture, reason })`** — the same-report ratio gate: (a) reads a vitest bench `--outputJson` report, (b) extracts the `{baseHz, candHz}` pair from the same report (device-independent BY CONSTRUCTION), (c) computes the ratio, (d) routes the miss through `declarePosture(posture, {reason})` from `ci-env.mjs` (the existing authority — NO new posture system), and (e) records the durable verdict in a `*-decision.json` beside `spring-vector-decision.json` (the P-inv-28 terminal-home shape).
- **`absoluteGate({ report, candidateCase, floorHz, posture, reason, marginComment })`** — for the warmEngine-class microtask floor and similar cases where no meaningful same-report baseline exists (e.g. `warmEngine: floorHz: 1000` in `taxonomy.json`). `marginComment` is MANDATORY — the caller must explain why an absolute floor is the honest choice here, preventing silent adoption of device-dependent hard predicates. Routes the miss through the same `declarePosture` path.
- **`jsonParseAnchor()`** (OPTIONAL, scoped) — the portable cross-engine normalizer (the candidate's MB/s ÷ an in-run `JSON.parse` MB/s ≥ a floor, the value.js `proof-perf-target.mjs` pattern). Include ONLY if kf has a CSS-parse throughput gate to protect (a value.js concern dispatched as VJ-L3/O.W6); otherwise it is copy-weight with no live consumer in kf.

Wire it as `proof:portable-perf` and import it from P.W2's new gate (calls `ratioGate`, never re-derives).

**Constraint (no-legacy, single-seam, gestalt).** After this cure there is ONE perf-gate path: `ratioGate()` for same-report ratio gates + `absoluteGate()` (mandatory `marginComment`) for the warmEngine-class microtask floors — both over `ci-env.mjs`'s `declarePosture`. `proof-bench-taxonomy.mjs` + `proof-spring-vector.mjs` are REFACTORED to import from `portable-perf.mjs` (the re-implemented ratio math deleted, not left beside — no-legacy; the `warmEngine` budgeted case migrates from its inline absolute-floor to `absoluteGate`). No new posture taxonomy is invented (the X4-radical "extract ci-env to a package" idea is NOT this wave — the helper imports the existing module; copy-commit/extraction is a constellation-shared concern recorded, not built). KISS: the helper is `ratioGate` + `absoluteGate` + the optional `jsonParseAnchor` + the decision-record, nothing more.

**Gate bite.** `proof:portable-perf` self-test clause: a fixture report with `candHz = 0.5 × baseHz` and `floorFraction = 1.2` → `ratioGate` REDs locally (HARD) and NOTES in CI (observe-only with a stated reason); an `absoluteGate` call without a `marginComment` → THROWS. Scripts/ lint clause: scan for any `if (hz < floor)` or `hz >= WIN_FRACTION` pattern outside of `portable-perf.mjs` → HARD fail (enforcing no re-implemented floor math). BITE: an aggressive idea ships a gate with a hardcoded absolute `floorHz` HARD predicate instead of a `ratioGate`/`absoluteGate` call → lint clause reds (the K3 "no absolute floorHz survives as a HARD predicate outside the helper").
...
**Gate:** `proof:lint-clean` (NEW — `scripts/proof-lint-clean.mjs` asserting the lint config exists + `npm run lint` exits 0) + `proof:portable-perf` (NEW — `scripts/proof-portable-perf.mjs` over `scripts/lib/portable-perf.mjs`) + the EXTENDED `proof:bench-taxonomy` (the new compositor-SoA + color2Into scenario classes + the sync-step run-check classification). Born-RED on today's tree, before any config/helper/scenario artifact exists.
...
|-----|-------------------------------------|------------------------------------------------|
| S1 lint-tier | `eslint.config.mjs` + `.dependency-cruiser.cjs` absent; a circular import (`internal/leaves.ts` → `../engine`) passes `tsc` silently | `ls eslint.config.* .dependency-cruiser*` → NO matches → `proof:lint-clean` cannot run → RED; PLANT a circular import → `tsc --noEmit` greens it (no source-graph lint floor) |
...
| S2 sync-step unclassified | 4 existing `bench/sync-step.bench.ts` cases invisible to the taxonomy coverage gate | the suite runs today (exit 0, 4 cases) but `taxonomy.json` has no entry → the coverage gate does not track real bench work → unclassified gap |
| S3 portable-perf (**KEYSTONE**) | a budgeted arm carrying an ABSOLUTE `floorHz` HARD predicate instead of `ratioGate`/`absoluteGate` (with mandatory `marginComment`) | `scripts/lib/portable-perf.mjs` ENOENT → no shared ratio-gate exists → a NEW perf gate would copy-paste the ratio math + could hardcode an absolute floor that flakes RED on the slow runner (the device-dependence the spine forbids) |
...
**The portability spine (the owner mandate — PORTABLE perf gate, ratio-normalized).** Every HARD predicate P.W1 ships is device-INDEPENDENT: the lint-tier (a circular import is a circular import on any runner), the taxonomy STRUCTURE (every scenario classified), and the same-report RATIO (`baseHz × frac`, numerator and denominator from the same pass). The absolute wall-clock magnitude survives ONLY as an `observe-only` note via `declarePosture("observe-only", {reason})` — NEVER as a HARD CI predicate except via `absoluteGate` with a mandatory `marginComment`. This is the floor every Band-B aggressive idea spends: the compositor-SoA ratio (P.W2) and the Typed-OM DOM-write cost (P.W3) author their born-RED gate by CALLING `ratioGate()`, never re-deriving; warmEngine-class microtask floors call `absoluteGate` with a stated `marginComment`.
...
**Green condition.** `eslint.config.mjs` + `.dependency-cruiser.cjs` authored + `npm run lint` exits 0 + the boundary rules bite a planted circular import (S1); the taxonomy EXTENDED with the compositor-SoA + color2Into scenario classes as `pendingBudgeted` arms + the sync-step 4 cases classified as `run-check` (S2); `scripts/lib/portable-perf.mjs` authored with `ratioGate` + `absoluteGate` (mandatory `marginComment`) + optional `jsonParseAnchor` + the decision-record, `proof-bench-taxonomy.mjs` + `proof-spring-vector.mjs` REFACTORED to import it, `warmEngine` case migrated to `absoluteGate` (S3); all three new gates wired into `proof:hygiene` with the correct posture, `proof:ci-coverage` green (S4). The apparatus floor is LIVE — Band B's aggressive optimizations have their measurement substrate, and no absolute `floorHz` survives as a HARD predicate outside the helper.

---

## Dependencies

- **O.W8 ratified (the measurement wave) — already developed.** O.W8 closed 7 un-measured gaps (S1–S9) and ratified the portability spine + the `baselineCase`×`floorFraction` ratio shape. P.W1 EXTENDS that taxonomy (the new scenario classes) and CRYSTALLIZES the spine into a shared helper. P.W1 does NOT re-author O.W8's bench files — `bench/numeric-soa.bench.ts` and `bench/color-interp.bench.ts` were proposed by O.W8 S1/S2 but are **NOT shipped** (confirmed ENOENT on today's tree); they are proposed, not authored. P.W1 ADDS the compositor + color2Into + sync-step classes O.W8 did not scope.
...
- **Independent of every sibling publish — pure NOW.** The lint-tier, the taxonomy STRUCTURE extension, and the portable-perf helper fire entirely on today's installed tree. The color2Into scenario class is AUTHORED born-RED (the manifest names it) but its BUDGETED graduation is GATED on value.js P (S2's `crossRepo`-tagged arm) — the manifest tracks the cross-repo frontier without waiting on it (the O.W8 crossRepo discipline). The sync-step run-check classification is NOW (the suite runs today).
- **Feeds P.W2 + P.W3 (the Band-B floor).** Every Band-B aggressive idea authors its born-RED gate by calling `ratioGate()` or `absoluteGate()` (S3) and graduates a scenario class P.W1 added to the taxonomy (S2). P.W1 lands FIRST in the band ordering (`P.md:175` "Band A (the apparatus) lands FIRST"); without it the perf claims have no portable born-RED gate.
...
This file is the Tranche P DEVELOPMENT spec for P.W1 — DOCS ONLY. It writes zero engine/demo/library source (inv-16: kf writes only keyframes.js; the color2Into arm is a DISPATCH-tracked cross-repo frontier, never a foreign-tree edit). The IMPLEMENTATION (the `eslint.config.mjs`/dep-cruiser config, the taxonomy extension, the `portable-perf.mjs` helper, the CI wiring) opens only on the owner's explicit authorization. When it opens it is gate-first (`proof:lint-clean` + `proof:portable-perf` authored born-RED BEFORE the configs/helper land), observable-truth (the lint floor bites a planted circular import; the taxonomy coverage bites a missing report row; the portable helper bites an absolute-floor HARD predicate outside the helper), no-legacy (the re-implemented ratio math DELETED from `proof-bench-taxonomy.mjs`/`proof-spring-vector.mjs`, not kept beside; the lint floor AGREES with `proof:boundary`, never duplicates it), KISS (`eslint.config.mjs` narrowed to `import/no-cycle` + `no-restricted-imports`; the helper is `ratioGate` + `absoluteGate` + optional `jsonParseAnchor` + the decision-record), gestalt (ONE perf-gate path — `ratioGate`/`absoluteGate` over the existing `ci-env.mjs` posture authority), and P-invariant-28 (the lint-tier carry M.W2→O.W1→P.W1 TERMINATES here as a BUILD-IN — not a 4th ride; every aggressive idea's verdict gets a durable `*-decision.json` terminal home).
...
|--------|------------------------|
| S1 lint-tier | A circular import or a LIGHT-module-imports-value.js boundary breach lands silently (passes `tsc --noEmit`) and is caught only LATE at the bundle stage — or not at all; the architectural boundary has no static source-graph floor (the O Band A carry rides a 4th tranche); the config file is named consistently `eslint.config.mjs` across all references |
| S2 taxonomy extension | An aggressive Band-B optimization (the compositor-SoA fold) ships with NO bench scenario → no born-RED gate → the campaign's falsify-first mandate is unmet (a perf claim with no portable measurement); `bench/sync-step.bench.ts`'s 4 real run-check cases stay invisible to the coverage gate (an unclassified gap); the color2Into cross-repo ask is silently un-tracked |
| S3 portable-perf (keystone) | A perf gate hardcodes an absolute `floorHz` outside the helper that passes on macOS and flakes RED on the slow Linux runner for a DEVICE reason (the device-dependence-greening lesson) — OR the ratio math is copy-pasted per gate (the X1 per-bench-process.exit duplication); `absoluteGate` without a `marginComment` passes silently |
...
- **The engine optimizations themselves** (the compositor SoA fold, the Typed-OM write path) — those are P.W2/P.W3. P.W1 builds ONLY the apparatus they spend (the lint floor + the bench scenario classes + the portable ratio helper).
- **The codegen-parse scenario class** — DROPPED. P.W4 is a RETIRED TOMBSTONE (owner directive, 2026-06-22); there is no graduating wave. The taxonomy must not track a scenario class for a wave that does not exist.
...
- **O.W8's proposed bench files** (`bench/numeric-soa.bench.ts`, `bench/color-interp.bench.ts`) — proposed by O.W8 S1/S2 but **not shipped** (confirmed ENOENT on today's tree). P.W1 ADDS the NEW scenario classes O.W8 did not scope (compositor-SoA, color2Into-graduation, sync-step run-check).
