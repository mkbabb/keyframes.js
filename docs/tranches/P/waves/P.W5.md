...
6. **`flick-boing`** (THE KEYSTONE — observable-truth): synthesize a hard pointer flick (dx≈20px/16ms × 8 frames) over the sphere, release WITHOUT a double-click, assert the boing arc engages (the mesh departs `SPHERE_HOME` / `boinging` true) within a settle window. A flick handler that is WIRED but never fires (threshold mis-set), or a stub that only repaints, REDs. BITE: remove the `endDrag` flick-threshold branch → red.
...
| `flick-boing` (**KEYSTONE**) | synthetic hard flick (dx≈20px/16ms × 8), release, NO dblclick | the boing fires ONLY on `dblclick` (`AmigaScene.vue:168`); a flick does nothing — the egg is a hidden trap, not emergent | a hard flick > 8 rad/s engages the boing arc autonomously (mesh departs `SPHERE_HOME`); the dblclick path still works |
