...
**Phase:** NOW (kf-internal) + GATED (the unshelf trigger). Two phases in one wave: **(NOW)** the responsive/mobile layout + the typed-directional View-Transition scene-switch ride entirely on today's installed tree (the live `scene-subject` VT at `App.vue:461`, the glass-ui-published `view-transition.css`, kf's LIGHT primitives — NO sibling publish). **(GATED)** the full N-Stage carousel UNSHELF is GATED on the glass-ui BC cut (the `n-stage-impl` branch carries stale `~4.0.0` glass-ui + `^0.9.0` parse-that + `^0.13.0` value.js pins it must rebase off — per DM-24 / O.W15). The named sibling trigger is the BC cut + the constellation re-pin; the NOW layer ships without it.
**Sequence:** `P.W1 apparatus (NOW) ─► C{P.W5 cube+amiga ‖ P.W6 square+spring ‖ P.W7 curve-editor ‖ P.W8 N-Stage+mobile (this wave)}` — the four Band-C waves are independent of each other. P.W8's NOW layer (mobile + typed VT) has zero sibling dependency; its GATED layer (carousel unshelf) sequences after the BC cut. (`P.md:137,163-164`; the GATED edge is `BC-cut → P.W8-unshelf`, the NOW edge is none.)
...
### S4 — The N-Stage carousel UNSHELF (GATED on the BC cut): the gate-first finishing rebase

**Breach.** The full theatrical 3D-ring Stage switcher (~3,500 LOC on `n-stage-impl`, geometry-correct + tested) is shelved on the BC cut (stale `~4.0.0` glass-ui + `^0.9.0` parse-that + `^0.13.0` value.js pins it must rebase off the constellation re-pin — `AUDIT-DIGEST.md:583`). It is a scoped finishing tranche, not a rebuild — but it CANNOT land before the BC cut + re-pin (the named GATED trigger, DM-24 / O.W15).

**Cure (SPEC the gated sequence — NOT executed this wave).** When the BC cut publishes + the constellation re-pins, the unshelf is gate-first, MEASURE-FIRST, in order (the STAGE-SPEC working method, `STAGE-SPEC.md:S0…S10`):

1. **Gate-first: author `proof:n-stage-boundary` BEFORE the rebase** — the bundled-DEMO import-graph walk rooted at `SceneStage.vue` asserting the HEAVY `engine.ts`/`value.js` chunk is ABSENT from the static stage-module graph (`loadAnimationEngine()` reachable only via dynamic `import()`) — the genuine oracle the N.W2 correction established over the source-grep proxy (`harden-findings.md`). Born-RED on a planted heavy import in a stage module.
2. **MEASURE-FIRST (blocker 4): the STAGE-SPEC S0 perf trace** — open the rebased Stage in Chrome via chrome-devtools-mcp, record the carousel-open + spin FPS as "the number every later stage is judged against" (`STAGE-SPEC.md:26`). This QUANTIFIES the "SLOW" the owner named — observable-truth, before any further code.
3. **Transpose mobile (blocker 1): the S1 scroll-snap layout becomes the phone view of the Stage** — the unshelf reuses S1's native scroll-snap carousel for phone-narrow (the 3D ring is desktop-only; phone gets the scroll-snap view), so the mobile gap stays dissolved.
4. **Verify the light-mode cure (blocker 2)**: a chrome-devtools-mcp screenshot at light + dark asserting the on-stage ink reads (the prototype's `oklch(0.96)` cure transposed into the impl — `harden-findings.md:[high]` + screenshot 11).
5. **Wire the stage-portal shared-element VT (blocker 3)**: set the SAME `view-transition-name: stage-portal` on the dock Select trigger AND the stage nameplate before `startViewTransition(open)` (the `same-document-transitions` pattern), so a single `::view-transition-group(stage-portal)` morphs the trigger into the nameplate — the "grows from the trigger" beat (`AUDIT-DIGEST.md:880-883`). Clear the transient name on `finished`.
6. **Top-layer hit-test (the recorded precondition)**: a chrome-devtools-mcp hit-test confirming any chrome that must stay operable over the top-layer Popover stage is itself top-layer (`harden-findings.md:[med]`).

**Constraint (GATED — never executed pre-cut, inv-16).** S4 is a SPEC of the gated sequence; this wave does NOT execute the rebase, the perf trace, or the VT wiring (the BC cut is unshipped, the re-pin not done). The unshelf gates (`proof:n-stage-boundary`, `proof:stage-perf-budget`, `proof:stage-previews-live`, the `stage-portal` VT observable) are NAMED here as born-RED-on-unshelf; they are authored when the trigger fires. The NOW layer (S1+S2+S3) is independent of S4 — the phone scroll-snap switcher + the directional VT ship without the 3D Stage.

**Gate bite (GATED).** On unshelf: `proof:n-stage-boundary` exits 1 on a planted heavy import in a stage module; the S0 trace quantifies the SLOW baseline; the `stage-portal` VT observable asserts a single `::view-transition-group(stage-portal)` morphs the trigger geometry into the nameplate. Pre-cut: these are SPECCED, not run (the GATED phase).
...
| `stage-portal` VT (GATED, named) | (on unshelf) the VT pseudo-tree on stage-open | (pre-cut) the trigger/nameplate carry DISTINCT names → no morph (`harden-findings.md:[low]`) | one `::view-transition-group(stage-portal)` morphs the trigger geometry into the nameplate |
...
**Green condition.** The native scroll-snap mobile carousel (S1), the typed-directional scene-switch VT over the live `scene-subject` name (S2), `proof:scene-switcher-mobile` two clauses GREEN incl. a live 390px non-clipping non-colliding scroll-snap layout + a directional scene-switch (S3), and the GATED N-Stage unshelf SPECCED gate-first (S4 — `proof:n-stage-boundary` + MEASURE-FIRST + the `stage-portal` morph, authored on the BC cut). The mobile gap (THE shelf-driver) is dissolved NOW; the theatrical Stage lands gate-first on the cut.
...
- **GATED (S4) — the BC cut + the constellation re-pin (DM-24 / O.W15).** The N-Stage UNSHELF gates on glass-ui's BC cut publishing + the re-pin (the `n-stage-impl` branch carries stale `~4.0.0` glass-ui + `^0.9.0` parse-that + `^0.13.0` value.js it must rebase off — `AUDIT-DIGEST.md:583`). This is a NAMED sibling trigger; the unshelf is SPECCED here, executed on the cut. The NOW layer does NOT wait on it.
...
| S4 MEASURE-FIRST / `stage-portal` (GATED) | The Stage unshelves WITHOUT quantifying the "SLOW" the owner named (the unmeasured 7-preview perf risk) OR without the "grows from the trigger" morph (the signature beat that justifies the feature, left as a bare fade-bloom) |

---

## Excluded from this wave

- **Executing the N-Stage unshelf** (the `n-stage-impl` rebase, the S0 perf trace, the `stage-portal` VT wiring) — GATED on the glass-ui BC cut + the constellation re-pin (DM-24 / O.W15); SPECCED here (S4), not run. The branch carries stale pins it must rebase off first.
