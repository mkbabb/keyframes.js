# P.W2 — MEASURE-FIRST the AnimationGroup compositor blend, then DEMOTE-TO-SPIKE the SoA fold (gated on the add/weighted arms)

**Band:** B — Engine-perf (the transpositions).
**Phase:** NOW — kf-internal, zero sibling dependency, executable on authorization. The wave's FIRST step is the MEASUREMENT (a bench over `transformFramesGrouped`); the SoA engine edit is a DEMOTE-TO-SPIKE, chartered ONLY if the bench clears the bar. The computed-unit composite-key arm is a kf-side OBSERVE-ONLY bench — NOT a value.js dispatch.
...
**Owning-DM-or-idea:** the **K1/V2 "SoA composite buffer for AnimationGroup"** (`AUDIT-DIGEST.md` K1 novelIdea "replace the per-layer `Record<string,ValueUnit[]>` blend with a single contiguous Float64Array composite") — RE-SCOPED to a MEASURE-FIRST spike per `docs/tranches/P/CONTRIVANCE-AUDIT.md` (item: the 3.86× headline was TRANSPLANTED from a different path; the default blend arm is already dispatch-free).
