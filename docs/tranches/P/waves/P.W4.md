...
- **Plane A — the inherited generated parser (GATED, free).** value.js generates its CSS-value parser from `css/l4/*.bbnf` via parse-that's emitter; kf inherits faster frame-compilation **for free** — a re-pin + a parity+throughput gate, NOT a kf-authored emitter (inv-16: the emitter lives in parse-that, value.js owns the generation, kf consumes). This re-wires the DEAD O.W6 SpanParser-jump-table consume edge into a LIVE codegen edge (CONSTITUTION §4).
...
| falsification record | CONSTITUTION §4 / `AUDIT-DIGEST.md` V1-N2 :23 | A.W3 falsified the SpanParser RUNTIME switch (~10-14% slower on V8) — codegen must emit STRAIGHT-LINE, never a runtime interpreter; gated to ≥0.85x hand-rolled |
...
**S1 (Plane A)** re-pins the published parse-that B + value.js P and consumes the generated CSS-value parser through the UNCHANGED `CSSKeyframesAnimation` facade (the consume is a re-pin — no kf source change beyond the dependency bump). **S2 (Plane A gate)** authors `proof:codegen-parity-throughput` born-RED — the consumer-vantage parity + throughput-ratio gate guarded against the A.W3 falsification. **S3 (Plane B)** authors the kf-internal per-keyframe-shape interp-plan codegen behind a long-running heuristic + a CSP feature-detect fallback. **S4 (Plane B gate)** authors `proof:interp-codegen` born-RED — the ≥20% long-running interp-throughput ratio with the one-shot/CSP guards. **S5** records both verdicts in a decision JSON beside `spring-vector-decision.json` (ADOPT or KILL-with-falsification-record).
...
| `throughput-ratio` (**KEYSTONE** — the falsification guard) | `bench/compile.bench.ts` over the hand-rolled parser is the baseline; no generated arm exists | the generated parser must be FASTER, not slower — a re-pin that REGRESSES proves value.js emitted a runtime interpreter (the A.W3 falsified shape) | `generated-compile-hz / hand-rolled-compile-hz ≥ 0.85` (closing the 0.58x gap toward ≥0.85x) over the editing-session re-parse corpus, measured PORTABLE (same-report ratio, P.W1 discipline) |
...
- **inv-16 — kf consumes, never authors the emitter.** The emitter lives in parse-that; value.js owns the generation; kf inherits faster frame-compilation for free (a re-pin + a parity+throughput gate). P.W4 writes NO emitter and NO grammar — it consumes the generated surface. (The kf-internal interp-plan codegen, Plane B, IS kf-authored — but it is a kf-internal codegen over kf's OWN `frame-compiler` interp plan, not a foreign-tree edit.)
...
- **falsification-guarded (the spine's non-negotiable)** — every codegen clause is gated against the A.W3 record: the generated parser must be STRAIGHT-LINE + ≥0.85x hand-rolled (Plane A); the interp codegen must be straight-line + ≥1.20x generic (Plane B). A SLOWER generated artifact PROVES the runtime tagged-union shape was re-introduced and REDs the consume. The falsification is never re-litigated — it is the gate's foundation.
