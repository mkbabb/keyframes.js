...
| **B — Engine-perf (the grounded core + the demoted spikes)** | P.W2 (SoA-compositor **DEMOTE-TO-SPIKE** + the computed-unit composite-key **MEASURE-FIRST**), P.W3 (the `_styleOut` out-buffer alloc-cure **GROUNDED CORE** + the Typed-OM write-path **DEMOTE-TO-SPIKE**; the `Playhead` value-object **DROPPED**), ~~P.W4~~ (RETIRED tombstone — codegen-consume, BBNF-lang separate session) | NOW W2,W3 | per the CONTRIVANCE-AUDIT: the GROUNDED payload is the `_styleOut` out-buffer alloc-cure + O.W7's seam split; the SoA composite fold + the `StylePropertyMap` batched write are MEASURE-FIRST spikes gated on a same-report ratio of their OWN target path; the `Playhead` value-object is dropped (no perf claim); codegen out of campaign scope |
