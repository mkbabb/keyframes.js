...
- **`FULL-LOOP-LEDGER.md`** — the validated-verdict evidence base (2026-06-22 harden ·
  validate · brainstorm · reformulate loop over all 67 constellation items). The MEASURED
  verdicts this board now reflects: **P.W6 spring-heatmap ADOPT** (closed-form **272×**
  settle-time / **507×** overshoot faster than 400 live `SpringProgress` — tint by the EXACT
  analytic overshoot); **P.W11 WeakMap ADOPT** (realm-clean, the S8 terminal — ZERO kf own-symbols
  post-swap); **O.W6 `fromMorphSVG` ADOPT** (mid-t distinct 11.27/11.27/22.54); **VJ-P1
  `color2Into` ADOPT** (the **84-alloc** display-p3 egress tail); **PT-B1 ADOPT** (the packrat
  cross-input BLOCKER); **PT-B2 RE-SCOPE / PT-B4 KILL** (the `*Span` tier has ZERO consumers).
...
executes the engine-seam split (O.W7 — **NOT VJ-L1-gated**, executable NOW per CONTRIVANCE-AUDIT +
FULL-LOOP-LEDGER `O.W7`), dispatches the sibling asks, consumes the
...
| O.W7 engine-seam (engine.ts 1397→~900, **NOT VJ-L1-gated** — executable NOW per CONTRIVANCE-AUDIT + FULL-LOOP-LEDGER `O.W7` KEEP: `FN_NAME` scoped to concern 1, never read in concern 3) | O Band D (**NOW**) | P Band B consumes the SHRUNK file (the `Playhead` value-object is **DROPPED** — no perf claim); P.W11 WeakMap keeps the seam in-realm but is NOT an "unblock" |
...
| **P.W6** | C | NOW | Square + Spring demo-fleet design pass | **DEVELOPED** | `proof:square-design-paint` + `proof:spring-design-paint` (NEW — born-RED: `role=slider` missing `aria-valuenow` at SquareScene.vue:37; SpringTrace.vue linear() stop parser assigns wrong x-coordinates [bug REPRODUCED + fix VERIFIED: anchor-before-fill, first=0/last=100 — FULL-LOOP-LEDGER `P.W6 S2`]; no phase-portrait or heatmap element in DOM) | NOW — no sibling dep. **S3 heatmap ADOPT** (FULL-LOOP-LEDGER `P.W6 S3`): closed-form **272×** settle-time / **507×** overshoot faster than 400 live `SpringProgress` (0.002-0.004 ms/mount) — tint by the EXACT analytic overshoot `exp(-ζπ/√(1-ζ²))`. **`springLinearStopsArray()` DROPPED** (no-speculative-API — no second consumer; the SpringTrace bug is cured in-place) |
...
| **P.W11** | F | NOW | realm-clean WeakMap — the TERMINAL S8 cure (**ADOPT**) | **DEVELOPED** | `proof:no-foreign-symbol-stamp` (NEW — born-RED on `utils.ts:45` Symbol-onto-foreign-`ValueUnit`; scoped to the SHAPE not the string) + `proof:workaround-deletion` S8 → GREEN on the swap (NOW, not VJ-L1-gated) | NOW (kf-internal `WeakMap<ValueUnit,string>` — the S8 TERMINAL; **ADOPT** per FULL-LOOP-LEDGER `P.W11-weakmap`: validated realm-clean [ZERO kf own-symbols post-swap], byte-equivalent over 20 arity-pad interp vars; the clone-restamp ceremony STAYS — honest inferiority, VJ-L1 strictly preferred for ceremony-retirement, demoted-to-spike. It ALSO keeps the O.W7 seam in-realm, but O.W7 is NOT VJ-L1-gated so this is **not an "unblock"**) |
...
P.W1 apparatus ─► B{P.W2 SoA+cache, P.W3 Typed-OM, [P.W4 RETIRED tombstone]} ─► C{P.W5 cube/amiga, P.W6 sq/spring, P.W7 easing, P.W8 N-Stage}
                         ...
                         │                                                                 ▼
   P.W11 (realm-clean WeakMap — TERMINAL S8 cure) ║ O.W7 engine-seam (NOW, NOT VJ-L1-gated) ──► P.WZ close
                                                                                         ▲
...
| **DM-5 Constellation workarounds — residual S1, S8, S9** (S2 is now Band G `P.W12`; S7 FOLD-LANDED) | K | **4 (K,L,M,O→P)** | **HANDOFF: S1 on BC SFC guard (P.W12); S8 IN-REALM at P.W11 (WeakMap, NOW); S9 on value.js P VJ-L3 (O.W16 inherited)** | **P.W12** (S1) · **P.W11** (S8) · **O.W16→P** (S9) | `proof:workaround-deletion` S1=PENDING (`:aria-orientation=undefined` at `AnimationControls.vue:72` + `SpringSidebar.vue:43` — BC asked; S1 delete GATES on the `role=group` conditional guard shipping — `SegmentedTabs.vue:406` STILL unconditional in 4.1.0, verified K5/F1 finding); **S8 EXITS THIS TRANCHE via P.W11's realm-clean `WeakMap<ValueUnit,string>` swap — NOW, no value.js wait** (FULL-LOOP-LEDGER `P.W11-weakmap` ADOPT; `proof:no-foreign-symbol-stamp` GREENs on the swap; VJ-L1 demoted-to-spike — ceremony-retirement only); S9=PENDING (direct parse-that import, VJ-L3 absent). **P-inv-28 belt fires at chronicity 4 (THIS TRANCHE)** — S8 terminal home: P.W11 WeakMap (in-realm); S9 terminal home: value.js P VJ-L3. |
...
| **DP-5 realm-clean WeakMap — the TERMINAL S8 cure** (S8 at chronicity 4 = P-inv-28 belt fires; the kf-internal WeakMap IS the terminal, ADOPT — not merely an early-exit) | P (2026-06-20) | 1 (P→P) | **FOLD → P.W11** (WeakMap replaces the foreign Symbol; clone-restamp ceremony STAYS — honest inferiority; VJ-L1 demoted-to-spike for ceremony-retirement; FULL-LOOP-LEDGER `P.W11-weakmap` ADOPT) | **P.W11** | `proof:no-foreign-symbol-stamp` born-RED on `utils.ts:45` (scoped to the SHAPE); GREEN on the WeakMap swap. **RETIRES the S8 arm** — `proof:workaround-deletion` S8 → GREEN on the swap, NOW (validated realm-clean: ZERO kf own-symbols post-swap, byte-equivalent over 20 arity-pad interp vars). |
...
| **DM-5 S8 FN_NAME** | **4** (K,L,M,O→P) | **BUILD-IN (in-realm TERMINAL)** — P.W11's realm-clean `WeakMap<ValueUnit,string>` IS the S8 terminal (NOW, no value.js dependency; FULL-LOOP-LEDGER `P.W11-weakmap` ADOPT). VJ-L1 is DEMOTED-TO-SPIKE — ceremony-retirement only, not the terminal | P.W11 (NOW — the terminal); O.W16 S3 inherits the WeakMap |
| **DM-5 S9 parse-that import** | **4** (K,L,M,O→P) | **HANDOFF** — terminal at value.js P VJ-L3; O.W16/P.W10 W96 boundary scan is the gate that bites even if VJ-L3 slips | O.W16 (VJ-L3 publish arm) |
