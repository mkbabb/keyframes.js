...
**Branch:** `tranche-o-dev` (P's development phase rides the O dev tip; O is RATIFIED —
...
| **P.W2** | B | NOW | SoA compositor + computed-unit cache (engine perf NOW) | **DEVELOPED** | `proof:soa-interp` (NEW — ratio born-RED: `SoA-engine-hz / per-channel-engine-hz < 1.0` at K=8 over FULL interp corpus today — numeric-soa.bench.ts + interp-buffer.bench.ts) + `proof:zero-alloc` heap-delta arm EXTENDED (transformTargetsStyle out-buffer alloc = 0; reconcileVars O(1) Map) | NOW — no sibling dep; value.js 1.0.2 `./math` subpath already published |
| **P.W3** | B | NOW | Typed-OM write path (DOM mutation batching) | **DEVELOPED** | `proof:typed-om-eligible` (NEW — born-RED: a pure-transform animation reports `waapiDelegated===false` and `typedOmUsed===false` today) + `proof:typed-om-bench` throughput ratio (Typed-OM path ≥ 1.15× string-serialize path in a 200-frame bench) | NOW — CSS Houdini `StylePropertyMap` Baseline 2024; no sibling dep |
...
| **P.W12** | G | GATED (glass-ui ≥4.1.0 + SegmentedTabs guard) | S2-delete NOW + S1-GATED-on-guard | **DEVELOPED** | `proof:workaround-deletion` S2 GREEN (born-RED: S2 PENDING today at `TransportDock.vue` 9 sites — `useDockClickIntegrity` IS in glass-ui 4.0.1 but kf not yet consuming) + S1 GATED on the conditional-guard SFC fix NOT yet in 4.1.0 | **S2:** GATED on BC cut re-pin (content-present probe — `useDockClickIntegrity` in installed dist); **S1:** GATED on BC authoring the `role=group` conditional-guard SFC wave (O.W11 dispatch, Verified NOT in 4.1.0 per `SegmentedTabs.vue:406`) |
...
|---|---|---|---|---|---|
| **DM-2 GlassControlPoint → `DemoControlPoint`** (ABSOLUTE terminal via O.W5 BUILD-IN; P carries the PROOF if O.W5 impl has not fired) | E | **9 (E,F,G,H,I,J,K,L,M,O→P — ABSOLUTE terminus: no 10th carry)** | **BUILD-IN (O.W5 home; P dogfoods it in Band C)** — if O.W5 NOT YET IMPLEMENTED when P opens, P.W7 is the FINAL BUILD-IN home; no 10th carry | **P.W7** (demo-fleet showcase: the curve-editor drag handle IS `DemoControlPoint`) | `proof:demo-control-point` (from O.W5 — born-RED on absent component; GREEN on `DemoControlPoint.vue` live-drag via LIGHT `drag2D`). P.W7 requires O.W5 as substrate. If O.W5 NOT IMPLEMENTED, P.W7 builds it first. |
| **DM-3 MorphSVG → `fromMorphSVG`** (ABSOLUTE terminal via O.W6 BUILD-IN; P carries the PROOF if O.W6 impl has not fired) | C | **9 (C,F,G,H,I,J,K,L,M,O→P — ABSOLUTE terminus: no 10th carry)** | **BUILD-IN (O.W6 home; P adds the topology-aware topology extension)** — if O.W6 NOT YET IMPLEMENTED when P opens, P.W5 is the FINAL BUILD-IN home; no 10th carry | **P.W5** (demo-fleet: the morph scene dogfoods `fromMorphSVG`) | `proof:morphsvg-consume` (from O.W6 — born-RED on absent export; GREEN when mid-t sample DISTINCT from both endpoints). P.W5 requires O.W6 as substrate or builds it inline. |
...
|---|---|---|---|
| **DM-2 GlassControlPoint** | **9** (E,F,G,H,I,J,K,L,M,O→P) | **BUILD-IN (ABSOLUTE terminal via O.W5; if O.W5 not impl, P.W7 is FINAL)** — no 10th carry | P.W7 requires O.W5 as substrate or executes inline |
| **DM-3 MorphSVG** | **9** (C,F,G,H,I,J,K,L,M,O→P) | **BUILD-IN (ABSOLUTE terminal via O.W6; if O.W6 not impl, P.W5 is FINAL)** — no 10th carry | P.W5 requires O.W6 as substrate or executes inline |
...
| **BUILD-IN (kf-owned, ABSOLUTE terminal — O or P must close)** | DM-2 (O.W5/P.W7 DemoControlPoint) · DM-3 (O.W6/P.W5 fromMorphSVG) |
...
6-tranche — the contingency KILL record carries. DM-2/DM-3 are at 9-tranche — FINAL BUILD-IN.
DM-5 S8/S9 and DM-7 are at 4-tranche P-inv-28 belt — named terminal homes are ENFORCED. The
codegen-consume (DP-2) is GATED on parse-that B + value.js P — a HANDOFF, not a BOOK.
