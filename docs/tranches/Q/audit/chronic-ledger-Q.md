...
| 13 | **DM-22 named-selector NaN-frame** | L.W1 | **4** (L,M,O,P→Q) | **BUILD-IN (in-realm TERMINAL)** — deferred-resolution + PLAY-time guard (NOT a parse-throw) | **Q.WD1** | `proof:named-selector-nan-frame` (4 clauses: S4 round-trip preserved; attach-resolution; `NAMED_SELECTOR_NO_TIMELINE` at PLAY; zero NaN at play) | `frame-compiler.ts:449` (the DEFERRED comment) |
...
| 15 | **DM-16 5.0.0 version cut** | L.W8 | **4** (L,M,O,P→Q) | **USER-DOMAIN** | **Q.WZ** | `proof:changelog-5.0.0` (authored gate-first at Q.WE1/Q.WA3); GREEN on the 5.0.0 cut + wired into `release.yml` | `package.json:4` = 4.4.0 (the phantom 5.0.0) |
