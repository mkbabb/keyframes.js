...
|---|---|---|---|
| **A — Apparatus** | Q.W0 (record-hygiene + shipped-truth reconcile + CHRONIC_LEDGER re-pin), Q.WA1 (the SLIM lint-tier — **dep-cruiser ONLY** per the measure-first audit verdict: eslint is KILLED-down as redundant with `tsc --strict` + prettier-organize-imports; the three named invariants [no-cycle, no-restricted-imports-as-boundary, the LIGHT-graph guard] all express in one `.dependency-cruiser.cjs`; the 3-tranche O-Band-A carry), Q.WA2 (**drag2D LIGHT certification** — the audit found drag2D is ALREADY a LIGHT barrel export [`index.ts:88`, gate-proven by `proof:drag-gesture` S4]; so this wave CERTIFIES + boundary-locks it as the documented DemoControlPoint substrate and RETIRES the dead `proof:control-point-live` "needs drag2D" premise — NOT an "add"; **this DISSOLVES the drag2D→DemoControlPoint gating edge in §3 — Q.WC1 is NOW-buildable**), Q.WA3 (CI-green + **master-merge reconcile** + `proof:ci-coverage` fix + the deploy round-trip oracle + the device-dependence CI-harden), Q.WA4 (`proof:wave-charter` + the DAG manifest + the constellation pin-ledger witness) | NOW | the floor every Q wave stands on — the lint tier finally lands, the drag2D export unblocks DM-2, CI goes genuinely green, the contrivance smell-test becomes a gate |
...
Q.WA3 master-merge-reconcile (NOW, all 3 repos to master — the FIRST motion)
   ...
   │
   ├─► Q.WA2 drag2D CERTIFY (already LIGHT-exported — NO gating edge) · Q.WC1 DemoControlPoint (NOW) ─► Q.WC2 easing-editor dogfood
   │
...
2. **The DemoControlPoint chain — friction DISSOLVED.** The audit found `drag2D` is ALREADY a LIGHT barrel
   export (`index.ts:88`, gate-proven by `proof:drag-gesture` S4), so the feared "demo reaches into HEAVY"
   breach cannot occur and **Q.WC1 DemoControlPoint is NOW-buildable** (no gating edge). Q.WA2 CERTIFIES +
   boundary-locks the existing export and RETIRES the stale `proof:control-point-live` "needs drag2D" gate
   (a dead premise — the thing it waited for already shipped). The 9th-carry DM-2 has no remaining blocker.
