...
- **the 17-sample viewBox overshoot scan is a numeric proxy** — `EasingCurveCanvas` samples the fn 17× to find y-extrema; for a cubic bezier the displayed PATH's y-extrema are exactly `min(0,1,y1,y2)`/`max(0,1,y1,y2)` (the convex-hull property), so 17 fn-evals per pointermove are replaceable by a closed-form clamp. This is KISS + a per-move perf win, folded into S2 (the editor consolidation already touches the viewBox). **y-domain inversion note:** `EasingHeroStage.vue:66` renders `demo.svgPath.value` with y inverted (`1 - easingFn`); the closed-form clamp must operate on `1-y1`/`1-y2` in the SVG y-domain: `Math.min(0, 1, 1-y1, 1-y2)` / `Math.max(0, 1, 1-y1, 1-y2)`.
...
3. **Fold the closed-form viewBox clamp**: replace the 17-sample scan with the analytic bound `Math.min(0, 1, 1-y1, 1-y2)` / `Math.max(0, 1, 1-y1, 1-y2)` (SVG y-domain; the cubic-bezier convex-hull is exact for the displayed PATH, and TIGHTER than 17 samples) — KISS, and it eliminates 17 fn-evals per pointermove during a drag.
...
| S1 closed-form viewBox | 17 fn-evals per pointermove persist as a numeric proxy for a bound the cubic-bezier convex-hull gives exactly — a per-move perf + clarity regression (KISS violation) |
