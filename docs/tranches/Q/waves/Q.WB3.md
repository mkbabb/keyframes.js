...
P.W2's SoA fold is genuinely complete and exemplary FOR WHAT IT SCOPED (`B1-kf-soa`): `group.ts:soaBlendLayer` folds the `add`/`weighted` GROUP compositor over a precomputed `Float64Array` plan, bit-identical (`maxErr=0`), zero-alloc, K-monotone (ADOPT at 2.92×/3.01× in `soa-composite-decision.json`). But the fold lives ONLY in the MULTI-animation compositor. The SINGLE-animation per-frame interp — `processFrame` (`engine.ts:745`) — is still megamorphic boxed-AoS:
...
2. **The partition discipline exists** — `group.ts:buildSoAPlans` (`group.ts:575`) already classifies a key NUMERIC (every component an `isNumericUnit` `ValueUnit<number>`) or BOXED (any non-numeric/mixed element → the boxed residual, never split). The SAME K3 partition the `processFrame` fold needs — a pure-numeric frame segment folds into a `Float64Array`; a color/computed/mixed leaf stays boxed.
...
| partition-discipline | `src/animation/group.ts:575-643` | `buildSoAPlans` — the K3 numeric/boxed partition (`isNumericUnit` per component, mixed → boxed); the SAME discipline `processFrame` reuses |
...
| compositor-decision | `scripts/soa-composite-decision.json` | records ONLY K=8 (add 2.916× / weighted 3.011×); the K-monotonicity witness is the `Q.W-SOA-VERDICT-LADDER` gap |
...
**Gate bite.** `proof:processframe-soa` `measured-first` clause: `processframe-soa-decision.json` exists, records the per-K ratio scoped to `processFrame`, and the `$comment` forbids citing the compositor/transplanted number. BITE: a decision-JSON that cites the compositor's 2.916× (a different path) instead of a `processFrame`-measured ratio → the `$comment`-scope assertion reds.
...
- **The fold over the numeric subset.** In `processFrame`: pack the numeric ivs' (start, stop) into the buffer ONCE at parse; per frame, `lerpArray(eased, startBuf, stopBuf, outBuf)` (the value.js contiguous-lerp), then a strided write-back of `outBuf` into the numeric leaves' `.value` slots. The boxed tail (color/computed/mixed) walks the existing per-element `lerpValue` UNCHANGED. The `transformFrames` apply + the composition honoring (`engine.ts:758-777`) are untouched (they read the now-folded `.value` slots, identical to the boxed result).
...
### S5 — the `SpringProgress.setTargets(Float64Array)` vector-sugar confirm (NOW companion)

**Breach.** `B5-kf-engine-arch`: the `SpringProgress.setTargets(Float64Array)` vector sugar is ADOPT-verdicted (`spring-vector-decision.json`, ratio 3.85×) but the run-board does NOT confirm the `setTargets(Float64Array)` overload SHIPPED. A dirty `spring-vector-decision.json` (the only dirty file in git status, re-timestamped 2026-06-22) is the open thread.

**Cure.** Confirm-or-ship the `setTargets(Float64Array)` overload inside this wave (the SAME `Float64Array`+`lerpArray` substrate the `processFrame` fold uses — the vector setTargets IS the LIGHT `NumericAnimation` analog of the HEAVY `processFrame` fold). A born-RED clause asserts the overload exists + folds through `lerpArray`; if already shipped, the clause greens immediately (a confirm); if not, S5 ships it.

**Gate bite.** `proof:processframe-soa` `spring-vector-shipped` clause: `SpringProgress.setTargets` accepts a `Float64Array` and folds through `lerpArray` (the 3.85× path). BITE: the overload absent → the assertion reds (the confirm-or-ship discipline).
...
| `spring-vector-shipped` (S5) | `SpringProgress.setTargets(Float64Array)` folds through `lerpArray` | the overload ship-status unconfirmed → RED until confirmed-or-shipped |
...
- **`buildSoAPlans` partition discipline — already shipped** (`group.ts:575`). The K3 numeric/boxed classify the `processFrame` fold reuses; NO new partition logic invented.
...
- **value.js `ColorChannelPlan` — GATED PUBLISH (Q.WG2 dispatch → Q.WG4 consume).** The ONLY sibling gate for the SoA arms, and ONLY for the COLOR arm (S4). The numeric arm (S1/S2/S3) is fully in-realm. inv-16 holds (kf authors only the consume; the plan is the sibling's). Terminal-or-KILL: the color tail ships boxed if value.js declines.
