...
1. **Make `--morph-d` REAL.** Drive the render via a per-frame `onTick`/`onFrame` write: when `target` is given, on each frame reassemble the interpolated polyline into a `d` string (`MorphSVG.sampleD` already does this — `morph-svg.ts:278-289`) and write `target.style.setProperty('--morph-d', dString)`. Write BOTH the `d:` CSS property (`<path style="d: var(--morph-d)">` — Chromium + recent Safari render `path()` in `d`) AND retain the `--morph-d` custom property + expose a JS reader (`MorphSVG.sampleD(t)`) for the Firefox-lags case — the documented-fallback render path is honest, not a single-browser bet.
2. **Avoid the per-frame alloc.** `MorphSVG.sampleD(t)` calls `interpFrames(ms, false)` (`:281`) which allocates a fresh result object per call (no hoisted buffer). For a per-frame `d:` render, hoist the buffer (the `interpFrames(ms, apply, buffer)` third arg) so the render path is zero-alloc on the steady frame — KISS, and it pre-empts a "morph is too heavy" perf deferral.
...
| no `--morph-d` write | `grep "morph-d\|setProperty\|onTick\|onRender" src/animation/morph-svg.ts` | ONLY the two docstring lines; ZERO write call site (the honesty gap) |
...
1. **Make `--morph-d` REAL (the per-frame write).** When `target` is given, wire a per-frame `onTick`/`onFrame` callback that reassembles the interpolated polyline into a `d` string and writes BOTH `target.style.setProperty('--morph-d', d)` AND `target.style.setProperty('d', \`path("${d}")\`)` (the `d:` CSS property — Chromium + recent Safari render `path()` in `d`; the `--morph-d` custom property is the cross-browser author channel + the JS-reader fallback for Firefox). The `sampleD` reassembly already exists (`MorphSVG.sampleD`, `:278-289`); the render path calls it (or its zero-alloc twin) per frame.
2. **Zero-alloc on the steady frame.** Hoist the `interpFrames` result buffer (the `interpFrames(ms, apply, buffer)` third arg) so the per-frame render is zero-alloc, not a fresh object per frame (`:281` allocates today). Pre-empts a "morph is too heavy at 64 samples" perf deferral.
...
**Constraint (boundary — HEAVY, single value.js edge unchanged; inv-16).** The render write is a kf-side per-frame DOM write on the target's style — it adds NO value.js edge (`morph-svg.ts` keeps its single `PathGeometry` import). The morph stays WAAPI-ineligible by construction (custom props are not the default DOM renderer's animatable set) so it always runs the rAF path where the `onTick` hook lands cleanly. This is a library source edit on `morph-svg.ts` (NOT a demo edit) — the honest render contract is a LIBRARY correctness fix.
...
**Cure.** Add an `orient?: boolean` option to `MorphSVGOptions`. When `orient: true`, `samplePolyline` additionally calls `sampleAtLength` at each step to read the tangent `angle`, and the keyframes carry an `--morph-angle` channel (the interpolated tangent) so a consumer can `rotate` a glyph along the morphed shape (the `rotate: auto` semantic). Gate the orient channel BEHIND the option (default `false`) so a position-only morph stays at ~130 keys — the orient adds ~65 angle keys (~195 total at `samples=64`) ONLY when requested, pre-empting a "morph is too heavy" perf deferral.
...
2. **Author `scripts/proof-morph-scene.mjs`** (playwright-headless, born-RED) over the BUILT `dist/gh-pages/`, mirroring the scene-runtime harness (`scripts/lib/demo-driver.mjs` `withPage`/`navToScene`). Wire it into the `proof:correctness` roster. Clauses:
...
3. **EXTEND `proof:morphsvg-consume` with a demo-scene clause** (per `B6-dag-ordering`): add a clause asserting `MorphSVGScene.vue` exists + consumes `fromMorphSVG` — so the library gate and the demo gate are linked (the primitive is never again library-built-but-undemoed).
