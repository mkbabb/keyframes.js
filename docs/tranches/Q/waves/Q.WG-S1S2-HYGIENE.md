...
| the dock twin | `demo/@/components/custom/.../TransportDock.vue:348-375` | `pointerHandled`/`onPlayPointerDown` (the S2 band-aid) |
...
- `ariaGuard` — the guard is present iff the installed `dist/tabs.js` (or the mounted `SegmentedTabs variant="pill"`) does NOT emit `aria-orientation` on `role=group`. The probe: grep the installed `dist/tabs.js` for the conditional guard shape (`isUnderline ? ... : undefined` on the `aria-orientation` bind) OR, stronger, defer to `proof:glassui-aria-ask`'s DOM-readback (S3). Content-present ⇒ true.
- `dockStrandKeepalive` — the collapse-crossfade keepalive is present iff the installed dist carries the keepalive marker GU-Q2 names in `KF-TO-GLASSUI-Q.md` (the EXACT API name/dist string is the dispatch's deliverable — the forward dependency the dispatch pre-empts). Content-present ⇒ true.
...
- S2 arm: add `apiPresent: glassCaps.dockStrandKeepalive`, AND retarget `sibling.version`/`sibling.name` off the wrong `useDockClickIntegrity`-4.1.0 sentinel to the BC cut version + the collapse-crossfade keepalive name (GU-Q2). Now S2 is PENDING until the keepalive ships, not RED on the unrelated 4.1.0 publish.
...
**Cure (the REAL observable — a mounted-DOM readback, NOT a grep).** Author `scripts/proof-glassui-aria-ask.mjs`: MOUNT the INSTALLED glass-ui `SegmentedTabs` with `variant="pill"` (jsdom or playwright-headless over the built demo), read the rendered `role=group` element's `aria-orientation` attribute, and assert it is `null`/absent. This is the appearance/interaction-axis gate the green-source-shape-gates-miss lesson demands — it reads what the BROWSER renders, not a source shape. CI posture: a device-bearing DOM-readback (it mounts a component), so it is wired observe-only-until-publish (PENDING when the guard is absent) to avoid a false-RED stall; it flips to HARD on the consume (Q.WG-GATED-CONSUMES). The `glassCaps.ariaGuard` probe (S1) can DELEGATE to this gate's verdict (the single source of content-truth).
...
**Portable / structural posture.** The `s1-s2-not-false-red` clause is a gate-logic correctness assertion (device-independent — it reads the installed dist content + the source-tree witness). The `aria-ask-content` clause is a device-bearing DOM-readback (it mounts a component); it is wired observe-only-until-publish (PENDING when the guard is absent) so it cannot flake-RED on the slow Linux CI runner before the fix ships — the device-dependence-greening discipline. On the consume (Q.WG-GATED-CONSUMES) it flips HARD.
...
- **KF-TO-GLASSUI-Q.md (Q.WG3) — the dispatch partner that NAMES the markers.** The `glassCaps.dockStrandKeepalive` content-probe needs the EXACT keepalive API name / dist marker GU-Q2 names in the dispatch (the forward dependency). The dispatch authoring (this Band) pre-empts it: GU-Q2's deliverable names the marker BEFORE this wave's S2 retarget writes the probe pattern. The `glassCaps.ariaGuard` probe delegates to GU-Q1's `proof:glassui-aria-ask` content-truth.
...
**Mid-tranche-friction pre-emption.** This wave could spawn ONE friction, pre-empted NOW: **the `dockStrandKeepalive` marker name is unknown until GU-Q2 authors the dispatch** (S2's content-probe needs the exact API string — a forward dependency on a dispatch glass-ui has not yet authored). **Pre-empted by the Band-G authoring order**: `KF-TO-GLASSUI-Q.md` (GU-Q2) is authored in THIS band and NAMES the keepalive marker (or a stable dist string) as its deliverable BEFORE this wave's S2 retarget writes the probe; if glass-ui ultimately declines GU-Q2, the S2 twin stays (recorded — a glass-ui-owned defect kf cannot self-cure) and S2 holds PENDING, never a false-RED.
