...
- `interpFrames` and `advanceTo` are NOT private playback internals — they are the engine's public sampling API consumed externally by `group.ts:358/929`, `group-layer-springs.ts:201/221`, `ingest.ts:268`, `morph-svg.ts:281`, `sequence.ts:300`. They stay in `engine.ts` with concern 1. The split is **concern 3 only**.
...
| `this`-bound contract | `engine.ts:288-294,382-402,453-462` | setters mutate `this.options` in place (never replace); `this.options === this.compiler.options` identity; `setDuration` re-reads `this.frames` to re-time |
| `_interpOut` zero-alloc | `engine.ts:215-219` | the ONE hoisted buffer for the standalone play loop's `interpFrames`; stays with the play concern (concern 3), re-derived from `this` |
...
- **`interpFrames`, `advanceTo`, `at` — stay in `engine.ts`.** These are external sampling APIs (`group.ts:358/929`, `ingest.ts:268`, `morph-svg.ts:281`, `sequence.ts:300`) and must NOT move into `engine-playback.ts`. The split scope is concern 3 (the standalone-play loop) ONLY.
