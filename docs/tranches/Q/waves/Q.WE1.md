# Q.WE1 — the @deprecated alias DROP (the BREAKING 5.0.0 surface): `Animation` + `ScrollTimeline`/`ScrollTimelineOptions` + the ~33-consumer migration + `MIGRATION-5.0.0.md` + `proof:alias-dropped` (gate-first, born-RED over the BUILT d.ts)
...
1. **The migration-discovery deferral** — "the ~33-consumer migration uncovers a 23rd scattered consumer mid-tranche" (the classic P-inv-28 spring). **Pre-empted by the S2 enumeration**: all 22 demo sites + `test/group.test.ts` (the value-import) + the kf-`ScrollTimeline` test/README sites are LISTED NOW, so there is no mid-tranche discovery — the surface is closed before the cut.
