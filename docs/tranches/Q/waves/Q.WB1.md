...
| setTargets-seam | `src/animation/engine.ts:1170-1184` | `setTargets(...targets)` — the lifecycle point where the animation first has a resolved DOM target (the Phase-2 call site). VERIFIED: it does NOT re-parse; it walks `this.frames[*].interpVars` and propagates the target onto each `start`/`stop`/`value` `ValueUnit`. The Phase-2 step (env-populate + re-resolve `templateFrames[i].vars` + re-`parse()`) is a NEW extension here |
| template-snapshot | `src/animation/frame-compiler.ts:255-269` + `:464-467` | `templateFrames[i].vars` retains the Phase-1-resolved UNFLATTENED `Record<string, ValueArray>` (the pre-flatten snapshot the 2nd pass re-resolves); `parse()` re-flattens it via `parseAndFlattenObject` (`:464-467`) — so a re-`parse()` after a Phase-2 write-back recompiles cleanly |
| parse-reentry | `src/animation/engine.ts:352` | `parse()` → `compiler.parse(this.targets)` — the idempotent recompile the Phase-2 step calls after writing the resolved templates back (re-sorts/re-flattens; `(startIx,stopIx)` segment identity keeps it idempotent) |
...
- `customProps = (name) => { const c = getComputedStyle(target).getPropertyValue(name).trim(); const v = c !== "" ? c : target.style.getPropertyValue(name).trim(); return v === "" ? undefined : v; }` — read the COMPUTED custom-prop, falling back to the INLINE `style.getPropertyValue` when computed is empty (jsdom does NOT resolve registered/inline custom props into `getComputedStyle` reliably, so the gate's `style.setProperty("--p", …)` fixture must be readable via the inline fallback — this is the FRICTION the gate would otherwise hit; stated here, not discovered at impl)
...
SSR-safe: no `document`/no target → the fields are omitted (not assigned) and the residual node stays unresolved (idempotent — the same posture Phase 1 holds for `style(--p)` today). The env is built ONCE per `setTargets`, not per frame (the pass is a compile-time lowering, never on the hot path). NOTE the verified mechanism: `setTargets` (`engine.ts:1170-1184`) does NOT re-parse today — it walks the already-compiled `this.frames[*].interpVars` and propagates the target onto each `ValueUnit`. S3 EXTENDS `setTargets` with the env-populate + a Phase-2 re-resolve of `templateFrames[i].vars` + a re-`parse()`; the env build here is the precondition for that extended step, not a standalone effect.
...
**Cure.** In `setTargets`, after building the element-aware env (S2), re-run the lowering over the TEMPLATE declarations that carry a Phase-2 node, then recompile:
- The pass runs over the **pre-flatten `templateFrames[i].vars` snapshot** (the Phase-1-resolved, UNFLATTENED `Record<string, ValueArray>` retained on each template frame at `frame-compiler.ts:258` — the SAME `Record` `parse()` re-flattens via `parseAndFlattenObject` at `frame-compiler.ts:464-467`), NOT the already-compiled `interpVars` — so a Phase-1-resolved node (already a concrete `ValueUnit`) is returned as-is by `resolveNode` (`resolve-values.ts:387`), making the second pass idempotent over the Phase-1 result. Gated by `hasPhase2Node` (S1) so a pure-Phase-1 animation never re-resolves.
...
- The re-resolved `ValueArray` is written back onto `templateFrames[i].vars[prop]`, then `setTargets` calls `this.parse()` to re-flatten/recompile from the now-Phase-2-resolved templates (the existing `parse()` at `engine.ts:352` → `compiler.parse(this.targets)`). Interpolation then runs EXACTLY as today — the frame pair holds two concrete values where it held an unresolved `FunctionValue`. The re-`parse()` fires ONLY when at least one template carried a Phase-2 node (else `setTargets` keeps its existing fast target-propagate-only path, untouched).
...
- **`resolve-values.ts` + `ResolveContext` + `setTargets` + `templateFrames[i].vars` + `parse()` — already shipped** (`resolve-values.ts`; `engine.ts:1170-1184` setTargets; `frame-compiler.ts:258,464-467` the template snapshot + re-flatten; `engine.ts:352` the re-`parse()` entry). The typed Phase-2 seam P.W13 built is the substrate; this wave activates it by EXTENDING `setTargets` (env-populate + re-resolve the template snapshot + re-`parse()`). NO new module — the recompile rides the existing `parse()`/`compiler.parse` machinery.
...
- **FRICTION: the Phase-2 lifecycle point — `setTargets` does NOT currently re-parse.** VERIFIED on the tree: `setTargets` (`engine.ts:1170-1184`) does NOT call `compiler.parse(this.targets)` — it ONLY walks the ALREADY-compiled `this.frames[*].interpVars` and propagates the bound target onto each `start`/`stop`/`value` `ValueUnit` (so value.js's computed-unit DOM resolution reads the live box). The pre-flatten snapshot the second pass needs is NOT a `decl.value` reachable from a `setTargets` re-flatten (none happens); it is `compiler.templateFrames[i].vars` (`frame-compiler.ts:258` — the Phase-1-resolved, UNFLATTENED `Record<string, ValueArray>` retained on each template frame, the SAME `Record` `parse()` re-flattens via `parseAndFlattenObject` at `frame-compiler.ts:464-467`). **PRE-EMPT:** S2/S3 add the Phase-2 step to `setTargets` EXPLICITLY: after binding the target + building the element-populated env, re-resolve each `templateFrames[i].vars` value that `hasPhase2Node` against the env (writing the resolved `ValueArray` back onto `templateFrames[i].vars`), THEN call `this.parse()` to re-flatten/recompile from the now-Phase-2-resolved templates. This is a NEW step in `setTargets` (not a pre-existing re-parse — the spec is explicit that `setTargets` is extended, not that it already re-flattens); the re-`parse()` is paid ONCE per `setTargets`, gated to the `hasPhase2Node` declarations, so a pure-Phase-1 (or all-concrete) animation skips the re-resolve AND — when no template carried a Phase-2 node — skips the extra `parse()` (the existing fast target-propagate path is unchanged for the common case).
