...
| glass-ui types ALREADY shipped (TWO-arg form) | `node_modules/@mkbabb/glass-ui/dist/composables/motion/useViewTransition.d.ts:57` (`startViewTransition(mutate, options?: { types?: string[] })`), `:4` (feature-detect degrade), `:1` (Chrome 140+ note) | the TWO-arg `startViewTransition(mutate, { types })` is on the INSTALLED 4.0.1 surface (NOT the native `{ update, types }` object overload) — the typed VT is a NOW kf consume, NO glass-ui publish |
| no sceneIndex seam | `demo/app/scenes.ts:92,165,166` (`scenes`, `allScenes`, `sceneMap`) | the ordered array exists but NO `sceneIndex(id)` helper; `useSceneTransition(mutate, sceneHost)` (`useSceneTransition.ts:27-30`) does not receive the current id (the `Q.WC3-SCENE-INDEX` seam) |
| current scene id source | `demo/app/useSceneMachineApp.ts:25,36` (`currentSceneId: ComputedRef<string>`), `App.vue:3,376` | the machine's `currentSceneId` computed is the ONE current-scene source — threaded into `useSceneTransition` as the third arg (no second source) |
...
**Cure.** Export `sceneIndex(id): number` (or an ordered id list) from `demo/app/scenes.ts` over `allScenes` (`scenes.ts:165`), and thread the existing `currentSceneId` computed (`useSceneMachineApp.ts:25,36` — `currentSceneId: ComputedRef<string>`, already wired to `App.vue:3` as `:current-scene-id`) into `useSceneTransition(mutate, sceneHost, currentSceneId)` (the composable signature gains a third arg; `App.vue:376` passes `useSceneTransition(switchScene, sceneHostEl, currentSceneId)`) so the composable can read both the current and target index. This is the pure-construction seam S3's directional pass reads; it is built FIRST so S3 is never built ad-hoc against a missing helper. The current id is read from the EXISTING machine computed — no second source-of-current-scene.
...
1. In `useSceneTransition.ts:32`, compute `sign(targetIndex − currentIndex)` and call glass-ui 4.0.1's ALREADY-INSTALLED **two-arg** `startViewTransition(mutate, options?)` (`useViewTransition.d.ts:57` — signature `startViewTransition(mutate: () => void | Promise<void>, options?: { types?: string[] })`) as `startViewTransition(() => mutate(id), { types: [delta > 0 ? 'forward' : 'backward'] })`. **NOTE the call shape:** glass-ui's wrapper takes the mutate callback as the FIRST positional arg and `{ types }` as the SECOND options arg — NOT the native `document.startViewTransition({ update, types })` object overload (passing `{ update, types }` to the glass-ui wrapper would treat the whole object as the mutate callback and silently drop `types`). NO glass-ui publish.
...
**Constraint (NOW — the single-name invariant is PRESERVED).** This does NOT add a second VT name — it drives DIRECTION via `view-transition-type` on the SAME `scene-subject` name. `view-transition-type` is NOT yet Baseline (Chromium-only as of 2026, absent on Firefox/Safari); where absent, glass-ui's wrapper feature-detect drops the `types` arg and the untyped cross-fade is the free degrade — so the gate's `vt-directional` clause asserts the type WHERE supported and the type-keyed CSS rule + a completed untyped cross-fade WHERE not (S4, the honest-degrade clause), never a hard requirement of an unsupported feature in the headless runner.
...
2. **`vt-directional`** (the INTERACTION-axis observable — robustly measurable, NOT a mid-transition snapshot race): `:active-view-transition-type()` is active only for the ~hundreds-of-ms of the transition, so the clause does NOT race to snapshot it. Instead it observes the DIRECTION-DERIVATION the wave introduces, via TWO robust probes: (i) the demo exposes the last-computed transition types on a test hook (e.g. `runSceneSwitch` records the resolved `types` onto a `data-last-vt-type` attribute on the scene host, or a `window.__lastVtTypes` the gate reads — the SAME instrumentation pattern the existing perf gate uses to read committed state), so a real nav `scene[i]→scene[i+1]` reads `['forward']` and `scene[i]→scene[i−1]` reads `['backward']`; AND (ii) the type-keyed CSS rules (`html:active-view-transition-type(forward)::view-transition-old(scene-subject)` etc.) EXIST in the served stylesheet (a `getMatchedCSSRules`/stylesheet-scan assertion). Where the runner's chromium supports `view-transition-type`, the nav additionally completes without throwing; where not, the untyped cross-fade completes — the degrade is honest, not a silent skip. BITE: a directionless `startViewTransition` with no `types` → the test hook records `[]`/undefined AND no type-keyed rule exists → red.
...
| `vt-directional` | a real `scene[i]→scene[i±1]` nav, read the demo's recorded last-`types` test hook + scan the served stylesheet for the type-keyed rules | `useSceneTransition.ts:32` passes NO `types` → the recorded types are `[]`/undefined AND no `:active-view-transition-type()` rule exists (directionless cross-fade) | the hook records `['forward']` for next / `['backward']` for prev, AND the type-keyed CSS rules exist; the nav completes (typed where supported, untyped degrade where not) |
...
- **Independent of the engine-perf / correctness / consume waves.** File surfaces: `demo/app/scenes.ts` (the `sceneIndex` seam), `demo/app/useSceneTransition.ts` (the `types` directional pass), `demo/app/App.vue` (the directional VT CSS beside `scene-subject`), a NEW phone-narrow scroll-snap layer (`demo/@/components/custom/` + a `useScrollSnapScene` composable), `scripts/proof-scene-switcher-mobile.mjs` (NEW), `package.json` (gate roster), `.github/workflows/ci.yml` (the new-gate `npm run proof:scene-switcher-mobile` step in the demo-smoke job), and (GATED, on unshelf) the `n-stage-impl` rebase + `scripts/proof-n-stage-boundary.mjs`.
