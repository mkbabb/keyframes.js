...
| `DragOptions.bounds` / `transform` | `src/animation/drag.ts:46` (`transform`), `:70` (`bounds: {min,max}`), `:307-317` (`applyBounds` rubber-band/clamp) | a `{min,max}` hard clamp (rubberBand 0) + a client-px→value-domain map — the normalization the curve handle needs to stay in `[0,1]` |

The demo today **hand-rolls** exactly what `drag2D` models. `EasingCurveCanvas.vue:20` binds `@pointerdown="startDragging"` on raw `<circle class="control-point handle">` handles (the two draggable circles at `:115-120` / `:122-127`); `startDragging` is the colocated composable **`useEasingCurveDrag`** (`demo/@/components/custom/composables/useEasingCurveDrag.ts:43`, hooked at `EasingCurveCanvas.vue:237`), which owns the hit-test + the bespoke pointer→SVG-CTM transform (`pointerToSVG`, `useEasingCurveDrag.ts:52-65`) + the rubber-band/smoothing AND rides the SHARED control-surface drag seam `useDragCapture` (`useEasingCurveDrag.ts:3,128` — capture + the global `body.is-dragging` select-suppression token). That bespoke `useEasingCurveDrag` pipeline IS, modulo the shared select-suppression seam, the LIGHT `drag2D` primitive — the build-in **consolidates** the hit-test + CTM-transform + spring-follow onto the published primitive (KISS, no-legacy) while PRESERVING the `useDragCapture` select-suppression participation (see S2). And it discharges a live no-legacy violation: `drag2D`/`Drag2DHandle` are exported from the barrel with **ZERO live consumers** (`grep drag2D demo/ src/ test/ bench/` → only the `index.ts:88` export + the `drag-2d.ts`/`drag.ts` definition lines, NO demo/test consumer). The CONTRIVANCE-AUDIT RE-SCOPED this ("no barrel export without a live consumer"); `DemoControlPoint.vue` becomes that consumer and discharges the RE-SCOPE (`B5-kf-demo-arch`, `B2-pw7-democontrolpoint`).
...
| bespoke handle | `demo/@/components/custom/EasingCurveCanvas.vue:20,115-127,237` | `@pointerdown="startDragging"` + the two raw `<circle class="control-point handle">` (`:115-120`/`:122-127`) + the `useEasingCurveDrag` hookup (`:237`) — the build-in consolidates these |
| bespoke gesture composable | `demo/@/components/custom/composables/useEasingCurveDrag.ts:43,52-65,128` | `startDragging` = `useEasingCurveDrag` — the hand-rolled hit-test + `pointerToSVG` inverse-CTM transform (`:52-65`) + rubber-band, riding the SHARED `useDragCapture` seam (`:3,128`, the `body.is-dragging` token). The thing `drag2D` + the component coordinate-map replaces (the `useDragCapture` token participation is PRESERVED, not re-invented) |
| live precedent + the non-dup delta | `scripts/proof-easing-editor-live.mjs` clause (b) (`:36-37` doc, `:157-189` impl — drags `.control-point.handle[data-index="0"]`) | clause (b) ALREADY drags a `.control-point.handle` and asserts the bezier `d` mutates + the subject re-times — so (i) the new gate's `live-drag` keystone must assert the DISTINCT obligation (the handle is the `DemoControlPoint`/`drag2D` path), not re-implement this, AND (ii) `DemoControlPoint` MUST keep rendering the `.control-point.handle[data-index]` markup so `proof:easing-editor-live` clause (b) stays green |
| harness | `scripts/proof-drag-gesture.mjs` S4-drag2D clause (`:65-67` doc, `:609-616` impl), `scripts/lib/demo-driver.mjs` (`withPage`/`navToScene`/`serveDist`/`newContext`) | `page.mouse.down→move→up` over the BUILT `dist/gh-pages/` — the runtime gate lifecycle |
...
- **Renders** an SVG `<circle>` handle (+ an optional control-line `<line>` from its anchor) inside the parent's `<svg>` coordinate space — the curve-editor handle shape, identical to the `.control-point.handle` markup it supersedes. **The rendered handle MUST keep the `class="control-point handle"` + `data-index` markup** the existing live gate selects (`proof:easing-editor-live.mjs:157-189` drags `.control-point.handle[data-index="0"]`), so the consolidation is class-preserving and that gate stays green.
...
- **Constrains** the emitted value via `drag2D`'s per-axis `DragOptions` (`drag.ts:46` `transform`, `:70` `bounds`, `:307-317` `applyBounds`): `transform` maps client-px → curve-`[0,1]` against the parent SVG's `getScreenCTM()` (the CTM math moves INTO the component as the `transform` fn — the same inverse-CTM math `useEasingCurveDrag.ts:52-65` `pointerToSVG` performs today), `bounds: {min,max}` holds `x ∈ [0,1]` hard (`rubberBand: 0`) while `y` permits the declared overshoot. The library owns the gesture physics; the component owns ONLY the coordinate map. The per-axis CTM decouples cleanly under non-uniform scale (`preserveAspectRatio="none"`, the easing hero's geometry — Q.WC2's concern).
...
**Breach.** `EasingCurveCanvas.vue:20,115-127` binds `@pointerdown="startDragging"` over two raw `<circle class="control-point handle">` handles, where `startDragging` is the colocated composable **`useEasingCurveDrag`** (`composables/useEasingCurveDrag.ts:43`) — a bespoke pointer→SVG-CTM transform (`pointerToSVG`, `:52-65`) + hit-test + rubber-band/smoothing that `drag2D` + the `DemoControlPoint` coordinate-map already provide. Leaving both is duplicated drag surface (no-legacy / KISS violation). Separately, `drag2D`/`Drag2DHandle` are barrel-exported with ZERO live consumers (the CONTRIVANCE-AUDIT RE-SCOPE).

**Cure.** Replace the two bespoke `<circle class="control-point handle">` handles in `EasingCurveCanvas.vue` with two `<DemoControlPoint>` instances (one per bezier control point, bound `v-model` to the canvas's `bezierPoints`). Retire the hand-rolled gesture: the bespoke `useEasingCurveDrag` hit-test + `pointerToSVG` CTM transform + rubber-band fold into the component's `drag2D` `transform`/`bounds` options (S1); the drag-in-flight state (the `currentHandleIndex` `trace-smear--active` read) is driven by the `DemoControlPoint`'s emission (a `@dragstart`/`@dragend`-style event or the `velocity ≠ 0` read). **Preserve the select-suppression seam, do NOT re-invent it:** the existing path rides the SHARED `useDragCapture` seam (`useEasingCurveDrag.ts:3,128`) that owns the global `body.is-dragging` token; `drag2D`'s `Draggable` uses `setPointerCapture` (`drag.ts:273`) but does NOT touch `body.is-dragging`. So `DemoControlPoint` composes the SAME `useDragCapture` seam (its `onPointerDown` sets/clears the token) on `pointerdown`, NOT a bespoke per-component token write — the demo-wide select-suppression contract is inherited, not re-authored (`proof:drag-gesture` + `proof:control-surface-single-writer` stay green).

**Constraint (no-legacy, single-seam).** After this cure there is ONE gesture-physics path for the curve handle: `DemoControlPoint` → `drag2D` → `SpringProgress` (with `useDragCapture` providing the shared select-suppression seam, unchanged). The bespoke `useEasingCurveDrag` hit-test + CTM transform + rubber-band is DELETED (not kept beside the new path); if `useEasingCurveDrag` retains only the `useDragCapture` token wiring after the gesture-physics is removed, that residual collapses into `DemoControlPoint`'s own `useDragCapture` composition (no orphaned half-composable left behind). `DemoControlPoint.vue` becomes the live `drag2D` consumer that **discharges the orphan-export RE-SCOPE** (record the discharge in `docs/tranches/Q/PROGRESS.md`). The `proof:no-dup-utility` / `proof:composable-encapsulation` hygiene gates stay green (no duplicated drag composable left behind).

**Gate bite.** `proof:demo-control-point` `consolidated` clause: `EasingCurveCanvas.vue` renders `<DemoControlPoint>` (not the bespoke `<circle class="control-point handle">` + `@pointerdown="startDragging"`), and the bespoke `useEasingCurveDrag` hit-test/CTM transform is gone (only the shared `useDragCapture` seam remains, now composed by `DemoControlPoint`). Today: the bespoke handle + `useEasingCurveDrag` are present → RED. (This is the no-legacy corroborator; the live observable is S3's `live-drag`.)
...
**Cure.** Author `scripts/proof-demo-control-point.mjs`, a **RUNTIME (interaction) gate** over the BUILT `dist/gh-pages/`, mirroring the `proof:easing-editor-live` / `proof:drag-gesture` harness (`scripts/lib/demo-driver.mjs` `withPage` = serveDist + `resolveChromium` + context/teardown; `navToScene` = the per-EXPECTED-state settle). Wire it into the **`proof:correctness`** roster (`package.json`, beside `proof:easing-editor-live` / `proof:drag-gesture`) — the BLOCKING runtime tier (a kf-side build-in with no sibling dependency, so it CAN be hard-gated; this is the structural distinction from the retired report-all `proof:control-point-live`). **CI-coverage wiring (mandatory, else `proof:ci-coverage` reds):** `proof:ci-coverage` (`scripts/proof-ci-coverage.mjs:198-209`) asserts every non-EXCLUDED `proof:*` script literally appears as `npm run <gate>` in `ci.yml` — so the new gate ALSO needs an explicit `run: npm run proof:demo-control-point` step in the **demo-smoke** job of `.github/workflows/ci.yml` (it needs the demo build), beside the other runtime gates; chain-membership in `proof:correctness` alone is INSUFFICIENT. Four clauses:
...
3. **`live-drag`** (THE KEYSTONE — the APPEARANCE/INTERACTION-axis observable): navigate `#/easing` (in from another scene, exercising the editor-mount path), assert ≥2 `DemoControlPoint` handles are present, then drive a REAL `page.mouse.down → move → up` over one handle to a measured non-trivial offset and assert BOTH: (i) the bound normalized `(x,y)` **changed** (read via the bezier path `d` mutating — `EasingCurveCanvas`'s `bezierPathD` recomputes from the control points), AND (ii) the curve **re-times the subject** — the sampled easing value at a fixed `t` is DISTINCT after the drag (a drag that only repaints the `<circle>` but does not re-shape the curve OR re-time the ball still REDs). The probe drives the real `drag2D`-backed handle, not a grep of the component name. **Delta over `proof:easing-editor-live` (the non-duplication clause):** `proof:easing-editor-live` clause (b) already asserts a generic `.control-point.handle` drag re-shapes the bezier `d` AND re-times the subject; this clause's DISTINCT obligation is that the dragged handle is the **`DemoControlPoint`-backed `drag2D` path** (assert the dragged element is a `DemoControlPoint` instance — e.g. a `data-demo-control-point` marker or the component's resolved class — NOT a bespoke `<circle>`), so a regression that re-introduced the hand-rolled `useEasingCurveDrag` gesture would green `proof:easing-editor-live` but RED this clause. The `live-drag` keystone is thus a SUPERSET assertion (same observable, plus the consolidated-substrate witness + the `damped-no-overshoot` + `keyboard-operable` clauses below), not a re-implementation of the existing gate.
4. **`damped-no-overshoot`** (the S1 precision contract — an APPEARANCE-axis assertion): drive a FAST drag (a quick `down → move → up` that imparts a non-trivial release VELOCITY — the overshoot only manifests on a velocity-bearing fling, since `handleUp` re-seats the spring with `releaseVelocity`, `drag.ts:357`; a slow release wouldn't distinguish `0.86` from `1`), then sample the bound `(x,y)` across the settle window and assert it is **monotonic to the release value** (no sample exceeds the release-point coordinate beyond a tight epsilon). BITE: the default under-damped `drag2D` spring (`dampingFraction: 0.86`, `spring.ts:109`) overshoots past the drop point on the fling → red until S1's `springOptions:{dampingFraction:1}` lands.
...
| `light-substrate` | `grep "drag2D" demo/@/components/custom/DemoControlPoint.vue` | the file does not exist; the curve handle rides the bespoke `useEasingCurveDrag` CTM path (`useEasingCurveDrag.ts:43,52-65`), not `drag2D` | imports `drag2D` from the kf barrel; the bespoke `useEasingCurveDrag` hit-test/CTM transform deleted (the shared `useDragCapture` seam preserved) |
...
| `damped-no-overshoot` | FAST drag-release (velocity-bearing fling), sample the settle window | `drag2D` default `dampingFraction: 0.86` (`spring.ts:109`) overshoots past the drop point on the fling (a precision regression) | the bound `(x,y)` monotonic to the release value (critically-damped, S1 — `springOptions:{dampingFraction:1}` confirmed passable via `drag.ts:56→:169`) |
