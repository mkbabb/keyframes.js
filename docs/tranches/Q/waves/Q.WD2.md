...
3. **File the kf-side dispatch note + name the owning VJ-Q slot.** Append to `docs/tranches/Q/KF-TO-VALUEJS-Q.md` the exact probe outputs as the regression corpus, AND name the concrete value.js obligation that gives the two tripwires a GATED auto-flip exit (per inv-16: the kf wave authors only this cross-reference note; the foreign-tree FIX is the dispatch doc's VJ-Q slot). The probe corpus:
   ...
   **CROSS-DOC DEPENDENCY (the GATED exit — honored by the dispatch-doc-owning lane):** the none-channel + `color()`-wrapper serialization-fidelity fix is owned by **VJ-Q9 (CSS serialization fidelity — none-channel `<percentage>`/`<number>`-or-`none` powerless-channel round-trip + `color()`-wrapper preservation)** in `KF-TO-VALUEJS-Q.md`, 1.2.0, with value.js born-RED gate `proof:serialize-fidelity` asserting `parseCSSValue('oklch(0.6 none 200)').toString() === 'oklch(0.6 none 200)'` AND `parseCSSValue('color(display-p3 1 0 0)').toString() === 'color(display-p3 1 0 0)'`. (The breach this slot redresses: on installed value.js 1.1.0, `grep "none-channel\|wrapper-loss\|display-p3(1" KF-TO-VALUEJS-Q.md` was ZERO — the slot was added in the final cross-doc reconciliation so these two tripwires are no longer orphaned: they have a value.js OBLIGATION + a Q.WG4 `^1.2.0` consume edge, not the perpetual-tripwire risk friction-pre-empt #5 names.) The kf tripwires auto-flip when VJ-Q9 lands and kf re-pins `^1.2.0` (Q.WG4). This is a cross-doc note (a dependency the dispatch lane owns), NOT a foreign-tree edit by this wave.
4. Add `fast-check ^3.x` to `devDependencies` in `package.json`.

**Gate bite.** `proof:grammar-fuzz` and `proof:kf-differential` both absent from `package.json` gate roster today → all RED. (The owning VJ-Q9 dispatch slot IS now authored in `KF-TO-VALUEJS-Q.md` — the cross-doc dependency is satisfied; the tripwires' GATED exit is in place, so they are EXPECTED_FAILURE green-now, never a blocked-on-an-orphan-ask RED.)
