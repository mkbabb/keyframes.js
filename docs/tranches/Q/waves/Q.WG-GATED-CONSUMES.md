...
| the @function seam | `src/animation/resolve-values.ts:402` (arm guard `if (node.name.startsWith("--") && ctx.functions.has(node.name))`) → `:405` `return node` | the inert no-op `--ident(...)` arm: guard at `:402`, the `return node` (UNCHANGED — no guess) at `:405` (the @function inline seam, inert until value.js parses the call — VJ-Q6) |
...
| the S1/S2 suppress | `demo/spring/SpringSidebar.vue:43`; `AnimationControls.vue:72`; `TransportDock.vue` script `:348-375` + template handlers `:151`,`:196` (all VERIFIED) | the aria suppress + the dock twin (script block AND the two template `@pointerdown` handlers) deleted on the glass-ui BC publish |
...
**Cure (GATED on VJ-Q8 — the `ColorChannelPlan` + `lerpColorChannels` — AND on the Q.WB3-color measure-first precondition).** On the re-pin, Q.WB3-color folds the color leaves through the published plan instead of per-element `Color` boxing. `proof:color-soa` (Q.WB3's gate, born-RED) greens on the consume. **The anti-contrivance grounding (charter §7):** the VJ-Q8 cross-repo ask is authorized ONLY after Q.WB3-color's measure-first born-RED gate witnesses the color tail is a non-trivial frame slice with a fold-win ≥ a measured floor on kf's OWN path (the `KF-TO-VALUEJS-Q.md` VJ-Q8 measure-first precondition) — not on the bare "the tail is boxed" assertion. **Terminal-or-KILL:** if value.js declines the plan, OR the measure-first floor is not cleared (the color tail is a negligible slice / the plan-build overhead eats the win), the color tail ships BOXED (recorded with a measured tombstone) — never a perpetual block (the numeric SoA arm is fully in-realm, NOW).

### S7 — the glass-ui re-pin + the S1/S2 deletes (GATED on the glass-ui BC publish, USER-DOMAIN)

**Breach.** kf carries the aria suppress (`SpringSidebar.vue:43` + `AnimationControls.vue:72`) + the dock twin (`TransportDock.vue` — the `pointerHandled`/`onPlayPointerDown` SCRIPT block `:348-375` [`let pointerHandled` at `:348`, `onPlayPointerDown` at `:358`, the guard at `:373` — all VERIFIED] PLUS the two TEMPLATE `@pointerdown="onPlayPointerDown(...)"` handlers at `:151` + `:196` [VERIFIED] that invoke the twin) — band-aids for glass-ui defects.

**Cure (GATED on the glass-ui BC publish — GU-Q1 aria guard + GU-Q2 dock keepalive, the content-observed cut).** On the BC publish (observed by Q.WG-S1S2-HYGIENE's content-aware `glassCaps` probe, NOT the version sentinel): re-pin `package.json` glass-ui `~4.0.0` → the cut version (consume-and-delete in ONE atomic wave — the no-orphan-pin discipline); DELETE the S1 suppress lines + the S2 dock twin (the `:348-375` script block AND the `:151`/`:196` template handlers — the deletion is INCOMPLETE if the template handlers are left dangling, which `proof:workaround-deletion` S2's `pointerHandled|onPlayPointerDown` witness catches: it greps ALL `.vue` lines, so a half-delete keeps the gate RED). `proof:workaround-deletion` S1+S2 flip PENDING→GREEN (witness ABSENT, `apiPresent` true on the content-probe); `proof:glassui-aria-ask` flips GREEN (the mounted pill renders `aria-orientation === null`); `proof:live-session` S5 confirms a dock-PLAY produces motion. **Terminal-or-KILL:** the glass-ui CUT is USER-DOMAIN (confirm-first); if it does not ship during Q, the deletes stay GATED (the band-aids stay, recorded as a glass-ui-owned defect kf cannot self-cure), never a false-RED — the false-RED itself is already cured by Q.WG-S1S2-HYGIENE.
