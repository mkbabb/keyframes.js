...
| **L.W11** | A | Design refinement (the instrument language, deftly folded) | the 11-page Opus design-fleet treatments, hardened to the user's verdict (REFINE not abrogate; the **crayon primaries KEPT by register**; the signature moments as proportionate easter eggs) — the four pillars (glass · paper · audacious typography · mathematics) amplified per-scene; the value.js pair a cross-repo design note | **`proof:crayon-preserved`** (NEW, born-RED on any keeper-token recolor) + per-scene refinement arms + visual-lock re-baseline; the **TASTE boundary is USER-DOMAIN** (the K precedent) | — (kf demo; glass-ui + value.js 0.13.0 sufficient) |
| **L.WZ** | — | Close (recap · deferred terminal · release) | FINAL.md (inv ε); the deferred ledger TERMINATED; the prompt-recap confirmed; **the version cadence** — the round-trip-totality + barrel-dogfood + keyframes-vue-publish set argues a MAJOR `5.0.0` (USER-DOMAIN; L.WZ proposes the criteria) | proof:all green; the deploy round-trip re-observed; the L.W11 TASTE verdict given | — |

**DAG:** L.W0 (now) → Band A (W1∥W2∥W3∥W4∥W5∥W6∥W7∥W8∥W11 — largely file-disjoint; W2 composes W1's engine, W6 gates on W1+W2, W11 is demo-only over the shipped four pillars, the rest independent) · Band B (W9 dispatch immediately, consume-on-publish; W10 research-spike then coordinate) · L.WZ closes when Band A is green, the L.W11 TASTE verdict is given, and Band B's edges have consumed or honestly circled-back. **Gate-first throughout: each Band-A wave's born-RED gate lands before its cure; `proof:crayon-preserved` reds the instant a refinement abrogates the kept palette.**
