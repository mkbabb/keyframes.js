...
| **K.W3** | The layout transposition | **IMPL** (landing — no commit yet) | `proof:dock-anchor-derived` (NO hardcoded dock offsets — anchor tokens derived, not retuned) + `proof:pathological-cluster` (a width AND height ceiling after which docks + controls CLUSTER; `clamp()`/container-query driven) | Working tree carries `scripts/proof-layout-cluster.mjs` (uncommitted); `package.json` bumped (uncommitted); no sha yet | after W1 (∥ W2); BINDING boundary: W3 owns the GRID/anchoring tier |
...
| **CH-3** mobile | 5 (D…J) | a born-RED mobile oracle reading the spring slider smooth-not-stepped (K.W4) | pending K.W4 |
| **CH-8** amiga | 4 (H…J) | a born-RED appearance/amplitude/persistence oracle (K.W0) | pending K.W0 gate clause |
...
| **DL-K7** AX-1 control-point | 5 (E…J) | consume-on-3.13.0 OR a reasoned build-in-kf decision (K.W1) | pending K.W1 full verification |
...
| **DL-K11** mobile Lighthouse floors | 5 (B…J) | a calibrated-host measurement (P6-WITNESS — the on-device born-RED witness) (K.W6) | pending K.W6 measurement |
| **DL-K16** FB-1 composition | 5 (F…J) | K.W7 LAND (the Band-II fidelity-floor LEAD; engine-internal, value.js-independent) | pending K.W7 |
| **DL-K17** diagnostics sink | 5 (F…J) | the K.W7 channel + the K.W6 published value.js producer (0.12.0 `ParseDiagnostic`, N2 row 10) | pending K.W7 |
| **DL-K18** parse-cache bound | 6 (C…J) | consume value.js VJ-4 `{maxCacheSize}` (K.W1) | **EXITED** `c427e39` published-consume-edge (^0.12.0) |
| **DL-K20** value.js next-slice | 8 (C…J) | published-consume-edge form (each VJ item born-RED-able on publish) (K.W1) | pending K.W1 per-item verify |
| **DL-K24** dock double-click | 10 (pre-A…J) | VERIFY-ONLY re-confirm on the re-pin (K.W1) | pending K.W1 re-verify |
