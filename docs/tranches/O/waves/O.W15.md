...
**Sequence (CANON #8 Band F consume order):** O.W12 (S1+S2 workaround deletes + BC re-pin) → O.W13 (design-paint baseline lock) → O.W14 (lighthouse posture flip + content-visibility + word-gap) → **O.W15** (the N Stage unshelf, the owning wave). O.W14's lighthouse floors lock BEFORE O.W15's N-Stage perf integration (the sequence + motion-path ceiling additions fold into W15's N-Stage perf integration). The S2 gate-script authoring (`proof:n-stage-boundary`) fires NOW pre-BC, independent of the W14 ordering; only the unshelf impl (S1/S3/S4/S5) waits on O.W14 + the BC cut.
...
- **Branch topology (CANON #7).** The O dev branch is **`tranche-o-dev`** (forked from master). The N-Stage substrate lives on **`n-stage-impl`** (forked at commit `5106416`, behind master, carrying the 4 impl commits + the SHELVED spec). O.W15 rebases `n-stage-impl` onto the O dev line (`tranche-o-dev`, post-O.W12 BC re-pin) and merges the unshelfed N impl in at the BC cut. "O-master" throughout this wave means the O dev branch `tranche-o-dev` (forked from master), not a separate master line.
- N was SHELVED by owner directive (commit `e2375b8`: "the stage selector SHELVED per owner; impl preserved on this branch, spec kept"). O.W15 is the unshelf — the N impl merges into the O dev line (`tranche-o-dev`) at the BC cut.
- The `n-stage-impl` branch diverged at commit `5106416` (11 commits behind master at O audit time), carrying stale deps (`@mkbabb/parse-that: "^0.9.0"`, `@mkbabb/value.js: "^0.13.0"`) and the S7 flat-comma normalize workaround (`utils.ts:183-200`) that master has since deleted (M consume, proof:workaround-deletion S7 GREEN). The rebase MUST resolve the S7 conflict by KEEPING master's deletion.
...
**Constraint.** DO NOT rebase before O.W12 is committed (the BC re-pin must be on the O dev line `tranche-o-dev` before the rebase base is set). Rebase only the four impl commits (`b271fa1`, `f14e943`, `8606eec`, `e2375b8`) — the earlier commits carry M-dev docs already on master (and thus on `tranche-o-dev`, which forks from master).
...
**The born-RED arm: the PLANTED-violation, NOT the vacuous ENOENT-exit.** On `tranche-o-dev` (the O dev branch, before S1's rebase) `SceneStage.vue` is absent, so a naive run exits 1 by file-not-found — but that ENOENT-exit is a VACUOUS born-RED: it proves nothing about the gate's boundary logic (any missing-entry esbuild call exits 1; the gate never walks a graph). The GENUINE born-RED is the PLANTED-violation arm, run against the REBASED tree (post-S1, where the stage component tree is present):

1. **PLANTED-violation born-RED (the real witness, post-rebase).** On the rebased tree, add `import { loadAnimationEngine } from "@src/animation/index"` to a stage composable (e.g. `useSceneStage.ts`) → esbuild bundles the entry, walks `result.metafile.inputs`, finds `src/animation/engine.ts` as a STATIC input (the dynamic-import split that `loadAnimationEngine` pulls is materialized as a static edge once it is imported eagerly) → the gate exits 1 NAMING `engine.ts` (and the composable that pulled it). This is the gate biting its REAL observable — a heavy static edge in the Stage graph — not a missing-file accident. Remove the planted import → exit 0 (the LIGHT-only graph holds).
2. **The ENOENT pre-S1 state is noted but is NOT the born-RED of record.** Pre-rebase, the entry is absent; the gate exits 1 trivially. That is the absent-component state, not the gate's discriminating bite. The born-RED witness is arm 1: the planted heavy import against the rebased tree, where esbuild genuinely walks the graph and the gate's assertion logic fires.
...
| Gate / clause | Witness | Failure mode (the REAL observable) | Expected after O.W15 |
|---|---|---|---|
| `proof:n-stage-boundary` PLANTED-violation arm (**the born-RED of record**) | on the REBASED tree, add `import { loadAnimationEngine }` to a stage composable (e.g. `useSceneStage.ts`) | exit 1 — esbuild walks `metafile.inputs`, finds `src/animation/engine.ts` as a STATIC input, NAMES it (and the composable). This is the gate's discriminating bite over a REAL heavy static edge — NOT a missing-file accident, NOT a grep proxy | exit 0 after removing the planted import — esbuild walks the STATIC module graph of `SceneStage.vue` and finds zero heavy-engine or value.js static input edges; the LIGHT boundary holds |
| `proof:n-stage-boundary` ENOENT pre-rebase state (NOTED, not the born-RED of record) | `SceneStage.vue` absent on `tranche-o-dev` before S1 | exit 1 (entry not found) — a VACUOUS born-RED: any missing-entry esbuild call exits 1; the gate never walks a graph, so this proves nothing about the boundary logic | (superseded by the rebase: after S1 the entry is present and the planted-violation arm is the real witness) |
| `proof:stage-supersedes-dropdown` C1 | `grep "SceneStage" demo/app/App.vue` → 0 hits on `tranche-o-dev`; Stage absent pre-rebase | C1 RED — `pointerdown` fires on dock trigger; no Stage overlay in DOM | C1 GREEN — Stage overlay present in DOM on `pointerdown` |
| `proof:stage-supersedes-dropdown` C7 (AT fallback) | n/a (Stage absent pre-rebase) | After S3: if `@pointerdown` mistakenly fires on keyboard path → C7 RED (Stage opens on Tab+Space) | C7 GREEN — keyboard path opens reka Select dropdown only; Stage NOT opened |

**Born-RED of record — the PLANTED-violation arm, NOT the vacuous ENOENT-exit.** The honest born-RED is the planted heavy import run against the REBASED tree: with the Stage component tree present (post-S1), add `import { loadAnimationEngine } from "@src/animation/index"` to a stage composable → `node scripts/proof-n-stage-boundary.mjs` exits 1, esbuild having walked `metafile.inputs` and found `src/animation/engine.ts` as a STATIC input, naming it. That is the gate's discriminating bite over a REAL heavy static edge. The pre-rebase ENOENT state (`SceneStage.vue` absent on `tranche-o-dev` → exit 1 file-not-found) is NOTED but is a VACUOUS born-RED — it proves nothing about the boundary logic, since any missing-entry esbuild call exits 1 without walking a graph. The gate-first sequence: author S2 (the script) NOW; rebase the Stage tree in S1; then witness the born-RED via the planted import on the rebased tree (and confirm exit 0 once removed).
...
- **`proof:n-stage-boundary` S2 (gate-first, NOW)** — the gate script (`proof-n-stage-boundary.mjs`) is kf-internal and can be authored NOW on `tranche-o-dev`. The gate-first obligation: author the script first; its born-RED witness of record is the PLANTED-violation arm against the REBASED tree (not the vacuous pre-rebase ENOENT-exit). The gate authoring does NOT wait for BC cut, but the discriminating born-RED is observed post-S1-rebase.
...
- **O.W14 (lighthouse posture flip) — the predecessor (CANON #8 W14→W15 edge).** O.W14's lighthouse floors lock BEFORE O.W15's N-Stage perf integration: O.W14 flips the posture on the 6 existing scenes, and O.W15's N-Stage perf integration adds the `sequence` + `motion-path` ceiling rows on the integrated tree. The Band F consume order is `O.W12 → O.W13 → O.W14 → O.W15`; O.W15's unshelf impl follows O.W14 GREEN (the S2 gate-script authoring is the only NOW, pre-BC, ordering-independent piece).
...
**S2 (gate authoring, `proof:n-stage-boundary`)** is kf-internal and may be authored NOW on `tranche-o-dev`. Its born-RED witness of record is the PLANTED-violation arm on the REBASED tree (a planted `loadAnimationEngine` import → esbuild names `engine.ts` as a static input → exit 1), NOT the vacuous pre-rebase ENOENT-exit. This is the pre-BC gate-first action.
...
The impl sequence:
1. NOW (pre-BC): author `scripts/proof-n-stage-boundary.mjs` (S2 gate script only). The pre-rebase ENOENT exit-1 is a VACUOUS born-RED; the witness of record is deferred to step 2's planted-violation arm on the rebased tree.
2. BC cut fires + O.W12 complete: rebase `n-stage-impl` onto `tranche-o-dev` (S1), verify `npm run check` exits 0, verify `proof:workaround-deletion` S7 still GREEN. THEN witness the gate's real born-RED: plant `import { loadAnimationEngine }` in a stage composable → exit 1 naming `engine.ts`; remove → exit 0.
