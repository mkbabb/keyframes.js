...
**Branch:** `tranche-j-dev` @ `b0a277b` (forked off `master` @ `4072af9` — the I-close tip + the
adopted post-close CI/deploy tail; value.js `^0.11.2`, parse-that `^0.9.0`, glass-ui `~3.11.2`
consumed PUBLISHED after the `56aa00f` currency re-pin).
**Type:** TRANCHE J — **CLOSE (IMPLEMENTED).** All ten waves + the in-flight W7c live-audit band +
the P0 subject-write fix LANDED and merged to `tranche-j-dev` @ `b0a277b` (§1 board carries the
per-wave commit + headline-gate outcome). **This board is the close state:** §1 records every wave
IMPLEMENTED, the §"Open deferrals" ledger is TERMINAL (75 rows, zero un-dispositioned), and the
chronic-closure substrate transition is EXECUTED (`proof:chronic-closure` GREEN on this J ledger).
The authoritative close narrative is `docs/tranches/J/FINAL.md`; this board is consistent with it
(the §G single-voice discipline — where the FINAL supersedes a frozen DEV-phase disposition it says
so explicitly).
**Dev-phase date:** 2026-06-10 · **Close date:** 2026-06-11. **Version in tree:** `4.1.0` at the
close commit; the honest MINOR `4.1.0 → 4.2.0` cut + the npm publish (the first-ever `release.yml`
run) + the close-merge auto-deploy RE-observation are USER-DOMAIN (Mike Babb, confirm-first), fired
AFTER this commit — `release.yml` had NEVER run before J (`audit/wave-I.WZ-postclose.md §F(g)`).
...
Status legend: **AUTHORED** (wave spec on disk in the dev phase) · **IMPLEMENTED** (the wave LANDED
on `tranche-j-dev` with its commit hash + its headline gate GREEN). **At the J.WZ close, ALL TEN J
waves PLUS the in-flight W7c live-audit band PLUS the P0 subject-write fix are IMPLEMENTED and merged
to `tranche-j-dev` @ `b0a277b`** — the board below records each wave's landing commit and its
headline-gate outcome, verified against the wave-impl records (`waves/J.W*-impl.md`) and the merged
tree. This is the inv-ε discipline at the close: a wave is recorded IMPLEMENTED only because the tree
HOLDS its commit AND its headline gate is GREEN — never the INVE-1 overclaim shape J indicts in I's
FINAL (`J.md §ENFORCEMENT`). (The dev-phase AUTHORED snapshot — *"all ten specs on disk; each header
`Phase: DEV — spec authored`"* — is superseded by this IMPLEMENTED close board; the FINAL §2 is the
sole authoritative landing narrative and this board is consistent with it.)
...
|---|---|---|---|---|
| **J.W0** | **THE DEPLOY BOUNDARY (leads — P0)** — formal adoption of the 8-commit I post-close tail (the tail gets its tranche home + terminal disposition); `navToScene(page, sceneId, expected)` lands in `scripts/lib/demo-driver.mjs` (wait predicate = PER-EXPECTED destination state, ceiling-timeout, load-independent); `scene-control-dfa` + `scene-transition-perf` migrate onto it; the PRODUCT half — the dock trigger projection made born-correct from the DFA (never the SOURCE scene's stale label); GH repo secrets verified | `scripts/lib/demo-driver.mjs` · the dock control-surface projection · the deploy chain | **One OBSERVED clean round-trip: a real master push → green CI (demo-smoke end-to-end GREEN on the Linux runner, scene-control-dfa included, ZERO escapes) → `deploy-pages.yml` auto-fires → the live site serves the pushed bytes.** Born-RED witness: red on CI run `27228309606`'s trigger='null'-under-load shape before the cure | **IMPLEMENTED · `09a56bf`** — `navToScene` + both gate migrations + the dock projection born-correct from the DFA + the tail adopted (`waves/J.W0-impl.md`). **The OBSERVED round-trip is the J oracle of record:** push `c6c3c37` → CI run **`27310054675`** green end-to-end on Linux (`scene-control-dfa` included, ZERO escapes — the first fully-green CI run in repo history) → deploy run **`27310920981`** fired via the AUTO `workflow_run`-on-`push` arm (`event_name == workflow_run`, NOT `workflow_dispatch`) → keyframes.babb.dev serves `index-DiVbdzH3.js`. INVE-1 CURED (re-witnessed at the J.WZ close merge) |
| **J.W1** | **THE ENGINE TOTALITY PASS** — ENG-1 the per-card serializer unified onto serialize-from-template (the sibling DOM-resolving path dies in the same motion); ENG-2 `createFrame`'s `seekPreviousValue` made total; SEAM-1 the selector guard rejects ALL non-conforming input with the typed error; SEAM-2 the kf-side `parseCSSValueUnit("")` pin; SEAM-4 the rotate()-shorthand round-trip de-vacuoused; SEAM-3 the round-trip corpus gains the `var()`/`calc()`/`matrix3d()` fixture trio (not var() alone); the unit pyramid (var()-round-trip, bind-proof, `binarySearch`, `decay`); W0-5 clause (e) | `src/animation` (un-fenced — the permanent engine rule) · the serialize seam | New unit tests witnessed born-RED on the pre-fix tree (git-stash probe). Runtime: `proof:engine-no-throw-on-play` extended — the EDITOR per-card pane round-trips a `var()`-bearing animation (the ENG-1 oracle is the rendered card); the garbage-selector path asserts the TYPED error in the live console | **IMPLEMENTED · `e8ff39e`** — ONE serialization authority (ENG-1, the sibling DOM-path DELETED in the same motion), `createFrame` made total (ENG-2), the typed selector guard (SEAM-1), the value.js `parseCSSValueUnit("")` pin (SEAM-2), the `var()/calc()/matrix3d()` fixture trio (SEAM-3), `rotate()` de-vacuoused (SEAM-4) + the unit pyramid. `proof:engine-no-throw-on-play` EXTENDED **GREEN all clauses** (the EDITOR per-card `var()` round-trip); the unit pyramid born-RED witnessed on the pre-fix tree (git-stash/planted-mutation probes); `npm test` 738 passed + 3 expected-fail (`waves/J.W1-impl.md`) |
| **J.W2** | **THE DEMO SEAM COMPLETION** — W4-3/W4-4 EasingCurveCanvas + PlaybackRibbon onto the shared drag seam (the B6 latent class to ZERO); DS-1 + S4-stretch the `selectedControl` single-writer COMPLETED (the CubeScene rogue write dies; the DFA projection is the ONLY writer); M2 `isPanelTransitionDone` off the spring settle (not CSS `transitionend`); CD-1 `AnimationMenuBar → TransportDock` executed (the D overclaim terminated); DS-2 the 17 reactive refs/frame onto the non-reactive write; `useRafLoop` → `onScopeDispose` | the shared drag composable · the controls single-authority seam | `proof:drag-gesture` extended to the two recovered surfaces (real `page.mouse` drag, no text-selection, gesture lands); the mobile sheet open→close→RE-OPEN on 390×844 scrolls to content (M2 runtime); a live scene sweep asserts the rendered control surface == the DFA projection on every entry (no stale latch), the no-rogue-writer grep as hygiene corroborator | **IMPLEMENTED · `7023e15`** — the B6 drag seam to ZERO residue (W4-3/W4-4), the `selectedControl` single-writer COMPLETED (DS-1 rogue write deleted + S4-stretch flat mount), M2 cured off the spring settle, `AnimationMenuBar → TransportDock` EXECUTED (CD-1 terminated, grep 0), the spring painters off the render graph, the 5 LS-20 casts typed. `proof:drag-gesture` (6 surfaces) + `proof:sheet-reopen-scroll` + `proof:control-surface-single-writer` GREEN on a fresh build; the cured battery born-RED of record (the easing↔cube projection round-trip) → GREEN (`waves/J.W2-impl.md`) |
| **J.W3** | **THE ESTATE INDUSTRIALIZED (net-deletion)** — `withPage()`/`withBrowser()` + the shared `serveDist`/MIME/chromium/`navToScene` exports in `scripts/lib/` (the 43/51/54 copies MIGRATE, ≈2 kLoC deleted); the ONE `IN_CI` helper + per-gate DECLARED on-device posture (P6 mechanical; the third tier named); `proof:all` == the CI roster (the 3 CI-only gates enter a tier; a two-way `proof:ci-coverage` clause); the meta-gate derives its roster FROM `proof:correctness` (T4); `proof:demo-fonts` tier-decided; KILL `proof:repin-safe`; `proof:deps-current` floors advance; W7-1/W7-2 leg-scoped; the stale-refs purge; T3 the two surviving proxies re-labeled honestly | `scripts/lib/` · the gate taxonomy · the CI roster | The bite-preservation oracle: a SAMPLED set of migrated gates re-witnessed born-RED on their recorded defect (no migration may lobotomize a gate); `proof:all` == CI proven both ways; gate COUNT and estate LoC measured strictly DOWN (the net-deletion rule, before/after recorded) | **IMPLEMENTED · `8c66e3e`** — the shared `scripts/lib/` lifecycle (~56-gate migration), the ONE `IN_CI`/`observe()` ci-env authority + per-gate DECLARED P6 posture + the taxonomy doc, the two-way `proof:all == CI` (`proof:ci-coverage`), the meta-gate roster DERIVED from `proof:correctness` (T4), `proof:repin-safe` KILLED, the floors advanced, the stale-refs purged, the two proxies re-labeled (T3). **Net-deletion EXECUTED: estate LoC `35,191 → 31,775` strictly DOWN**; bite-preservation 9/9 sampled gates RED-on-plant → GREEN-on-restore; ADVERSARIALLY VERIFIED (`waves/J.W3-impl.md`) |
| **J.W4** | **THE AXES BATTERY** — `proof:live-session` gains the MOBILE (390×844, `hasTouch`, real touch taps/drags), REDUCED-MOTION (`emulateMedia`), DARK (color-scheme token surface), KEYBOARD (Tab order/focus-visible/Enter-Space) legs + the SCENE-SWEEP widened to every routed scene; CH-3 re-certified on a MOBILE oracle (`sheet.bottom ≤ menubar.top` measured ON the mobile viewport); `proof:lighthouse-mobile` enters a tier under its P6 posture; EP-3 flip/drag/draw-svg dispositioned (demo home or recorded BOOK) | the live-session battery · the W3 harness · the mobile/a11y axes | Each new leg witnessed born-RED-able against a PLANTED defect (a deliberate local mutation reds the leg); then the full battery GREEN with the accumulated budget = 0 per leg. The CH-3 mobile oracle bites on a planted occlusion | **IMPLEMENTED · `5fd5df4` (ph1) + `a9f4b2a` (ph2)** — PHASE 1 (input-modality, on W0+W3): `proof:live-session` GREEN with the mobile-touch (390×844 `hasTouch`) + reduced-motion-snap + keyboard-actuation + every-scene-sweep legs at budget 0; `proof:live-session-mobile` GREEN 11/11, CH-3 re-certified on the MOBILE oracle (`sheet.bottom ≤ menubar.top`); `lighthouse-mobile` tiered under P6, EP-3 dispositioned. PHASE 2 (appearance-certification, on the suffused W7a tree): the mobile overlap==0 + dark `--ball-tone` contrast + ghost-rail-absent legs GREEN, **and `proof:appearance-suffusion` (the W7a hard gate, flagged NOT-YET-AUTHORED at the W7a close) AUTHORED, tiered, and GREEN**. Every leg born-RED witnessed on a PLANTED dist defect (byte-restore shasum-verified) (`waves/J.W4-impl.md`) |
| **J.W5** | **THE PUBLISHED SURFACE** — `proof:published-surface` (`npm pack --dry-run` contents == declared files, no `_redirects`, d.ts complete; every public export taught or enumerated; ENG-6 interface drift-gated); README §Beyond CSS completes (all ~13 primitives); the doc-rot purge (root CLAUDE.md REWRITTEN to the tree, `src/animation/CLAUDE.md` +9 modules, `demo/CLAUDE.md` rewritten); BP-1 `_redirects` out of the tarball; the changeset consolidation (the honest MINOR, both patches consumed — version cut + publish USER-DOMAIN at WZ); the supersession annotations (pointer-only); the stray PNG relocated | the npm surface · the README · the doc tree | `proof:published-surface` witnessed born-RED on the CURRENT tree (the `_redirects` leak + the 9 untaught exports red it today); GREEN only when the tarball, the exports, and the README agree. The README examples EXECUTE against the built dist (the docs oracle is runnable truth) | **IMPLEMENTED · `9b475aa`** (+ `c6c3c37` the `!dist/gh-pages` files-negation hardening) — `proof:published-surface` GREEN (all five runtime clauses + the hygiene tripwire, against the BUILT dist + the packed tarball; `_redirects` out, the d.ts complete, the spurious `vue` peer removed per ED-5); `proof:readme-runs` GREEN (17/17 snippets EXECUTE against dist, 20 `// =>` verified); the doc-rot purged (root CLAUDE.md REWRITTEN to the real 28-module tree + 69-file test count). **Born-RED witnessed on the pre-fix tree** for clauses (a)/(b)/(e)/(f) + a planted (d) drift (`waves/J.W5-impl.md`) |
| **J.W6** | **TERMINATIONS (P-invariant-28 — nothing rides a fifth tranche)** — FB-2 author `proof:event-ordering`, LAND-or-KILL on the numbers; SoA `lerpArray` bench on the real-K corpus, ADOPT-or-KILL; FB-5 intrinsic-size verify Baseline, BOOK-with-date or KILL; FB-6 `Mod+K` owner decided, BUILD-or-KILL; PF-1 Three.js named imports (measured bundle delta); PF-3 Monaco static-edge re-verify; EF-3 `parseLinearStops` shim retirement check; GH-6/DEP-1 CNAME confirm (OUT — verify only) | the four ≥4-tranche riders · the perf frontier | Every rider exits with a MEASUREMENT ARTIFACT (bench/probe output checked into the wave note) or a reasoned KILL record; the J close ledger carries ZERO rows tagged MEASURE-FIRST without a measurement. The PF-1 bundle delta recorded before/after | **IMPLEMENTED · `40fc605`** — the full termination ledger, every rider EXITED with its measurement artifact pasted INLINE: **FB-2 LANDED** (microtask turns/frame 1.998→0 + 4.993→0 = −100%, `proof:event-ordering` GREEN, born-RED ×2); **SoA `lerpArray` ADOPTED** (16.6× = 94.0% at K=8, threshold cleared 4.7×); **FB-5 KILLED** (Chromium-only Baseline, no kf commitment); **FB-6 KILLED** (no owner across 4 tranches); **PF-1 KILLED** (measured `vendor-three` delta = exactly 0 bytes, the 100–200 KB estimate FALSIFIED, refactor reverted); **PF-3 PASS** (verify-only); **EF-3 KEEP-with-probe** (HANDOFF); DEP-1 verify-only. ZERO MEASURE-FIRST rows without a measurement (`waves/J.W6-impl.md`) |
| **J.W7a** | **THE APPEARANCE-GRAMMAR SUFFUSION** (the design directive — the half J.W4 certifies; ON J.W4's critical path) — the protagonist made unambiguous (plated stage grammar unified, the amiga slab joins the rounded-glass register, subjects sized/centered, the controls pane recedes via `glass-wash`, mobile hero/subject collisions die); the Instrument Serif display voice suffused from the doorway to the scenes (scene-identity at the display rungs, MATH readouts promoted MetricBadge xl + AnimatedDigit PROMOTION use); the `--ball-tone` parameterization of `.progress-ball` (every subject keeps its icon's hue; the aquamarine literal dies; readouts gain the scene accent; the colorful favicon + `theme-color`); the easing stage projects its OWN bezier; designed coordinate-grid fields + FourierField where befitting; the ghost rail dies; the top-center band de-conflicted; the playback-affordance grammar converged. Every visual delta NAMED + enumerated (the isomorphic exception); CP-1/TYP-2/XH-1..4/protagonist band | the demo appearance (the named isomorphic exception) · the visual-lock baseline | The per-finding RUNTIME assertions on the live page: the subject's computed `--ball-tone` per scene == its icon hue token; the scene-title display register present; the mobile 390×844 hero/subject overlap == 0 px; the easing stage's projected curve PRESENT + MUTATING with a handle drag; the ghost rail ABSENT; the amiga stage rounded-glass computed style. `proof:live-session` budget stays 0; the visual-lock baseline re-captured WITH the wave | **IMPLEMENTED · `8c7910b`** — all six bands LANDED (D1–D23): protagonist plated+sized + the amiga slab joined the rounded-glass register, the Instrument Serif display voice suffused from the doorway (D7), the `--ball-tone` per-scene icon-hue parameterization (D9, the aquamarine literal dead), the easing floor projects its OWN bezier (D14), the ghost rail dead (D20), the colourful favicon. **`proof:appearance-suffusion` clauses (a)–(g) computed live on the page** (subject `--ball-tone` == icon hue per scene; mobile hero/subject overlap == 0 px; ghost rail ABSENT; amiga rounded-glass); the visual-lock baseline RE-CAPTURED in the close motion; every delta NAMED in the §S6 register (`waves/J.W7a-impl.md`) |
| **J.W7b** | **THE CONSUME-EDGE + glass-ui HANDOFF** (inv-16 sibling-coordination — file-disjoint from W7a; NOT on J.W4's path; parallel, AX-gated) — consume-to-delete (MetricBadge/AnimatedDigit consume-to-delete use, SegmentedTabs, ToggleChip cell, ScrubberTimeline, `.fade-slide` PRM classes, gold-shimmer dedup STY-1, the STY-2..6 token items) — each kf-hand-rolled surface deleted in the SAME motion the published glass-ui primitive is consumed (no legacy beside its replacement); the glass-ui HANDOFF ledger (25 REFINE + 21 ABSTRACT, headlined by the CurveEditorCanvas/control-point primitive, the rail/ball pair, the status-badge tone recipe) → `glassui-AX-handoff.md` (kf consumes only what AX PUBLISHES, never a kf-side patch). NO kf appearance delta is owned here; re-captures NO baseline | the glass-ui consume-edge · the AX coordination doc (no kf appearance delta) | **The boundary oracle (NOT a kf appearance gate):** `glassui-AX-handoff.md` COMPLETE (every one of the 46 REFINE/ABSTRACT items dispositioned with its evidence anchor + the consuming kf seam named); each consume-to-delete edge gated on the PUBLISHED glass-ui version arriving — deleted kf surface + consumed primitive land in ONE motion (the hand-rolled-surface-GONE grep is the hygiene corroborator). No born-RED kf-runtime witness shared with W7a — no kf delta to re-baseline | **IMPLEMENTED · `ad84961`** — set (i) consume-to-delete LANDED on glass-ui `3.9.0` (each twin DELETED in the SAME motion, the gone-grep clean, pixel-isomorphism proven on the pre-J visual-lock baseline with ZERO re-capture); set (ii) BOOK-with-target-version recorded for the consume-on-FUTURE-AX-publish band (NO kf deletion — the B7 vaporware lesson); **`glassui-AX-handoff.md` 46/46 dispositioned** (clause (a) completeness-oracle GREEN). ZERO kf appearance delta; `proof:visual-lock` GREEN on the pre-J baseline (`waves/J.W7b-impl.md`) |
| **J.W7c** *(in-flight live-audit band — not an originally-planned wave)* | **THE LIVE-AUDIT REFINEMENT** — the user's binding mid-implementation U-register U1–U8 (golden dock proportions, shrunken transport, easing card, conditional selects, spring redesigned first-principles + keyframes variant, sequence `<decision>`, motion-path redesign, the 03.21.14 pane folded into U5) | the live demo surface (the named isomorphic exception) · glass-ui `~3.11.2` consumed | Per-U named deltas with `file:line`; `proof:live-session` budget 0; the visual-lock golden RE-CAPTURED in-motion; glass-ui NEVER patched (the two gaps booked RF-16/RF-17) | **IMPLEMENTED · `377eb3e`** — all eight U-rows dispositioned (six REDESIGN/REFINE, one DECISION-redesign, U8 folded into U5); the spring panel redesigned (`SegmentedTabs` + the single `.preset-grid`/`.preset-cell` surface — the W7c surface evolution the `proof:control-surface-single-writer` + `proof:easing-editor-live` probes track); nothing un-enumerated changed (`waves/J.W7c-impl.md`) |
| **(P0)** *the subject-write play regression* | **THE P0 SUBJECT-WRITE FIX** (born during impl; the missing oracle the whole battery lacked) — `_frame` split into `_advance` (playhead) + `_renderFrame` (the `interpFrames apply=true` subject write); the battery's "play is live" gates read idle-bob/orbital/tilt motion, never the subject write itself | `src/animation` (the permanent engine rule) · the live subject-animates oracle | `proof:subject-animates` born-RED witnessed (the missing oracle) → GREEN; tiered into `proof:correctness` | **IMPLEMENTED · `a2c3a5b`** — the seam fix + the new `proof:subject-animates` correctness gate that the whole live battery had been blind to (`FINAL.md §4`) |
| **(re-pin)** *glass-ui currency* | **THE glass-ui RE-PIN** `~3.9.0 → ~3.11.2` — the currency advance (sliders cured upstream); the consume-edges re-verified on 3.11.2 | the published glass-ui consume contract (inv-16) | consume-edges verified on `3.11.2`; PUBLISHED-consumed only, ZERO sibling patch | **IMPLEMENTED · `56aa00f`** — the currency re-pin; W7c consumes `3.11.2` idiomatically (`FINAL.md §Commit ledger`) |
| **J.WZ** | **the close** — FINAL.md (held to inv ε — each boundary claim cites its observed oracle); the prompt-recap (every fold row dispositioned); **the chronic-closure SUBSTRATE TRANSITION** (the meta-gate's parse target moves from I's PROGRESS to THIS J ledger in the same motion the J ledger becomes authoritative); the changeset version cut + npm publish (USER-DOMAIN, Mike Babb, confirm-first — the honest minor); the T5 memory update; the auto-deploy round-trip RE-observed on the close merge | — | `proof:all` GREEN where `proof:all == CI`; the close merge's own CI run auto-deploys (the J.W0 oracle re-witnessed on the close itself); the published tarball passes `proof:published-surface`; the J ledger terminal (zero un-dispositioned rows) | **IMPLEMENTED (the close) · the J close tree `b0a277b`** — `docs/tranches/J/FINAL.md` authored (held to inv ε, the five-boundary report, single-voiced); the chronic-closure SUBSTRATE TRANSITION EXECUTED (`proof-chronic-closure.mjs` re-pointed `I/PROGRESS.md → J/PROGRESS.md §"Open deferrals"`, the grammar evolved WITH it, the gate GREEN on the terminal J ledger); this board + the §"Open deferrals" ledger made terminal; the T5 memory rule narrowed (USER-VISIBLE). The honest-minor cut (`4.1.0 → 4.2.0`) + npm publish + the close-merge auto-deploy RE-observation are USER-DOMAIN (Mike Babb, confirm-first) — fired AFTER this commit, the `release.yml` first-ever run id recorded at publish |
...
| **props-destructure (T5)** | UNPOLICED + 6 live Vue-3.5 sites — **NARROWED to its true kernel at the J close · the memory file UPDATED (USER-VISIBLE)** | `audit/precepts.md §2-F3`; the narrowed kernel: only a destructured prop passed INTO a composable (the reactivity-losing INTO-a-closure leak) is gated; template/`watchEffect`/`computed`-body destructure is platform-idiomatic post-Vue-3.5 and NO LONGER policed. The MEMORY rule `feedback_props_destructuring.md` is REWRITTEN to the narrowed kernel in ONE motion (no stale rule beside its replacement — no-legacy applies to memory); the 6 live Vue-3.5 sites are idiomatic and correct |
...
**THE chronic-closure parse substrate (for `proof:chronic-closure`) — AUTHORITATIVE as of the J
close.** This is the consolidated J open-deferrals ledger built from `audit/deferred-ledger.md` +
`J.md §fold bands`. Every row carries born-tranche, chronicity count, disposition, owning wave, and
the gate/evidence.

> **SUBSTRATE-TRANSITION NOTE (binding — EXECUTED at J.WZ; this ledger is now the parse target).**
> Through J's development + implementation phases the authoritative parse target for
> `proof:chronic-closure` was `I/PROGRESS.md §"Open deferrals"`. **At the J.WZ close the substrate
> TRANSITION was EXECUTED in ONE motion:** the meta-gate's single path constant moved
> `I/PROGRESS.md → J/PROGRESS.md §"Open deferrals"` (`scripts/proof-chronic-closure.mjs:109`
> `CHRONIC_LEDGER`), and the grammar EVOLVED WITH the ledger (header-aware columns + the disposition
> bands + the Chronicity-integer ≥4 EXIT-ONLY enforcement) so the gate BITES on the new substrate —
> never a vacuous swap. **The transition is GATED, non-vacuity PROVEN:** the gate was re-run against
> deliberately-malformed planted J-ledger rows (a FOLD citing a resolving-but-source-shape gate; a
> HANDOFF targeting an unpublished future version `9.9.9`; a ≥4-tranche bare BOOK) and REDDED on all
> three clause shapes (the born-RED witness — S4 rules 3/(a)/P-invariant-28); the probes were removed
> and the gate GREENS on this clean terminal ledger (75 rows parsed, zero un-dispositioned). The
> binding contract every row honors: a kf-runtime-closing row (FOLD / VERIFY-ONLY) cites a RUNTIME
> CORRECTNESS gate witnessed born-RED on a defect tree OR names its terminal non-gate mechanism; a
> HANDOFF row targets ONLY a PUBLISHED version or a kf-owned consume-edge, never a future version /
> unreleased commit (the B7 vaporware lesson); a ≥4-tranche row carries an EXIT-shaped disposition.
> The narrative bands §4a–§4i carry the full live-state probe + disposition; THIS table is the gate's
> authoritative parse target.
>
> **CHRONICITY COLUMN SHAPE (binding for the parse target).** Because this table IS the
> `proof:chronic-closure` parse substrate (as of the J close), the Chronicity cell MUST be machine-readable: every row
...
|---|---|---|---|---|---|
| **scene-control-dfa** deploy-block + product lag ★ | I (post-close) | 1 (net-new at I close) | **FOLD (P0) — LANDED `09a56bf`** | **J.W0** | `proof:control-surface-single-writer` GREEN — the dock projection born-correct from the DFA (the `navToScene` per-expected-state predicate, never the SOURCE scene's stale label); **born-RED** on CI run `27228309606` trigger='null'-under-load BEFORE the cure. The OBSERVED green-CI→auto-deploy round-trip closes it terminally: CI `27310054675` → deploy `27310920981` (`event == workflow_run`) → live `index-DiVbdzH3.js` (`waves/J.W0-impl.md`, `audit/wave-I.WZ-postclose.md §C`) |
| **The 8-commit post-close tail** (no tranche home) | I (post-close) | 1 | **FOLD (P1) — LANDED `09a56bf`** | **J.W0** | Terminal non-gate mechanism: the adoption record names each commit's disposition over the `git log a4b1472..master` corpus (the tail's load-bearing CI-deploy hazard is the scene-control-dfa block, closed by the OBSERVED round-trip above); the ci.yml YAML-invalidity since H FIXED and witnessed by the first-ever green Linux CI run `27310054675` (`waves/J.W0-impl.md §S1`, `audit/wave-I.WZ-postclose.md §A`) |
| **`proof:all` ≠ CI roster** (GC-2/F5/§E) ★ | I | I (1), re-found J | **FOLD (P0) — LANDED `8c66e3e`** | **J.W3** | Terminal non-gate mechanism (a CI-roster equivalence, not a browser actuation): the two-way `ci-coverage` clause measured `proof:all == CI` both ways (no gate hard-gates CI without a tier; no tier gate absent from CI) — born-RED on the 3 CI-only orphan gates BEFORE they entered a tier; ADVERSARIALLY VERIFIED (`audit/gate-census.md` GC-2, `audit/wave-I.WZ-postclose.md §E`) |
| **`IN_CI` triplication / 3 strategies** (WZ-5/CICD-3) | I (post-close) | 1 | **FOLD (P1) — LANDED `8c66e3e`** | **J.W3** | Terminal non-gate mechanism: the 3 duplicate `IN_CI` strategies REMOVED, collapsed to ONE `scripts/lib` `IN_CI`+`observe()` helper with the per-gate DECLARED P6 posture; the stale-refs purge grep clean; net-deletion measured (`audit/wave-I.WZ-postclose.md §D`, `waves/J.W3-impl.md`) |
...
| **ENG-1 sibling per-card serializer** (latent B1-class) ★ | I | I (1) | **FOLD (P1) — LANDED `e8ff39e`** | **J.W1** | `proof:engine-no-throw-on-play` (EXTENDED) GREEN — the EDITOR per-card pane round-trips a `var()`-bearing animation through the ONE serialize-from-template authority; the sibling DOM-resolving path DELETED in the same motion; **born-RED** on the pre-fix tree (the sibling path threw on the var()-bearing card) (`audit/engine-core.md` ENG-1, `waves/J.W1-impl.md §S1`) |
...
| **SEAM-3 round-trip fixture trio** (var()/calc()/matrix3d()) | I | I (1) | **FOLD (P2) — LANDED `e8ff39e`** | **J.W1** | Terminal non-gate mechanism (a unit-test corpus, not a browser actuation): the roundtrip-fidelity corpus (`test/fixtures/keyframes/manifest.json`) gained a `var()` + `calc()` + `matrix3d()` fixture each — born-RED on the pre-fix corpus (covered NONE; serialize-from-template round-trips them verbatim now, 29 tests over the de-vacuoused corpus) (`audit/parsing-units-valuejs-seam.md` SEAM-3, `waves/J.W1-impl.md`) |
...
| **W4-3/W4-4 drag seam residue** (B6 latent class) ★ | I | I (1) | **FOLD (P1) — LANDED `7023e15`** | **J.W2** | `proof:drag-gesture` GREEN, EXTENDED to EasingCurveCanvas + PlaybackRibbon (real `page.mouse` drag, no text-selection, the gesture lands — 6 surfaces); **born-RED** on the pre-fix tree (the two recovered surfaces bypassed the shared seam — the drag selected text instead of scrubbing) (`audit/wave-I.W4.md`, `waves/J.W2-impl.md §S1`) |
| **DS-1 / S4-stretch selectedControl single-writer** | I | I (1) | **FOLD (P1) — LANDED `7023e15`** | **J.W2** | `proof:control-surface-single-writer` GREEN — a live scene sweep asserts the rendered control surface == the DFA projection on every entry; the CubeScene rogue write DELETED (the no-rogue-writer grep clean); **born-RED** on the pre-fix tree (CubeScene wrote `storedControls.selectedControl` directly, bypassing the single authority) (`audit/demo-scenes.md` DS-1, `waves/J.W2-impl.md §S2`) |
| **M2 mobile sheet re-open scroll latch** | I (CH-3 leg) | 3 (D,H,I via CH-3) | **FOLD (P1) — LANDED `7023e15`** | **J.W2** | `proof:sheet-reopen-scroll` GREEN — open→close→RE-OPEN on 390×844 scrolls to content (`isPanelTransitionDone` off the spring settle, NOT the CSS `transitionend` that never fires on the spring-driven sheet); **born-RED** on the pre-fix tree (the re-open latched, content unreachable) (`waves/J.W2-impl.md §S3`) |
...
| **FVT-2/INVE-2 live-session axes** ★ | I | I (1) | **FOLD (P1) — LANDED `5fd5df4`+`a9f4b2a`** | **J.W4** | `proof:live-session` + `proof:live-session-mobile` GREEN — the MOBILE (390×844 `hasTouch`) / REDUCED-MOTION / DARK / KEYBOARD legs + the every-scene sweep, budget 0 per leg; each leg **born-RED** witnessed against a PLANTED dist defect (mutate built dist → leg reds → byte-restore shasum-verified → green). INVE-2 CURED (`audit/final-vs-tree-inv-epsilon.md` INVE-2/P0-2, `waves/J.W4-impl.md`) |
| **CH-3 mobile chronic** (desktop-certified) ★ | D(D10) | 3 (D,H,I) | **FOLD (P1) — LANDED `5fd5df4`** | **J.W4** | `proof:live-session-mobile` GREEN — CH-3 re-certified on a MOBILE oracle (`sheet.bottom ≤ menubar.top` measured ON the 390×844 viewport at BOTH detents); **born-RED** on a planted occlusion (`waves/J.W4-impl.md`) |
| **EP-3 flip/drag/draw-svg uncovered exports** | I | I (1) | **FOLD (P1) — LANDED `5fd5df4`** | **J.W4** | Terminal non-gate mechanism: dispositioned per spec as a recorded BOOK — the uncovered-export list enumerated in the published-surface taught-or-enumerated roster so the packed tarball ships nothing untaught; no demo home elected (`audit/engine-periphery.md` EP-3, `waves/J.W4-impl.md`) |
| **SCOPE-1 the 16-export orchestration tier** (unpublished) ★ | E→I | 5 (E,F,G,H,I) | **FOLD (P1) — LANDED `9b475aa`** | **J.W5** | Terminal non-gate mechanism (the published-surface oracle — a tarball/source/README equivalence, not a browser actuation): the packed tarball surface == the source exports == the README taught API; **born-RED** on the current tree's 9 untaught exports BEFORE the README grew to the full roster (`audit/scope-adversary.md §0`, `waves/J.W5-impl.md §S1`) |
| **The doc-rot band** (LS-1..8 / SEAM-0 / CLAUDE.md) | I | I (1) | **FOLD (P1) — LANDED `9b475aa`** | **J.W5** | Terminal non-gate mechanism: root CLAUDE.md REWRITTEN to the tree (the real 28-module animation tree, the real 69-file test count); the README examples EXECUTE against the built dist (the readme-runs runner, 17/17 snippets, the docs oracle is runnable truth) (`audit/legacy-sweep.md`, `audit/deferred-ledger.md §H`) |
| **BP-1 `_redirects` in the npm tarball** | I | I (1) | **FOLD (P1) — LANDED `9b475aa`** (+`c6c3c37` files-negation) | **J.W5** | Terminal non-gate mechanism: the packed tarball (`npm pack --dry-run`) contents == declared files, `_redirects` REMOVED; **born-RED** on the current tree (the `_redirects` leak red it) (`audit/build-packaging-release.md` BP-1, `waves/J.W5-impl.md`) |
| **ENG-6 AnimationEngine interface drift** | I | I (1) | **FOLD (P2) — LANDED `9b475aa`** | **J.W5** | Terminal non-gate mechanism: the hand-maintained interface drift-gated in the published-surface tripwire; **born-RED** on a planted interface drift (`audit/engine-core.md` ENG-6, `waves/J.W5-impl.md`) |
| **The design protagonist + grammar band** (8 panes) | I | I (1) | **FOLD (P1) — LANDED `8c7910b`** | **J.W7a** | `proof:appearance-suffusion` (a)–(g) GREEN — the per-finding RUNTIME asserts on the live page: mobile hero/subject overlap == 0 px; the ghost rail ABSENT; amiga rounded-glass computed style; subjects sized/centered/plated; **born-RED** on the pre-suffusion tree (the overlap, the ghost rail, the gray amiga slab each red it) (`audit/design/pane-*.md`, `audit/design/cross-hierarchy.md`, `waves/J.W7a-impl.md`) |
| **CP-1 the two-colour-system drift** (`--ball-tone` seam) | I | I (1) | **FOLD (P1) — LANDED `8c7910b`** | **J.W7a** | `proof:appearance-suffusion` GREEN — the `.progress-ball` `--ball-tone` parameterization (the ONE seam, NOT 8 per-scene overrides); the aquamarine literal DELETED; computed `--ball-tone` == icon hue per scene; **born-RED** on the pre-fix tree (the one-green subjects against the six-colour icon rainbow) (`audit/design/cross-color-pops.md` CP-1, `waves/J.W7a-impl.md`) |
| **TYP-2 the display-voice concentration** | I | I (1) | **FOLD (P1) — LANDED `8c7910b`** | **J.W7a** | `proof:appearance-suffusion` GREEN — the Instrument Serif display register present at the named scene-identity moments (computed font-family resolves it); the math readouts at the MetricBadge/AnimatedDigit PROMOTION rungs; **born-RED** on the pre-suffusion tree (the display voice at ONE doorway only) (`audit/design/cross-typography.md` TYP-2, `waves/J.W7a-impl.md`) |
| **W6-3 / S3 substrate-depth legibility** (the glass reads as glass) | I | I (1) | **FOLD (P1) — LANDED `8c7910b`** | **J.W7a** | An un-homed FOLD-tagged P1 the harden-lane caught (S3 was a perpetual punt with NO terminal home — deferred NON-BLOCKING (M-2), never KILLed: `audit/wave-I.W6.md §6`, `impl/I.W6.md:36`), homed in the W7a glass-identity band (adjacent the amiga slab XH-3). `proof:appearance-suffusion` GREEN — the substrate-depth legibility runtime assert: the page substrate computes as refracting glass, NOT a near-white plate; **born-RED** on the pre-suffusion tree (the substrate read as a flat plate). The appearance-depth residue P-invariant-28 forbids leaving un-rowed is closed (`waves/J.W7a-impl.md`) |
| **STY-1..6 + the consume-to-delete band** | I | I (1) | **FOLD (P1/P2) — LANDED `ad84961`** | **J.W7b** | Terminal non-gate mechanism (set (i), consumed on PUBLISHED glass-ui `3.9.0`): SegmentedTabs/ToggleChip/ScrubberTimeline/.fade-slide PRM hand-rolled twins DELETED in the SAME motion the published primitive is consumed (the gone-grep clean, pixel-isomorphism proven on the pre-J visual-lock baseline); gold-shimmer dedup STY-1 (`audit/styling-design-system.md`, `waves/J.W7b-impl.md §A`) |
| **FB-2 async sync-step half** ★ | F | 4 (F,G,H,I) | **FOLD — EXIT-ONLY · LANDED `40fc605`** (probed → LAND) | **J.W6** | Terminal measurement artifact (a bench, not a browser actuation): `event-ordering` authored + the sync-step half MEASURED — microtask turns/frame **1.998 → 0** (Animation·K=8) and **4.993 → 0** (Group·32cells) = −100%; **born-RED ×2** before the conversion was trusted; LANDED on the numbers (`waves/J.W6-impl.md §S1`, `audit/deferred-ledger.md §3`) |
| **SoA `lerpArray`** ★ | E(G-2) | 5 (E,F,G,H,I) | **FOLD — EXIT-ONLY · LANDED `40fc605`** (benched → ADOPT) | **J.W6** | Terminal measurement artifact: benched on the real-K corpus — K=8 full-pipeline 0.0928 → 0.0056 ms = **16.6× = 94.0% reduction** (threshold 20% cleared 4.7×); ADOPTED with the 3-clause guard, the FrameCompiler transposition the elected separate authorized motion (`audit/perf-frontier.md`, `waves/J.W6-impl.md §S2`) |
| **FB-5 intrinsic-size `0→auto`** ★ | E/F | 5 (E,F,G,H,I) | **FOLD — EXIT-ONLY · KILL `40fc605`** (verified-Baseline → KILL) | **J.W6** | Terminal measurement artifact: the LIVE cross-engine Baseline probe (2026-06-10) — Chromium-only since Chrome 129, neither Gecko nor WebKit shipped, no Baseline date scheduled; a Chromium-only path is not a kf commitment → KILLED reasoned, grep=0, zero code touched (`waves/J.W6-impl.md §S3`, `audit/deferred-ledger.md §3`) |
| **FB-6 `Mod+K` palette** ★ | F | 4 (F,G,H,I) | **FOLD — EXIT-ONLY · KILL `40fc605`** (owner → KILL default) | **J.W6** | Terminal disposition: owner decided KILL (the spec DEFAULT) — deleted at E.W11, absent from the J.W7a suffusion, no owner election across 4 tranches; the residue swept (the `style.css:29` comment REMOVED), grep clean (`waves/J.W6-impl.md §S4`, `audit/deferred-ledger.md §3`) |
| **PF-1 Three.js named imports** | I | I (1) | **MEASURE-FIRST · KILL `40fc605`** (measured → estimate falsified) | **J.W6** | Terminal measurement artifact: the measured `vendor-three` chunk delta is **exactly 0 bytes** (`B9C4oDST` byte-identical before/after — rolldown already tree-shakes the namespace); the 100–200 KB estimate FALSIFIED, the refactor REVERTED → KILLED (`audit/perf-frontier.md` PF-1, `waves/J.W6-impl.md §S5`) |
| **PF-3 Monaco static-edge / EF-3 parseLinearStops shim** | G/I | I (1) | **VERIFY-ONLY · LANDED `40fc605`** (re-verified) | **J.W6** | Terminal measurement artifact (a node probe, not a browser actuation): the Monaco static-edge re-verified by the fresh-build edge-list node probe (the I-era win held); the `parseLinearStops` retirement node probe read the value.js E1 status — KEEP-with-probe, the HANDOFF rides the next re-pin unchanged (`audit/perf-frontier.md`, `audit/engine-periphery.md` EF-3, `waves/J.W6-impl.md §S6/§S7`) |
| **CH-5/B1+B5 `"......"` crash** ★ | A(W0)→H | 3 (A,H,I) | **VERIFY-ONLY** (TERMINATED) | J.W1 (re-run) | `proof:engine-no-throw-on-play` GREEN — re-run on the built dist; **born-RED** in its origin tranche (the empty-input parse threw on play), `parseCSSValueUnit("")=>{value:0}` no throw (node probe, value.js 0.11.2) (`audit/deferred-ledger.md §1-A`) |
| **CH-6/B2 `_gen` DFA suspend crash** | H | 2 (H,I) | **VERIFY-ONLY** (TERMINATED) | J.W2 (re-run) | `proof:fsm-suspend-resume-live` GREEN — bind-proof RAFPlayback + `useRafScene`; **born-RED** in its origin tranche (the `_gen` suspend threw) |
| **CH-7/B4 lost easing editor** | H | 2 (H,I) | **VERIFY-ONLY** (TERMINATED) | J.W2 (re-run) | `proof:easing-editor-live` GREEN — the unified `EasingEditor.vue`; **born-RED** in its origin tranche (the editor was lost on scene switch); the scene-control-dfa lag that partially undercut the single-authority claim is itself CLOSED at J.W0 |
| **CH-8/B3 amiga floats** | H | 2 (H,I) | **VERIFY-ONLY** (TERMINATED) | J.W4 (re-run) | `proof:amiga-subject-is-pivot` GREEN — subject=pivot geometry, content-visibility shed; **born-RED** in its origin tranche (the amiga subject floated off-pivot) |
| **CH-9/B6 square drag** | H | 2 (H,I) | **VERIFY-ONLY** (TERMINATED) | J.W2 (re-run) | `proof:drag-gesture` GREEN (EXTENDED in J.W2) — the shared `useDragScrub`; **born-RED** in its origin tranche (the square drag selected text) |
| **CH-10/B9+K dev ENOENT + title** | H | 2 (H,I) | **VERIFY-ONLY** (TERMINATED) | J.W4 (re-run) | `proof:icon-paint-live` GREEN — one build root, `<title>keyframes.js</title>` (`demo/app/index.html:14`); **born-RED** in its origin tranche (the dev ENOENT + the missing title) |
| **CH-1/B7 specular sheen** ★ | D(D14)→H | 3 (D,H,I) | **VERIFY-ONLY** (TERMINATED) | J.W4 (re-run) | `proof:specular-absent-at-rest` GREEN — glass-ui `3.9.0` consumed (the flat default); **born-RED** in its origin tranche (the cartoon specular sheen at rest); `proof:specular-handoff` DELETED, the self-guard `:51-54` asserts its absence (RETIRED/required-ABSENT) |
...
| **DC-8 scene-swap VT dead-CSS** ★ | A | 5 (A,C,D,H,I) | **VERIFY-ONLY** (TERMINATED — the one A→I chronic that finally died) | — | Terminal non-gate mechanism: the LIVE `startViewTransition` consumer (`useSceneTransition.ts:2,32`) — the dead-CSS grep stays 0, no re-open; **born-RED** historically (the VT CSS was dead — animated nothing); the fourth-defer prohibition HONORED (`audit/deferred-ledger.md §C`) |
| **value.js empty-input contract** (B1 value-half) | I | I (1) | **VERIFY-ONLY** (LANDED + PUBLISHED) | J.W1 (pin) | `@mkbabb/value.js ^0.11.2` PUBLISHED-consumed; the contract holds (node probe — `parseCSSValueUnit("")` no throw); the kf-side pin LANDED in J.W1 (`e8ff39e`), born-RED-able on a value.js regression |
| **glass-ui specular consume-edge** (B7) ★ | H | 2 (H,I) | **VERIFY-ONLY** (LANDED — published 3.9.0) | — | Terminal non-gate mechanism: the flat-default specular consumed from PUBLISHED glass-ui `~3.9.0` (the `::before{content:none}` workaround REJECTED — DELETED, the hand-rolled twin gone-grep clean); corroborated by `proof:specular-absent-at-rest` GREEN — **born-RED** in its origin tranche (the cartoon specular sheen rendered at rest) (`audit/deferred-ledger.md §1-A`) |
| **C-1 value.js next-slice** (VJ-1..9; the EF-5 charter-v2 / tranche-M status) ★ | C | 7 (C,D,E,F,G,H,I) | **OUT / sibling-HANDOFF — BOOK-reaffirm** (chronic-by-design) | — | The published base is consumed: value.js `0.11.2` is PUBLISHED-consumed (the pin holds, B1 contract landed); the VJ-1..9 next-slice rides the NEXT re-pin as a kf-owned consume-edge (ZERO kf edit), each item flipping the instant its primitive PUBLISHES — `parseLinearStops`/`getPointAtLength` are correctly absent in the consumed 0.11.2, the consume-edge born-RED-able the moment value.js ships them. **Named OUT-band follow-up (EF-5):** J re-confirms the value.js **tranche-M** status at the next re-pin and points at `valuejs-sota-handoff-v2.md` — the verify-action is not dropped under the VJ-1..9 umbrella. NOT a J wave (`audit/deferred-ledger.md §1-D`, `audit/recap-EF.md` EF-5, `F/FINAL.md:89-93`) |
...
| **dock double-click** (memory) | pre-A (glass-ui root) | 9 (pre-A…I, recurring) | **OUT / sibling-HANDOFF — VERIFY-ONLY** | — | Terminal non-gate mechanism: the glass-ui root fix landed (published-consumed); the demo's transition-intercept workaround REMOVED — the grep at `AnimationMenuBar.vue:106` is clean (the dock single-clicks); a glass-ui-root resolution, never kf-patched |
...
| **GH-secret creds vs sibling-.env bypass** | I | I (1) | **OUT — VERIFY-ONLY** (J confirms GH secrets) | J.W0 | Terminal non-gate mechanism: J VERIFIED the GH repo secrets exist + match the CF account (`deploy-pages.yml:66-67` consumes `CLOUDFLARE_API_TOKEN`/`CLOUDFLARE_ACCOUNT_ID`/`NPM_TOKEN`) — proven by the OBSERVED auto-deploy round-trip (deploy run `27310920981` consumed them and served live bytes), NOT the sibling-`.env` bypass; the manual `wrangler` path REMOVED (`audit/wave-I.WZ-postclose.md §F`, `waves/J.W0-impl.md §S4`) |
...
| **C-6 engine line-ceiling watch** ★ | D | 6 (D,E,F,G,H,I) | **VERIFY-ONLY** (CONTAINED — gated, 25 lines headroom) | J.W1 (respects ceiling) | Terminal non-gate mechanism (a source-decomposition ceiling, measured): `engine.ts` = 1375 lines, cap 1400 (`proof-decomposition.mjs:132`) — a standing gate that reds on a planted overshoot, not a punt; J.W1's totality motion respected the ceiling (`audit/deferred-ledger.md`) |
...
### 4a — The four ≥4-tranche riders (EXIT-ONLY — no fifth ride; P-invariant-28) — **EXITED at `40fc605` (J.W6)**

These four had ridden ≥4 tranches as bare BOOK/MEASURE-FIRST with NO probe authored. Per
P-invariant-28, continuing them a fifth time as "BOOK" IS the perpetual punt the invariant forbids
(`audit/deferred-ledger.md §3`). **Each EXITED J via a MEASUREMENT ARTIFACT or a reasoned KILL —
the full termination ledger with each artifact pasted INLINE is `waves/J.W6-impl.md`.** The J close
ledger carries ZERO rows tagged MEASURE-FIRST without a measurement.

| Rider | Born | Tranches | TREE state at J open | **The EXIT (J.W6 `40fc605`, artifact in `J.W6-impl.md`)** |
|---|---|---|---|---|
| **FB-2 async sync-step half** | F | 4 (F,G,H,I) | `engine.ts:840 async advanceTo` + `group.ts:469` async | **LAND** — `proof:event-ordering` authored; microtask turns/frame **1.998 → 0** (Anim·K=8) + **4.993 → 0** (Group·32cells) = −100%; born-RED ×2 (§S1) |
| **SoA `lerpArray`** | E(G-2) | 5 (E,F,G,H,I) | absent — `lerpArray` grep=0 in `src/` | **ADOPT** — benched: K=8 full-pipeline 0.0928 → 0.0056 ms = **16.6× = 94.0%** (threshold 20% cleared 4.7×); the 3-clause guard landed (§S2) |
| **FB-5 intrinsic-size `0→auto`** | E/F | 5 (E,F,G,H,I) | no `interpolate-size`/`calc-size` path (grep=0) | **KILL** — the LIVE cross-engine Baseline probe: Chromium-only since Chrome 129, no Gecko/WebKit, no Baseline date — not a kf commitment (§S3) |
| **FB-6 `Mod+K` palette** | F | 4 (F,G,H,I) | only a CSS ref; no `CommandPalette`/`cmdk` component | **KILL** (the spec default) — no owner election across 4 tranches; the residue swept, grep clean (§S4) |
...
These three fire AFTER the close commit, confirm-first — the gated pre-conditions are GREEN on the
close tree (`proof:all == CI` GREEN, `proof:published-surface` GREEN on the cut tree, the J.W0
auto-deploy round-trip OBSERVED: CI `27310054675` → deploy `27310920981` → live `index-DiVbdzH3.js`).

| Item | Disposition |
|---|---|
| **The version cut** | the honest MINOR `4.1.0 → 4.2.0` (J.W5 authored `.changeset/tranche-j.md`, both pending `patch` changesets consumed/deleted into it — H's patch subsumed, never published; the additive E→I orchestration tier ⇒ minor, the value.js floor advance an honest contractual tightening) |
| **The npm publish** | USER-DOMAIN at J.WZ, confirm-first — rides `release.yml`'s first-ever run on the pushed `v4.2.0` tag; the run id + published version recorded at publish (the FINAL `[RUN-ID / VERSION recorded at publish]` placeholder) |
| **The close-merge deploy** | the auto-deploy round-trip RE-observed on the close merge (the J.W0 oracle re-witnessed on the close itself, via the AUTO `workflow_run`-on-`push` arm — NO `workflow_dispatch` substitute) |
...
- **P-invariant-28 at the J level (EXECUTED):** every carry in §"Open deferrals" exits with a
  terminal J disposition; the four ≥4-tranche riders EXITED via a measurement artifact or a reasoned
  KILL (FB-2 LAND, lerpArray ADOPT, FB-5/FB-6 KILL — `40fc605`); the net-new `scene-control-dfa`
  chronic the I FINAL could not enumerate is CLOSED P0 via J.W0's observed round-trip (`09a56bf`);
  zero perpetual punts; zero MEASURE-FIRST rows without a measurement.
- **The chronic-closure substrate transition was EXECUTED at J.WZ (gated, non-vacuous):** the
  meta-gate's path constant moved `I/PROGRESS.md → J/PROGRESS.md §"Open deferrals"`
  (`proof-chronic-closure.mjs:109`) in the SAME motion the J ledger became authoritative; the grammar
  evolved WITH the ledger so the gate BITES (proven born-RED on planted malformed rows, GREEN on the
  clean 75-row terminal ledger). Every kf-runtime closure cites a runtime/correctness gate witnessed
  born-RED on a defect tree; every HANDOFF targets a published version / kf consume-edge; every
  ≥4-tranche row is EXIT-shaped.
