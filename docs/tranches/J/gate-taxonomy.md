...
| `proof:visual-lock` | observe-only | pixel-render | Re-baseline the masked pixel reference INSIDE the CI Linux container (the same rasterizer/font-hinting stack as the runner), so the golden is captured on the comparison host — eliminating the cross-OS rasterization delta. | cross-OS render; re-baseline in-container |
