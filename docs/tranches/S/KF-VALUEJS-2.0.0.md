...
The pass-2 "`extractFunctions` is dist-only, a fresh build drops it" finding was **FALSE** — a stale-worktree artifact (the measuring lane sat at 1.0.2). The symbol is in committed source since 1.1.0: `src/parsing/extract.ts:124`, re-exported at `src/index.ts:315` + `src/subpaths/parsing.ts:47`; a fresh build keeps it. value.js landed a fresh-build `.d.ts` regression assertion at R.W1 as a lock against future barrel drift. **Your consumed symbol is safe; the `^2.0.0` re-pin resolves everything in this letter.**
...
| # | Item | R action |
|---|---|---|
| KF-2 | parse-that `^0.13.0` pin | Current — no action. Book the `^1.0.0` re-pin on kf **S.H4**'s cut (H1+H2 land in one 1.0.0 publish). Verify at re-pin = span-absence (0 `*Span` consumers) **+ the 4 `.chain()` sites** (`stylesheet.ts:831`, `parsing/utils.ts:182`, `color.ts:600,651` — the stylesheet + color line refs shifted at 2.0.0's KF-1 grammar rewrite); 1.0.0 also retires `chainError` (value.js passes 0) and FIXES `chain()` falsy-seed semantics (value.js seeds non-falsy — verify, don't assume). Wait, don't pre-pin. |
...
| KF-7 | value.js `PropertyDescriptor` collides with the DOM lib global (a29 F3 / kf S.B6 S8) | value.js's exported `PropertyDescriptor` type name collides with the ambient DOM `PropertyDescriptor` global, so kf's API-Extractor roll-up collision-renames it to `PropertyDescriptor_2` in the published `dist/keyframes.d.ts` (`propertyRegistry: Map<string, PropertyDescriptor_2>`, `ResolvedKeyframes.properties`). A kf-LOCAL import alias does NOT fix it (API-Extractor renames by value.js's SOURCE symbol, not the importer's alias), and the two leak sites are consumer-observable (a `platform-adopt` test asserts on `propertyRegistry`; `properties` is a public `ResolvedKeyframes` field) so they cannot be `@internal`-trimmed. **DISPATCH**: rename value.js's export to a collision-free name (e.g. `CSSPropertyDescriptor`) on the 2.0.0 grammar-rename cadence — the same NO-legacy total-rename shape as KF-1's `CustomFunctionParameter`. Same defect class kf already fixed locally for `globalThis.Animation`→`KeyframesAnimation` / `OscillatorOptions`→`OscillatorConfig`; this one originates upstream. kf re-points its import + the `Map<string, …>` types at the value.js `^2.0.0` re-pin (KF-2 cadence). No immediate value.js gate. |
