...
- **S1 — The `.morph-ghost--from` FOLD (fold row 2; SPEC §2.1-2 "Genuine source").** Add the missing
  ...
  **PROVEN (pass3: ci-fix-proto.md §c): pre-fix exit 1 (`✗ morph-ghost--from ×1, first: demo/scenes/morph/MorphTarget.vue`) / post-fix exit 0** (`PASS: every referenced idiom-shaped class resolves to an owned definition`). The landed shape is **one CSS rule at `demo/scenes/morph/MorphTarget.vue` beside `.morph-ghost--to`** (`stroke-dasharray: 6 4; opacity: 0.55;` — a meaningful `from`-endpoint distinction reading as a longer dash vs the `--to` endpoint's denser dash, **not a no-op stub**); static gate, no browser, device-independent — confirmed verbatim, ready to land as the keystone's first motion.
...
- **S4 — The shared importmap harness fix (fold rows 10 + 12; SPEC §3 S.A0 item 4; §2.1-2; SA-2).**
  ...
  **PROVEN (pass3: ci-fix-proto.md §a): DM-13 `engine-no-throw-on-play` pre-fix exit 1 (`✗ [J.W1 b] TypeError: Failed to resolve module specifier "@mkbabb/value.js/math"`) / post-fix exit 0** (`✓ [J.W1 b] typed AnimationOptionError; ✓ conforming control compiles; PASS`) — the clean full GREEN. **DM-11b `subject-animates` pre-fix exit 1 (`waitForFunction __kfReady Timeout 30000ms`) / post-fix exit 1\*** — the **importmap root is DISCHARGED** (all three synthetic arms flip RED→GREEN: `✓ [raf] 36 distinct · ✓ [waapi] 37 · ✓ [group] 38`); the surviving red is the orthogonal **`[real-cube]` cold-path**, a *separate* born-RED folded into S6 (NOT the importmap).
  **S.A0(4) refinement CONFIRMED + REFINED (pass3: ci-fix-proto.md §a "S.A wave upgrade note"):** the SPEC prescription ("`@mkbabb/value.js/`: `.../dist/subpaths/` + serve the whole value.js `dist/` subtree") is correct **but incomplete as written** — **parse-that 0.13.0 is ALSO multi-chunk** (`dist/parse.js` imports `./core.js`, `./packrat-entry-*.js`), and value.js 1.2.0's `dist/value.js` imports `./units-*.js`, `./math-*.js`, … So the fix must **serve BOTH deps' full dist subtrees** (not value.js alone) behind a prefixed whole-subtree server, AND carry an **extensionless-subpath `.js` fallback** (importmap prefix substitution yields `.../subpaths/math` with no extension → serve `subpaths/math.js`). **Diff-shape** (identical ×2 across both scripts — p12's "ONE change ×2"): a `VENDOR_ROOTS` map `{ "/__kf-vendor__/value.js/": node_modules/@mkbabb/value.js/dist, "/__kf-vendor__/parse-that/": node_modules/@mkbabb/parse-that/dist }` served by a `serveVendor(urlPath, res)` prefix-walker (`if (!fs.existsSync(fp) && fs.existsSync(fp + ".js")) fp += ".js"`; the old `if (VENDOR[urlPath]) {…}` block becomes `if (serveVendor(urlPath, res)) { return; }`) + the importmap `{ "@mkbabb/value.js":".../value.js/value.js", "@mkbabb/value.js/":".../value.js/subpaths/", "@mkbabb/parse-that":".../parse-that/parse.js", "@mkbabb/parse-that/":".../parse-that/" }`. Resolution walked end-to-end: bare → `dist/value.js`; its `./units-*.js` → `dist/units-*.js`; `@mkbabb/value.js/math` → `dist/subpaths/math.js`; that file's `../math-*.js` → `dist/math-*.js` — all present. **Build precondition (both artefacts present simultaneously):** `npm run build:lib` (EXIT 0, empties `dist/gh-pages/`) → `npm run gh-pages` (EXIT 0, leaves `dist/keyframes.js`) — that order leaves BOTH artefacts DM-11b/13 need (pass3: ci-fix-proto.md §2).
- **S5 — The `demo-usability` gate-staleness re-point (fold row 5, gate-staleness bucket; SPEC §3
  ...
  **or** assert reachability at runtime — **~5–15 lines**. A green demo must not red on the gate's
  ...
  here.)
  **PROVEN (pass3: ci-fix-proto.md §e): pre-fix exit 1 (`✗ every scenes.ts id UNROUTED: cube, amiga, square, easing, spring, sequence, motion-path, morph, home`) / post-fix exit 0** (`✓ route-reachability: all 9 scene id(s) resolve a declared non-redirecting route`). **Diff-shape** at `scripts/proof-demo-usability.mjs` after the literal-name scan: `const routesAreGenerated = /\ballScenes\s*\.\s*map\b/.test(routerSrc) && /\bname:\s*[A-Za-z_$][\w$]*\.id\b/.test(routerSrc); if (routesAreGenerated) for (const id of sceneIds) routeNames.add(id);` — sound because `allScenes = [homeScene, ...scenes]` (`scenes.ts:233`) is the exact descriptor set `sceneIds` is parsed from, so the generated `name: s.id` routes **every** scene id by construction; hand-declared `name:"<literal>"` routes still honoured (literal scan runs first), the catch-all still carries `redirect:` with no `name:`. **Scope-honest:** verified the **static X-6 clause** flips GREEN; the gate's browser half (X-5 hero-gap / X-3 duplicate-aria) is a separate axis, untouched by this fix and not the p12-cited red — the worked exemplar of S.A2's fourth "stale-gate" bucket.
- **S6 — The two cheap genuine behavioral fixes that unblock the widest downstream surface (fold
  ...
  - **DM-14 the spring pause/resume-continuity defect:** a **genuine source fix** with a live
    ...
    **CAUSE PRECISELY LOCATED (pass3: ci-fix-proto.md §b; born-RED reproduced deterministically on macOS, byte-identical to CI):** a bounded live playwright probe on the built `dist/gh-pages/` spring scene clicked the visible "Pause animation" button (a real, hit button — NOT an aria/selector miss) and the machine's `perScene[spring].playing` stayed **true** — so it is a **genuine dispatch defect**, not selector drift. The chain: (1) spring is a **raw-rAF scene** (no real `AnimationGroup`); its bottom-bar transport binds to a **contract group** (`useContractAnimGroup.ts`) — one opacity-only placeholder anim, synthetically pre-started (`animationGroup.started = true; animationGroup.paused = !isPlaying`), driving no motion; (2) the dock "Pause animation" click → `useAnimationGroupPlayback.toggleAnimationGroup` → group non-empty + `started===true` → else-branch `animationGroup.toggle(); syncPlayState();`; (3) `syncPlayState()` reads `animationGroup.playing()`, but `toggle()`/`playing()` on a `started=true`, **never-run** group returns `playing=true`, so `App.onPlayStateChange(true)` (`useSceneMachineApp.ts:207`) dispatches **PLAY**, not PAUSE — the machine never leaves `playing`. **One-line root:** the contract-group transport derives emitted `playing` from `toggle()/playing()` on a synthetically-started, never-run group, decoupling the dock toggle from the scene-machine intent **for every raw-rAF scene** (spring/easing/sequence/motion-path). **Wave-shaped cure (verbatim — hand to the impl drive with this seam pinned):** for a scene that owns a `scenePlayback` adapter (the raw-rAF family), the dock transport must derive intent from the **scene-machine status**, not the contract group. One of — **(preferred, surgical)** in `useAnimationGroupPlayback.toggleAnimationGroup`, when the bound group is a non-authoritative contract host (flag it at construction, or detect `scenePlayback` ownership), emit `syncPlayState(!isPlaying.value)` instead of `animationGroup.toggle(); syncPlayState()` — so the dock toggle emits the true inverse and `onPlayStateChange` dispatches PAUSE; **guard the real cube/amiga path unchanged.** **(alternative)** Fix `AnimationGroup.toggle()/playing()` at the library level to be correct on a `started=true`, loop-never-run group — broader blast radius; only if the contract-group `started=true` synthetic is deemed a library contract, not a demo hack. **Gate:** re-run `proof:fsm-suspend-resume-live` clause (c) GREEN **and** `proof:subject-animates [group]` + the cube/amiga transport gates unchanged (regression guard). **Out of prototype scope** because `toggleAnimationGroup` is **shared with the real-group scenes (cube/amiga)**, whose `toggle()`/`playing()` are correct — any fix requires an iterative rebuild-gh-pages + full-browser-gate verify loop per candidate (the SPEC's own ½–1 day sizing); shipping an unverified patch would violate the no-threshold-games charter.
  - **The `subject-animates [real-cube]` cold-path residual (cold-entry family — LOCATED pass3: ci-fix-proto.md; surfaced only AFTER the S4 importmap unblocked the synthetic arms):** DM-11b's `realSceneHalf` reds `[real-cube] the demo cube's REAL subject did NOT receive the engine write — playhead stayed 0 → 0 (engineWrote=false)`. The gate clears localStorage, `navToScene("cube","Controls")`, clicks "Play animation" from **cold**, and the slider `aria-valuenow` never advances past 0. This is the **cold-entry** family — but p12's cited `scenePlaybackAdapters.ts:76-79` P0 cure (`if (!group.started) group.play()`) is **already present** on this tree, so the residual is a **deeper cold-start no-op on the cube's real group**, distinct from that adapter line; same class as DM-14 (dock/machine transport ↔ engine cold-start decoupling). **Wave-shaped cure:** a live repro-and-verify on the cube's real group's cold-start path — NOT a static-diff prototype, NOT a threshold game (it is a real subject-write red). **Owner:** the S.A0(6)/cold-entry behavioral wave, with the live repro-and-verify.
...
named owning wave.
**PROVEN refinement (pass3: ci-fix-proto.md):** S1/S2/S5 + the S4 DM-13 leg are proven **red→green exit 0** verbatim (the four device-independent static cures land as-is). The S4 clause "flips **both** DM-13 and DM-11b green" is refined by the reproduction: S4 discharges DM-11b's **importmap ROOT** (its three synthetic arms `[raf]/[waapi]/[group]` flip RED→GREEN) but DM-11b's gate stays exit 1 on the orthogonal **`[real-cube]` cold-path** born-RED — that residual is folded into **S6** (cold-entry family), so DM-11b greens across **S4 (importmap) + S6 (cold-start)**, not S4 alone. DM-14 and the `[real-cube]` cold-path are **cause-located, out of static-prototype scope** (both need the iterative rebuild-gh-pages + browser verify loop, seams pinned in S6) — they remain born-RED under S6 until the behavioral impl drive discharges them.
...
- **S5 — The FOUR disposition buckets (per-gate; SPEC §3 S.A2; SA-5).**
  ...
  **S.A0 item 5** — S.A2 owns the generic bucket, S.A0 owns the instance. **PROVEN (pass3:
  ci-fix-proto.md §e):** the `demo-usability` instance is a **worked exemplar** of this bucket — a
  static gate whose parser (`/\bname:\s*"([^"]+)"/g`) drifted from the R.W5 refactor it should
  tolerate (generated `name: s.id` routes); the ~5–15 LOC parser re-point flips it **exit 1→0**
  (the diff-shape is recorded at S.A0 S5), confirming the bucket disposition "re-point the parser, do
  not red on the gate's own obsolescence."
